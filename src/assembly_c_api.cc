...
assembly_object_t *assembly_object_create_from_shape(topo_shape_t *shape) { try {
...
assembly_object_t *
assembly_object_create_from_workplane(workplane_t *workplane) { try {
...
void assembly_object_free(assembly_object_t *obj) { try {
  if (obj)
    delete obj;
  }
  catch (const std::exception &e) {
...
topo_shape_t *assembly_object_get_shape(assembly_object_t *obj) { try {
...
workplane_t *assembly_object_get_workplane(assembly_object_t *obj) { try {
  if (auto wp =
...
_Bool assembly_object_is_shape(assembly_object_t *obj) { try {
...
_Bool assembly_object_is_workplane(assembly_object_t *obj) { try {
...
const char *assembly_element_get_name(assembly_element_t *el) { try {
...
topo_shape_t *assembly_element_get_shape(assembly_element_t *el) { try {
...
color_t assembly_element_get_color(assembly_element_t *el) { try {
...
topo_location_t *assembly_element_get_location(assembly_element_t *el) { try {
...
void assembly_element_free(assembly_element_t *el) { try {
  if (el)
    delete el;
  }
  catch (const std::exception &e) {
...
constraint_param_t *constraint_param_create_from_double(double value) { try {
...
constraint_param_t *constraint_param_create_from_double2(double x, double y) { try {
  return new constraint_param_t{
...
void constraint_param_free(constraint_param_t *param) { try {
  if (param)
    delete param;
  }
  catch (const std::exception &e) {
...
assembly_t *assembly_create(assembly_object_t *obj, topo_location_t *loc,
                            const char *name, color_t *color) { try {
...
void assembly_free(assembly_t *as) { try {
  if (as)
    delete as;
  }
  catch (const std::exception &e) {
...
assembly_t *assembly_copy(assembly_t *as) { try {
...
void assembly_remove(assembly_t *as, const char *name) { try {
...
void assembly_constrain(assembly_t *as, const char *q1, const char *q2,
                        int kind, constraint_param_t *param) { try {
...
void assembly_solve(assembly_t *as, int verbosity) { try {
...
void assembly_export_to(assembly_t *as, const char *path, int mode) { try {
...
topo_shape_t **assembly_shapes(assembly_t *as, int *size) { try {
...
topo_compound_t assembly_to_compound(assembly_t *as) { try {
...
assembly_element_t **assembly_get_elements(assembly_t *as, int *size) { try {
...
void assembly_element_list_free(assembly_element_t **list, int size) { try {
  if (list) {
...
void assembly_element_list_free_shallow(assembly_element_t **list) { try {
  delete[] list;
  }
  catch (const std::exception &e) {
    (void)e;
...
const char *assembly_get_name(assembly_t *as) { try {
...
topo_location_t *assembly_get_location(assembly_t *as) { try {
...
color_t assembly_get_color(assembly_t *as) { try {
  auto c = as->ptr->color();
...
assembly_object_t *assembly_get_object(assembly_t *as) { try {
...
assembly_t **assembly_children(assembly_t *as, int *size) { try {
...
void assembly_list_free(assembly_t **list, int size) { try {
  if (list) {
    for (int i = 0; i < size; i++) {
      delete list[i];
    }
...
void assembly_list_free_shallow(assembly_t **list) { try { delete[] list;   }
  catch (const std::exception &e) {
    (void)e;
  }
  catch (...) {
...
bool assembly_has_error(assembly_t *as) { try { return as->ptr->has_error();   }
  catch (const std::exception &e) {
    (void)e;
    return false;
  }
...
const char *assembly_error(assembly_t *as) { try { return as->ptr->error().c_str();   }
  catch (const std::exception &e) {
    (void)e;
    return nullptr;
  }
