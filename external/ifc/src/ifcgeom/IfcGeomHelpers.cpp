...
bool IfcGeom::Kernel_T::convert(const IFC_NAMESPACE::IfcSchema::IfcAxis2Placement3D* l, gp_Trsf& trsf) {
	...
	if (hasAxis) {
		IfcGeom::Kernel_T::convert(l->Axis(), axis);
	}

	if (hasRef) {
		IfcGeom::Kernel_T::convert(l->RefDirection(), refDirection);
	} else {
...
bool IfcGeom::Kernel_T::convert(const IFC_NAMESPACE::IfcSchema::IfcAxis1Placement* l, gp_Ax1& ax) {
	...
	IfcGeom::Kernel_T::convert(l->Location(),o);
	if ( l->hasAxis() ) IfcGeom::Kernel_T::convert(l->Axis(), axis);
	ax = gp_Ax1(o, axis);
...
bool IfcGeom::Kernel_T::convert(const IFC_NAMESPACE::IfcSchema::IfcCartesianTransformationOperator3D* l, gp_Trsf& trsf) {
	...
	gp_Dir axis2 (0.,1.,0.);
	gp_Dir axis3 (0.,0.,1.);
	if ( l->hasAxis1() ) IfcGeom::Kernel_T::convert(l->Axis1(),axis1);
	if ( l->hasAxis2() ) IfcGeom::Kernel_T::convert(l->Axis2(),axis2);
	if ( l->hasAxis3() ) IfcGeom::Kernel_T::convert(l->Axis3(),axis3);
...
bool IfcGeom::Kernel_T::convert(const IFC_NAMESPACE::IfcSchema::IfcPlane* pln, gp_Pln& plane) {
	...
	if ( l->hasAxis() ) IfcGeom::Kernel_T::convert(l->Axis(),axis);
	if ( hasRef ) IfcGeom::Kernel_T::convert(l->RefDirection(),refDirection);
	gp_Ax3 ax3;
...
bool IfcGeom::Kernel_T::convert(const IFC_NAMESPACE::IfcSchema::IfcAxis2Placement2D* l, gp_Trsf2d& trsf) {
	...
	if ( l->hasRefDirection() )
		IfcGeom::Kernel_T::convert(l->RefDirection(),V);
...
void IfcGeom::Kernel_T::set_conversion_placement_rel_to(const IfcParse::declaration* type) {
