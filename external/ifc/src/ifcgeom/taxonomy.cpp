...
bool compare(const ellipse &a, const ellipse &b) {
  ...
  return std::tie(a.radius, a.radius2) < std::tie(b.radius, b.radius2);
}
...
template <typename T>
typename std::enable_if<std::is_base_of<item, T>::value, int>::type
less_to_order(const T &a, const T &b) {
  const bool a_lt_b = compare(a, b);
  const bool b_lt_a = compare(b, a);
  return a_lt_b ? -1 : (!b_lt_a ? 0 : 1);
}
...
template <typename T>
int less_to_order_optional(const boost::optional<T> &a,
...
int compare(const boost::variant<boost::blank, point3::ptr, double> &a,
            const boost::variant<boost::blank, point3::ptr, double> &b) {
  bool a_lt_b, b_lt_a;
  if (a.which() == 0) {
    return 0;
  } else if (a.which() == 1) {
...
bool compare(const extrusion &a, const extrusion &b) {
  // @todo extrusions can also have non-identity matrices right? perhaps it's
  // time
  //       for a dedicated transform node and not on the abstract geom_item.
  const int order[3] = {less_to_order(a.basis, b.basis),
                        less_to_order(a.direction, b.direction),
                        a.depth < b.depth ? -1 : (a.depth == b.depth ? 0 : 1)};
  auto it =
      std::find_if(std::begin(order), std::end(order), [](int x) { return x; });
  if (it == std::end(order))
    return false;
  return *it == -1;
}
...
bool compare(const style &a, const style &b) {
  const int order[5] = {less_to_order(a.name, b.name),
...
template <size_t N> struct dispatch_comparison {
...
template <> struct dispatch_comparison<type_by_kind::max> {
  static bool dispatch(const item *, const item *) { return false; }
};
} // namespace

bool ifcopenshell::geometry::taxonomy::less(item::const_ptr a,
                                            item::const_ptr b) {
  if (a == b) {
    return false;
  }
...
bool compare(const trimmed_curve &a, const trimmed_curve &b) {
  ...
  if (std::tie(a.orientation, a_which_start, a_which_end) ==
    ...
    return std::tie(a.orientation, a_which_start, a_which_end) <
           std::tie(b.orientation, b_which_start, b_which_end);
  }
}

template <typename T>
bool compare_collection(const collection_base<T> &a,
                        const collection_base<T> &b) {
...
ifcopenshell::geometry::taxonomy::solid::ptr
ifcopenshell::geometry::create_box(double dx, double dy, double dz) {
  return create_box(0., 0., 0., dx, dy, dz);
}

ifcopenshell::geometry::taxonomy::solid::ptr
ifcopenshell::geometry::create_box(double x, double y, double z, double dx,
                                   double dy, double dz) {
...
piecewise_function::piecewise_function(
    double start, const spans_t &s, const IfcUtil::IfcBaseInterface *instance)
    : function_item(instance), start_(start), spans_(s) {}

piecewise_function::piecewise_function(
    double start, const std::vector<piecewise_function::ptr> &pwfs,
    const IfcUtil::IfcBaseInterface *instance)
    : function_item(instance), start_(start) {
  for (auto &pwf : pwfs) {
    spans_.insert(spans_.end(), pwf->spans().begin(), pwf->spans().end());
  }
};

const piecewise_function::spans_t &piecewise_function::spans() const {
  return spans_;
}
...
double piecewise_function::length() const {
  return std::accumulate(
      spans_.begin(), spans_.end(), 0.0,
      [](const auto &v, const auto &s) { return v + s->length(); });

  // this is a secondary option where we only compute length once and cache it.
  // mutex is needed to prevent interruption of the accumulation if there is
  // multi-threading skipping this detail for now and just adding up the span
  // lengths every time
  // if (!length_.is_initialized()) {
  //    length_ = std::accumulate(spans_.begin(), spans_.end(), 0.0, [](const
  //    auto& v, const auto& s) { return v + s->length(); });
  //}
  // return *length_;
}

gradient_function::gradient_function(piecewise_function::const_ptr horizontal,
                                     piecewise_function::const_ptr vertical,
                                     const IfcUtil::IfcBaseInterface *instance)
    : function_item(instance), horizontal_(horizontal), vertical_(vertical) {}
double gradient_function::start() const {
  return std::max(horizontal_->start(), vertical_->start());
}
double gradient_function::end() const {
  return std::min(horizontal_->end(), vertical_->end());
}
piecewise_function::const_ptr gradient_function::get_horizontal() const {
  return horizontal_;
}
piecewise_function::const_ptr gradient_function::get_vertical() const {
  return vertical_;
}

cant_function::cant_function(gradient_function::const_ptr gradient,
                             piecewise_function::const_ptr cant,
                             const IfcUtil::IfcBaseInterface *instance)
    : function_item(instance), gradient_(gradient), cant_(cant) {}
double cant_function::start() const {
  return std::max(gradient_->start(), cant_->start());
}
double cant_function::end() const {
  return std::min(gradient_->end(), cant_->end());
}
gradient_function::const_ptr cant_function::get_gradient() const {
  return gradient_;
}
piecewise_function::const_ptr cant_function::get_cant() const { return cant_; }

offset_function::offset_function(function_item::const_ptr basis,
                                 piecewise_function::const_ptr offset,
                                 const IfcUtil::IfcBaseInterface *instance)
    : function_item(instance), basis_(basis), offset_(offset) {}
...
piecewise_function::const_ptr offset_function::get_offset() const {
  return offset_;
}

ifcopenshell::geometry::taxonomy::collection::ptr
ifcopenshell::geometry::flatten(const taxonomy::collection::ptr &deep) {
  auto flat = make<taxonomy::collection>();
  ifcopenshell::geometry::visit<taxonomy::collection>(
      deep, [&flat](taxonomy::ptr i) {
...
void ifcopenshell::geometry::taxonomy::item::print(std::ostream &o,
                                                   int indent) const {
  o << std::string(indent, ' ') << kind_to_string(kind()) << std::endl;
}

void ifcopenshell::geometry::taxonomy::matrix4::print(std::ostream &o,
                                                      int indent) const {
  print_impl(o, kind_to_string(kind()), indent);
}

void ifcopenshell::geometry::taxonomy::colour::print(std::ostream &o,
                                                     int indent) const {
  print_impl(o, kind_to_string(kind()), indent);
}

void ifcopenshell::geometry::taxonomy::style::print(std::ostream &o,
                                                    int indent) const {
...
void ifcopenshell::geometry::taxonomy::point3::print(std::ostream &o,
                                                     int indent) const {
  print_impl(o, kind_to_string(kind()), indent);
}

void ifcopenshell::geometry::taxonomy::direction3::print(std::ostream &o,
                                                         int indent) const {
  print_impl(o, kind_to_string(kind()), indent);
}

void ifcopenshell::geometry::taxonomy::line::print(std::ostream &o,
                                                   int indent) const {
  print_impl(o, kind_to_string(kind()), indent);
}

void ifcopenshell::geometry::taxonomy::circle::print(std::ostream &o,
                                                     int indent) const {
  print_impl(o, kind_to_string(kind()), indent);
  o << std::string(indent + 4, ' ') << "radius " << radius << std::endl;
}

void ifcopenshell::geometry::taxonomy::ellipse::print(std::ostream &o,
                                                      int indent) const {
  print_impl(o, kind_to_string(kind()), indent);
  o << std::string(indent + 4, ' ') << "radii " << radius << " " << radius2
    << std::endl;
}

void ifcopenshell::geometry::taxonomy::trimmed_curve::print(std::ostream &o,
                                                            int indent) const {
  ...
  const boost::variant<boost::blank, point3::ptr, double> *const start_end[2] =
      {&start, &end};
  for (int i = 0; i < 2; ++i) {
    o << std::string(indent + 4, ' ') << (i == 0 ? "start" : "end")
      << std::endl;
    if (start_end[i]->which() == 1) {
      boost::get<point3::ptr>(*start_end[i])->print(o, indent + 4);
    } else if (start_end[i]->which() == 2) {
      o << std::string(indent + 4, ' ') << "parameter "
        << boost::get<double>(*start_end[i]) << std::endl;
    }
  }
...
void ifcopenshell::geometry::taxonomy::extrusion::print(std::ostream &o,
                                                        int indent) const {
...
boost::optional<face::ptr>
ifcopenshell::geometry::taxonomy::loop_to_face_upgrade_impl(ptr item) {
...
boost::optional<edge::ptr>
ifcopenshell::geometry::taxonomy::curve_to_edge_upgrade_impl(ptr item) {
...
boost::optional<loop::ptr>
ifcopenshell::geometry::taxonomy::curve_to_loop_upgrade_impl(ptr item) {
...
boost::optional<loop::ptr>
ifcopenshell::geometry::taxonomy::edge_to_loop_upgrade_impl(ptr item) {
  boost::optional<loop::ptr> loop_;
...
boost::optional<face::ptr>
ifcopenshell::geometry::taxonomy::curve_to_face_upgrade_impl(ptr item) {
  boost::optional<face::ptr> face_;
  auto circle_ = dcast<circle>(item);
  auto ellipse_ = dcast<ellipse>(item);
  auto line_ = dcast<line>(item);
...
double project_onto_curve(const circle::ptr &c, const point3 &p) {
  Eigen::Vector2d xy =
      (c->matrix->ccomponents().inverse() * p.ccomponents().homogeneous())
          .head<2>();
...
ifcopenshell::geometry::taxonomy::loop_to_function_item_upgrade_impl(ptr item) {
  ...
  if (loop_) {
    if (loop_->fi.is_initialized()) {
      ...
      // piecewise_function is a specialization of function_item - callers don't
      ...
      for (auto &edge_ : loop_->children) {
        if (edge_->basis && edge_->basis->kind() == CIRCLE) {
          const circle::ptr circ =
              std::static_pointer_cast<circle>(edge_->basis);
          ...
          std::function<Eigen::Matrix4d(double)> fn = [circ, s](double u) {
            ...
            return matrix4(P.ccomponents(),
                           circ->matrix->ccomponents().col(2).head<3>(),
                           d.ccomponents())
                .components();
          };
          ...
          if (edge_->basis && edge_->basis->kind() != LINE) {
            Logger::Message(Logger::Severity::LOG_WARNING,
                            "Basis curve not supported - edge is treated as a "
                            "straight line edge");
          }
          ...
          v.normalize(); // normalize the vector so that it is a unit direction
                         // vector
          ...
          Logger::Message(Logger::Severity::LOG_ERROR,
                          "Basis curve not supported");
          return boost::none;
        }
      }
