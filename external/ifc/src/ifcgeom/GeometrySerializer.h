...
struct UseElementNames : public SettingBase<UseElementNames, bool> {
  ...
  static constexpr const char *const description =
      "Use entity instance IfcRoot.Name instead of unique IDs for naming "
      "elements upon serialization. "
...
struct UseElementGuids : public SettingBase<UseElementGuids, bool> {
  ...
  static constexpr const char *const description =
      "Use entity instance IfcRoot.GlobalId instead of unique IDs for naming "
      "elements upon serialization. "
      "Applicable for OBJ, DAE, STP, and SVG output.";
  static constexpr bool defaultvalue = false;
};
...
struct UseElementStepIds : public SettingBase<UseElementStepIds, bool> {
  ...
  static constexpr const char *const description =
      "Use the numeric step identifier (entity instance name) for naming "
      "elements upon serialization. "
      "Applicable for OBJ, DAE, STP, and SVG output.";
  static constexpr bool defaultvalue = false;
};
...
struct UseElementTypes : public SettingBase<UseElementTypes, bool> {
  ...
  static constexpr const char *const description =
      "Use element types instead of unique IDs for naming elements upon "
      "serialization. "
...
struct UseYUp : public SettingBase<UseYUp, bool> {
  ...
  static constexpr const char *const description =
      "Change the 'up' axis to positive Y, default is Z UP. Applicable to OBJ "
      "output.";
  static constexpr bool defaultvalue = false;
};
...
struct FloatingPointDigits : public SettingBase<FloatingPointDigits, int> {
  ...
  static constexpr const char *const description =
      "Sets the precision to be used to format floating-point values, 15 by "
      "default. "
      "Use a negative value to use the system's default precision (should be 6 "
      "typically). "
      "Applicable for OBJ and DAE output. For DAE output, value >= 15 means "
      "that up to 16 decimals are used, "
...
struct WktUseSection : public SettingBase<WktUseSection, bool> {
  ...
  static constexpr const char *const description =
      "Use a geometrical section rather than full polyhedral output and "
      "footprint in TTL WKT";
  static constexpr bool defaultvalue = false;
};
} // namespace settings
...
class SerializerSettings
    : public SettingsContainer<
          // @todo should we use tuple_cat here to unify the settings into a
          // single class?
          std::tuple<UseElementNames, UseElementGuids, UseElementStepIds,
                     UseElementTypes, UseYUp, WriteGltfEcef,
                     FloatingPointDigits, BaseUri, WktUseSection>> {};
...
public:
  ...
  stream_or_filename() : oss_(new std::ostringstream), stream(*oss_) {}

  std::string get_value() const { return oss_->str(); }

  boost::optional<std::string> filename() const { return filename_; }
...
public:
  ...
  GeometrySerializer(const ifcopenshell::geometry::Settings &geometry_settings,
                     const ifcopenshell::geometry::SerializerSettings &settings)
      : geometry_settings_(geometry_settings), settings_(settings) {}
  virtual ~GeometrySerializer() {}

  virtual bool isTesselated() const = 0;
  virtual void write(const IfcGeom::TriangulationElement *o) = 0;
  virtual void write(const IfcGeom::BRepElement *o) = 0;
  virtual void setUnitNameAndMagnitude(const std::string &name,
                                       float magnitude) = 0;
  virtual IfcGeom::Element *read(IfcParse::IfcFile &f, const std::string &guid,
                                 const std::string &representation_id,
                                 read_type rt = READ_BREP) = 0;

  const ifcopenshell::geometry::SerializerSettings &settings() const {
    return settings_;
  }
  ifcopenshell::geometry::SerializerSettings &settings() { return settings_; }

  const ifcopenshell::geometry::Settings &geometry_settings() const {
    return geometry_settings_;
  }
  ...
  virtual std::string object_id(const IfcGeom::Element *o) {
    if (settings_.get<ifcopenshell::geometry::settings::UseElementGuids>()
            .get())
      return o->guid();
    if (settings_.get<ifcopenshell::geometry::settings::UseElementNames>()
            .get())
      return o->name();
    if (settings_.get<ifcopenshell::geometry::settings::UseElementStepIds>()
            .get())
      return "id-" + boost::lexical_cast<std::string>(o->id());
    return o->unique_id();
  }
...
public:
  WriteOnlyGeometrySerializer(
      const ifcopenshell::geometry::Settings &geometry_settings,
      const ifcopenshell::geometry::SerializerSettings &settings)
      : GeometrySerializer(geometry_settings, settings) {}

  virtual IfcGeom::Element *read(IfcParse::IfcFile &, const std::string &,
                                 const std::string &, read_type = READ_BREP) {
