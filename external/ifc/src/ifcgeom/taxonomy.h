...
#ifdef TAXONOMY_USE_SHARED_PTR
template <typename T> T clone(T &t) { return t; }
...
// untested currently
template <typename T> T clone(T &t) { return t->clone_(); }
template <typename T, typename U> T *cast(const std::unique_ptr<U> &u);
template <typename T, typename U> T *dcast(const std::unique_ptr<U> &u);
#endif
#ifdef TAXONOMY_USE_NAKED_PTR
// untested currently
template <typename T> T clone(T &t) { return t->clone_(); }
template <typename T, typename U> T *cast(const U *&u);
template <typename T, typename U> T *dcast(const U *&u);
#endif
...
// 4) Add compare function (see compare functions in taxonomy.cpp starting
// around line 9) 4) Update Python bindings
//    a) update list of assign_repr in IfcGeomWrapper.i
//    b) update inheritance list in IfcGeomWrapper.i (around line 120 in the
//    file) c) update item_to_pyobject function definition in type_conversion.i
enum kinds {
  MATRIX4,
  POINT3,
  DIRECTION3,
...
public:
  ...
  item(const IfcUtil::IfcBaseInterface *instance = nullptr)
      : identity_(counter_++), computed_hash_(0), instance(instance) {}
...
template <typename T> const T &eigen_defaults();

template <> const Eigen::Vector3d &eigen_defaults<Eigen::Vector3d>() {
  static Eigen::Vector3d identity = Eigen::Vector3d::Zero();
  return identity;
}

template <> const Eigen::Matrix4d &eigen_defaults<Eigen::Matrix4d>() {
...
template <typename T> struct eigen_base {
  T *components_;

  eigen_base() { components_ = nullptr; }

  eigen_base(const eigen_base &other) {
    this->components_ = other.components_ ? new T(*other.components_) : nullptr;
  }

  eigen_base(const T &other) { this->components_ = new T(other); }

  eigen_base &operator=(const eigen_base &other) {
    if (this != &other) {
      this->components_ =
          other.components_ ? new T(*other.components_) : nullptr;
    }
    return *this;
  }

  void print_impl(std::ostream &o, const std::string &class_name,
                  int indent = 0) const {
  ...
  virtual ~eigen_base() { delete this->components_; }
  ...
  explicit operator bool() const { return components_; }
...
private:
  void init(const Eigen::Vector3d &o, const Eigen::Vector3d &z,
            const Eigen::Vector3d &x) {
    auto Z = z.normalized();
    auto Y = Z.cross(x).normalized();
    auto X = Y.cross(Z);
    components_ = new Eigen::Matrix4d;
    (*components_) << X(0), Y(0), Z(0), o(0), X(1), Y(1), Z(1), o(1), X(2),
        Y(2), Z(2), o(2), 0, 0, 0, 1.;
    if (is_identity()) {
...
public:
  ...
  bool is_identity() const { return !components_ || components_->isIdentity(); }
  ...
  Eigen::Vector3d translation_part() const {
    return ccomponents().col(3).head<3>();
  }
};
...
struct style : public item {
  ...
  virtual size_t calc_hash() const {
    auto v = std::make_tuple(static_cast<size_t>(STYLE), name, diffuse.hash(),
                             surface.hash(), specular.hash(), specularity,
                             transparency);
    return boost::hash<decltype(v)>{}(v);
  }
  ...
  style()
      : specularity(std::numeric_limits<double>::quiet_NaN()),
        transparency(std::numeric_limits<double>::quiet_NaN()),
        use_surface_color(false) {}
  style(const std::string &name)
      : name(name), specularity(std::numeric_limits<double>::quiet_NaN()),
        transparency(std::numeric_limits<double>::quiet_NaN()),
        use_surface_color(false) {}
  ...
  bool has_specularity() const { return !std::isnan(specularity); }

  bool has_transparency() const { return !std::isnan(transparency); }
};
...
struct geom_item : public item {
  ...
  geom_item(const IfcUtil::IfcBaseInterface *instance = nullptr)
      : item(instance), surface_style(nullptr) {}
  geom_item(const IfcUtil::IfcBaseInterface *instance, matrix4::ptr m)
      : item(instance), surface_style(nullptr), matrix(m) {}
...
struct function_item : public implicit_item {
  ...
  function_item(const IfcUtil::IfcBaseInterface *instance = nullptr)
      : implicit_item(instance) {}
  ...
  virtual double length() const { return end() - start(); }
...
struct functor_item : public function_item {
  ...
  functor_item(double length, std::function<Eigen::Matrix4d(double u)> fn,
               const IfcUtil::IfcBaseInterface *instance = nullptr)
      : function_item(instance), length_(length), fn_(fn) {}
...
struct piecewise_function : public function_item {
  ...
  piecewise_function(double start, const spans_t &s,
                     const IfcUtil::IfcBaseInterface *instance = nullptr);
  piecewise_function(double start,
                     const std::vector<piecewise_function::ptr> &pwfs,
                     const IfcUtil::IfcBaseInterface *instance = nullptr);
  ...
  piecewise_function *clone_() const override {
    return new piecewise_function(*this);
  }
...
struct gradient_function : public function_item {
  ...
  gradient_function(piecewise_function::const_ptr horizontal,
                    piecewise_function::const_ptr vertical,
                    const IfcUtil::IfcBaseInterface *instance = nullptr);
  ...
  gradient_function *clone_() const override {
    return new gradient_function(*this);
  }
...
struct cant_function : public function_item {
  ...
  cant_function(gradient_function::const_ptr gradient,
                piecewise_function::const_ptr cant,
                const IfcUtil::IfcBaseInterface *instance = nullptr);
...
struct offset_function : public function_item {
  ...
  offset_function(function_item::const_ptr basis,
                  piecewise_function::const_ptr offset,
                  const IfcUtil::IfcBaseInterface *instance = nullptr);
  ...
  offset_function *clone_() const override {
    return new offset_function(*this);
  }
...
typedef std::uniqe_ptr<item> ptr;
typedef std::uniqe_ptr<const item> ptr;
template <typename T, typename... Args> std::uniqe_ptr<T> make(Args &&...args) {
...
struct cartesian_base : public item, public eigen_base<Eigen::Vector3d> {
  ...
  cartesian_base(double x, double y, double z = 0.)
      : eigen_base(Eigen::Vector3d(x, y, z)) {}
};
...
struct direction3 : public cartesian_base<3> {
  ...
  virtual size_t calc_hash() const {
    auto v =
        std::make_tuple(static_cast<size_t>(DIRECTION3), hash_components());
    return boost::hash<decltype(v)>{}(v);
  }
...
struct curve : public geom_item {
  void print_impl(std::ostream &o, const std::string &classname,
                  int indent = 0) const {
...
struct line : public curve {
  ...
  virtual size_t calc_hash() const {
    auto v =
        std::make_tuple(static_cast<size_t>(LINE), matrix->hash_components());
    return boost::hash<decltype(v)>{}(v);
  }
...
struct circle : public curve {
  ...
  virtual size_t calc_hash() const {
    auto v = std::make_tuple(static_cast<size_t>(CIRCLE),
                             matrix->hash_components(), radius);
    return boost::hash<decltype(v)>{}(v);
  }

  void print(std::ostream &o, int indent = 0) const;

  static circle::ptr from_3_points(const Eigen::Vector3d &p1,
...
struct ellipse : public curve {
  ...
  virtual size_t calc_hash() const {
    auto v = std::make_tuple(static_cast<size_t>(ELLIPSE),
                             matrix->hash_components(), radius, radius2);
    return boost::hash<decltype(v)>{}(v);
  }
...
struct offset_curve : public curve {
  ...
  virtual size_t calc_hash() const {
    auto v =
        std::make_tuple(static_cast<size_t>(OFFSET_CURVE), reference->hash(),
                        offset, basis ? basis->hash() : size_t(0));
    return boost::hash<decltype(v)>{}(v);
  }
};
...
struct trimmed_curve : public geom_item {
  ...
  // @todo The copy constructor of point3 within the variant fails on the avx
  // instruction on the default gcc in Ubuntu 18.04 and a recent AMD Ryzen.
  // Probably due to allignment.
  boost::variant<boost::blank, point3::ptr, double> start, end;

  // @todo somehow account for the fact that curve in IFC can be trimmed curve,
  // polyline and composite curve as well.
  item::ptr basis;

  // @todo does this make sense? this is to accommodate for the fact that
  // orientation is defined on both TrimmedCurve as well CompCurveSegment
  boost::optional<bool> curve_sense;

  trimmed_curve() : basis(nullptr) {}
  trimmed_curve(const point3::ptr &a, const point3::ptr &b)
      : start(a), end(b), basis(nullptr) {}
...
struct edge : public trimmed_curve {
  ...
  virtual size_t calc_hash() const {
    auto v = std::make_tuple(static_cast<size_t>(EDGE), start, end,
                             basis ? basis->hash() : size_t(0),
                             curve_sense ? *curve_sense ? 2 : 1 : 0);
    return boost::hash<decltype(v)>{}(v);
  }
};

template <typename T = item> struct collection_base : public geom_item {
  std::vector<typename T::ptr> children;

  collection_base() {}
  collection_base(const collection_base &other) : geom_item() {
    std::transform(other.children.begin(), other.children.end(),
                   std::back_inserter(children),
                   [](typename T::ptr p) { return clone(p); });
  }
...
struct loop : public collection_base<edge> {
  ...
  virtual size_t calc_hash() const {
    auto v = std::make_tuple(static_cast<size_t>(LOOP), hash_elements(),
                             external ? *external ? 2 : 1 : 0,
                             closed ? *closed ? 2 : 1 : 0);
    return boost::hash<decltype(v)>{}(v);
  }
};
...
struct face : public collection_base<loop> {
  ...
  virtual size_t calc_hash() const {
    auto v = std::make_tuple(static_cast<size_t>(FACE), hash_elements(),
                             basis ? basis->hash() : size_t(0));
    return boost::hash<decltype(v)>{}(v);
  }
};
...
struct shell : public collection_base<face> {
  ...
  virtual void print_impl(std::ostream &o, int indent) const {
    ...
    o << std::string(indent, ' ') << "closed "
      << (closed ? *closed ? "yes"s : "no"s : "unknown"s) << std::endl;
  }
  ...
  virtual size_t calc_hash() const {
    auto v = std::make_tuple(static_cast<size_t>(SHELL), hash_elements(),
                             closed ? *closed ? 2 : 1 : 0);
    return boost::hash<decltype(v)>{}(v);
  }
};
...
struct loft : public collection_base<geom_item> {
  ...
  virtual size_t calc_hash() const {
    auto v = std::make_tuple(static_cast<size_t>(LOFT), hash_elements(),
                             axis ? axis->hash() : size_t(0));
    return boost::hash<decltype(v)>{}(v);
  }
};
...
struct plane : public surface {
  ...
  virtual size_t calc_hash() const {
    auto v =
        std::make_tuple(static_cast<size_t>(PLANE), matrix->hash_components());
    return boost::hash<decltype(v)>{}(v);
  }
};
...
struct cylinder : public surface {
  ...
  virtual size_t calc_hash() const {
    auto v = std::make_tuple(static_cast<size_t>(CYLINDER),
                             matrix->hash_components());
    return boost::hash<decltype(v)>{}(v);
  }
};
...
struct sphere : public surface {
  ...
  virtual size_t calc_hash() const {
    auto v =
        std::make_tuple(static_cast<size_t>(SPHERE), matrix->hash_components());
    return boost::hash<decltype(v)>{}(v);
  }
};
...
struct torus : public surface {
  ...
  virtual size_t calc_hash() const {
    auto v =
        std::make_tuple(static_cast<size_t>(TORUS), matrix->hash_components());
    return boost::hash<decltype(v)>{}(v);
  }
};
...
struct extrusion : public sweep {
  ...
  extrusion(matrix4::ptr m, item::ptr basis, direction3::ptr dir, double d)
      : sweep(m, basis), direction(dir), depth(d) {}

  void print(std::ostream &o, int indent = 0) const;

  virtual size_t calc_hash() const {
    auto v = std::make_tuple(static_cast<size_t>(EXTRUSION),
                             matrix->hash_components(), basis->calc_hash(),
                             direction->hash_components(), depth);
    return boost::hash<decltype(v)>{}(v);
  }
};
...
struct revolve : public sweep {
  ...
  revolve(matrix4::ptr m, item::ptr basis, point3::ptr pnt, direction3::ptr dir,
          const boost::optional<double> &a)
      : sweep(m, basis), axis_origin(pnt), direction(dir), angle(a) {}

  virtual size_t calc_hash() const {
    auto v =
        std::make_tuple(static_cast<size_t>(REVOLVE), matrix->hash_components(),
                        basis->calc_hash(), axis_origin->hash_components(),
                        direction->hash_components(), angle ? *angle : 1000.);
    return boost::hash<decltype(v)>{}(v);
  }
};
...
struct sweep_along_curve : public sweep {
  ...
  virtual sweep_along_curve *clone_() const {
    return new sweep_along_curve(*this);
  }
  virtual kinds kind() const { return SWEEP_ALONG_CURVE; }

  sweep_along_curve(matrix4::ptr m, face::ptr basis, item::ptr surf,
                    item::ptr crv)
      : sweep(m, basis), surface(surf), curve(crv) {}

  virtual size_t calc_hash() const {
    auto v = std::make_tuple(static_cast<size_t>(SWEEP_ALONG_CURVE),
                             matrix->hash_components(), basis->calc_hash(),
...
struct boolean_result : public collection_base<geom_item> {
  ...
  virtual size_t calc_hash() const {
    auto v = std::make_tuple(static_cast<size_t>(BOOLEAN_RESULT),
                             hash_elements(), static_cast<size_t>(operation));
    return boost::hash<decltype(v)>{}(v);
  }
};
...
namespace impl {
typedef std::tuple<
    matrix4, point3, direction3, line, circle, ellipse, bspline_curve,
    offset_curve, plane, cylinder, sphere, torus, bspline_surface, edge, loop,
    face, shell, solid, loft, extrusion, revolve, sweep_along_curve, node,
    collection, boolean_result, function_item, functor_item, piecewise_function,
    gradient_function, cant_function, offset_function>
    KindsTuple;
typedef std::tuple<line, circle, ellipse, bspline_curve, offset_curve, loop,
                   edge>
    CurvesTuple;
typedef std::tuple<plane, cylinder, sphere, torus, bspline_surface, extrusion,
                   revolve>
    SurfacesTuple;
...
template <typename T> class loop_to_face_upgrade {
private:
...
public:
  ...
  operator bool() const { return face_.is_initialized(); }
...
public:
  ...
  operator bool() const { return edge_.is_initialized(); }
...
template <typename T> class curve_to_loop_upgrade {
private:
...
public:
  ...
  operator bool() const { return loop_.is_initialized(); }
...
template <typename T> class edge_to_loop_upgrade {
private:
  boost::optional<taxonomy::loop::ptr> loop_;
...
template <typename T> class curve_to_face_upgrade {
private:
  boost::optional<taxonomy::face::ptr> face_;
...
boost::optional<function_item::ptr>
loop_to_function_item_upgrade_impl(ptr item);
template <typename T> class loop_to_function_item_upgrade {
private:
...
public:
  ...
  operator bool() const { return fi_.is_initialized(); }
...
void visit_2(const typename U::ptr &c, const Fn &fn) {
  static_assert(std::is_same<T, taxonomy::point3>::value,
                "@todo Only implemented for point3");
...
template <typename Fn>
taxonomy::collection::ptr filter(const taxonomy::collection::ptr &collection,
                                 Fn fn) {
...
// @nb traverses nested collections
template <typename Fn>
taxonomy::collection::ptr filter_in_place(taxonomy::collection::ptr collection,
                                          Fn fn) {
  auto &c = collection->children;
  auto new_end =
      std::remove_if(c.begin(), c.end(), [fn](taxonomy::geom_item::ptr i) {
...
taxonomy::solid::ptr create_box(double x, double y, double z, double dx,
                                double dy, double dz);
...
enum connection_type { ATPATH, ATSTART, ATEND, NOTDEFINED };

typedef std::tuple<connection_type, connection_type, IfcUtil::IfcBaseEntity *>
    endpoint_connection;
} // namespace geometry
