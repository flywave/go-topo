...
bool IfcGeom::util::axis_equal(const gp_Ax3 &a, const gp_Ax3 &b,
                               double tolerance) {
  if (!a.Location().IsEqual(b.Location(), tolerance))
    return false;
  // Note that the tolerance below is angular, above is linear. Since
  // architectural objects are about 1m'ish in scale, it should be somewhat
  // equivalent. Besides, this is mostly a filter for NULL or default values in
  // the placements.
  if (!a.Direction().IsEqual(b.Direction(), tolerance))
    return false;
  if (!a.XDirection().IsEqual(b.XDirection(), tolerance))
    return false;
  if (!a.YDirection().IsEqual(b.YDirection(), tolerance))
    return false;
  return true;
}

bool IfcGeom::util::axis_equal(const gp_Ax2d &a, const gp_Ax2d &b,
                               double tolerance) {
  if (!a.Location().IsEqual(b.Location(), tolerance))
    return false;
  if (!a.Direction().IsEqual(b.Direction(), tolerance))
...
int IfcGeom::util::count(const TopoDS_Shape &s, TopAbs_ShapeEnum t,
                         bool unique) {
  if (unique) {
...
bool IfcGeom::util::is_nested_compound_of_solid(const TopoDS_Shape &s,
                                                int depth) {
...
template <> struct dimension_count<gp_Trsf2d> {
  static const int n = 2;
};
template <> struct dimension_count<gp_GTrsf2d> {
  static const int n = 2;
};
template <> struct dimension_count<gp_Trsf> {
  static const int n = 3;
};
template <> struct dimension_count<gp_GTrsf> {
  static const int n = 3;
};

template <typename T> bool is_identity_helper(const T &t, double tolerance) {
...
gp_Trsf
IfcGeom::util::combine_offset_and_rotation(const gp_Vec &offset,
                                           const gp_Quaternion &rotation) {
...
bool IfcGeom::util::project(const Handle_Geom_Surface &srf,
                            const TopoDS_Shape &shp, double &u1, double &v1,
                            double &u2, double &v2, double widen) {
  ...
  } else if (srf->DynamicType() == STANDARD_TYPE(Geom_OffsetSurface) &&
             Handle(Geom_OffsetSurface)::DownCast(srf)
                     ->BasisSurface()
                     ->DynamicType() == STANDARD_TYPE(Geom_Plane)) {
    // For an offset planar surface the projected UV coords are the same as the
    // basis surface
    pln = Handle(Geom_Plane)::DownCast(
        Handle(Geom_OffsetSurface)::DownCast(srf)->BasisSurface());
  } else {
  ...
  gp_Pnt median;
  int vertex_count = 0;
  for (TopExp_Explorer exp(shp, TopAbs_VERTEX); exp.More();
       exp.Next(), ++vertex_count) {
  ...
  if (vertex_count > 0) {
    ...
    // Add a little bit of resolution so that the median is shifted towards the
    // mass of the curve. This helps to find the parameter ordering for conic
    // surfaces.
    for (TopExp_Explorer exp(shp, TopAbs_EDGE); exp.More();
         exp.Next(), ++vertex_count) {
...
TopoDS_Shape IfcGeom::util::apply_transformation(const TopoDS_Shape &s,
                                                 const gp_Trsf &t) {
...
TopoDS_Shape IfcGeom::util::apply_transformation(
    const TopoDS_Shape &s, const ifcopenshell::geometry::taxonomy::matrix4 &t) {
...
bool IfcGeom::util::fit_halfspace(const TopoDS_Shape &a, const TopoDS_Shape &b,
                                  TopoDS_Shape &box, double &height,
                                  double tol) {
...
const Handle_Geom_Curve IfcGeom::util::intersect(const Handle_Geom_Surface &a,
                                                 const Handle_Geom_Surface &b) {
...
const Handle_Geom_Curve IfcGeom::util::intersect(const TopoDS_Face &a,
                                                 const Handle_Geom_Surface &b) {
  return intersect(BRep_Tool::Surface(a), b);
}

bool IfcGeom::util::intersect(const Handle_Geom_Curve &a,
                              const Handle_Geom_Surface &b, gp_Pnt &p) {
  GeomAPI_IntCS x(a, b);
...
bool IfcGeom::util::intersect(const Handle_Geom_Curve &a, const TopoDS_Face &b,
                              gp_Pnt &c) {
  return intersect(a, BRep_Tool::Surface(b), c);
}

bool IfcGeom::util::intersect(const Handle_Geom_Curve &a, const TopoDS_Shape &b,
...
bool IfcGeom::util::intersect(
    const Handle_Geom_Surface &a, const TopoDS_Shape &b,
    std::vector<std::pair<Handle_Geom_Surface, Handle_Geom_Curve>> &out) {
  TopExp_Explorer exp(b, TopAbs_FACE);
  for (; exp.More(); exp.Next()) {
...
bool IfcGeom::util::closest(const gp_Pnt &a, const std::vector<gp_Pnt> &b,
                            gp_Pnt &c) {
  double minimal_distance = std::numeric_limits<double>::infinity();
  for (std::vector<gp_Pnt>::const_iterator it = b.begin(); it != b.end();
       ++it) {
...
bool IfcGeom::util::project(const Handle_Geom_Curve &crv, const gp_Pnt &pt,
                            gp_Pnt &p, double &u, double &d) {
...
bool IfcGeom::util::is_convex(const TopoDS_Wire &wire, double tol) {
  for (TopExp_Explorer exp1(wire, TopAbs_VERTEX); exp1.More(); exp1.Next()) {
    ...
    for (TopExp_Explorer exp3(wire, TopAbs_EDGE); exp3.More(); exp3.Next()) {
      ...
      for (TopExp_Explorer exp2(edge, TopAbs_VERTEX); exp2.More();
           exp2.Next()) {
      ...
      if (edge_points.size() != 2)
        continue;
      if (edge_points[0].IsEqual(P1, tol))
        neighbors.push_back(edge_points[1]);
      else if (edge_points[1].IsEqual(P1, tol))
        neighbors.push_back(edge_points[0]);
    }
    ...
    for (TopExp_Explorer exp2(wire, TopAbs_VERTEX); exp2.More(); exp2.Next()) {
      ...
      if (P1.IsEqual(P2, tol))
        continue;
      bool found = false;
      for (std::vector<gp_Pnt>::const_iterator it = neighbors.begin();
           it != neighbors.end(); ++it) {
        if ((*it).IsEqual(P2, tol)) {
          found = true;
          break;
        }
      }
      if (!found)
        non_neighbors.push_back(P2);
    }
    ...
    // Now for the non-neighbors see whether a greater angle can be found with
    // one of the edges
    for (std::vector<gp_Pnt>::const_iterator it = non_neighbors.begin();
         it != non_neighbors.end(); ++it) {
      gp_Dir dir3((*it).XYZ() - P1.XYZ());
      const double angle2 = acos(dir3.Dot(dir1));
      const double angle3 = acos(dir3.Dot(dir2));
      if (angle2 > angle || angle3 > angle)
        return false;
    }
  }
  return true;
...
TopoDS_Shape IfcGeom::util::halfspace_from_plane(const gp_Pln &pln,
                                                 const gp_Pnt &cent) {
...
bool IfcGeom::util::shape_to_face_list(const TopoDS_Shape &s,
...
bool IfcGeom::util::create_solid_from_compound(const TopoDS_Shape &compound,
                                               TopoDS_Shape &shape,
                                               double tol) {
...
bool IfcGeom::util::create_solid_from_faces(
    const TopTools_ListOfShape &face_list, TopoDS_Shape &shape, double tol,
    bool force_sewing) {
  ...
  // In case there are wire intersections or failures in non-planar wire
  // triangulations the idea is to let occt do an exhaustive search of edge
  // partners. But we have not found a case where this actually improves boolean
  // ops later on. if (!faceset_helper_ || !faceset_helper_->non_manifold()) {

  for (face_iterator.Initialize(face_list);
       !force_sewing && face_iterator.More(); face_iterator.Next()) {
    // As soon as is detected one of the edges is shared, the assumption is made
    // no additional sewing is necessary.
    if (!has_shared_edges) {
  ...
  if (valid_shell) {
    ...
    for (; exp.More(); exp.Next()) {
      ...
      try {
        ...
        TopoDS_Solid solid_shape =
            solid.SolidFromShell(TopoDS::Shell(exp.Current()));
        // @todo: BRepClass3d_SolidClassifier::PerformInfinitePoint() is done by
        // SolidFromShell
        //        and this is done again, to be able to catch errors during this
        //        process. This is double work that should be avoided.
...
bool IfcGeom::util::flatten_shape_list(const IfcGeom::ConversionResults &shapes,
                                       TopoDS_Shape &result, bool fuse,
                                       bool create_shell, double tol) {
  ...
  for (IfcGeom::ConversionResults::const_iterator it = shapes.begin();
       it != shapes.end(); ++it) {
    TopoDS_Shape merged;
    const TopoDS_Shape &s =
        std::static_pointer_cast<ifcopenshell::geometry::OpenCascadeShape>(
            it->Shape())
            ->shape();
