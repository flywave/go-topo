...
void ifcopenshell::geometry::OpenCascadeShape::Triangulate(
  ...
  // @todo remove duplication with OpenCascadeKernel::convert(const
  // taxonomy::matrix4::ptr matrix, gp_GTrsf& trsf); above can be static?
  ...
  if (place.components_) {
    ...
    rotation_matrix.emplace(m(0, 0), m(0, 1), m(0, 2), m(1, 0), m(1, 1),
                            m(1, 2), m(2, 0), m(2, 1), m(2, 2));
  }

  // When welding vertices, vertex coords will be shared among faces so we need
  // to per-shape set to keep track of which edges were already emitted.
  std::set<std::pair<int, int>> emitted_edges;

  // Triangulate the shape
  try {
    BRepMesh_IncrementalMesh(
        shape_, settings.get<settings::MesherLinearDeflection>().get(), false,
        settings.get<settings::MesherAngularDeflection>().get());
  } catch (...) {
  ...
  for (exp.Init(shape_, TopAbs_FACE); exp.More(); exp.Next(), ++num_faces) {
    ...
    const bool is_planar =
        ...
        BRep_Tool::Surface(face)->DynamicType() == STANDARD_TYPE(Geom_Plane);
    const bool has_inner_bounds = num_bounds > 1;

    const bool polyhedral_output_with_holes =
        settings.get<settings::TriangulationType>().get() ==
            settings::POLYHEDRON_WITH_HOLES &&
        is_planar;
    const bool polyhedral_output_without_holes =
        settings.get<settings::TriangulationType>().get() ==
            settings::POLYHEDRON_WITHOUT_HOLES &&
        is_planar && !has_inner_bounds;
    ...
    if (tri.IsNull()) {
      ...
      // Vertex normals are only calculated if vertices are not welded and
      // calculation is not disable explicitly.
      const bool calculate_normals =
          !settings.get<settings::WeldVertices>().get() &&
      ...
      for (int i = 1; i <= tri->NbNodes(); ++i) {
        ...
        dict[i] = t->addVertex(item_id, surface_style_id, last.X(), last.Y(),
                               last.Z());
      ...
      for (int i = 1; i <= triangles.Length(); ++i) {
        ...
        if (dict[n1] == dict[n2] || dict[n2] == dict[n3] ||
            dict[n3] == dict[n1]) {
        ...
        */
        ...
        if (polyhedral_output_without_holes || polyhedral_output_with_holes) {
          ...
          if (settings.get<settings::TriangulationType>().get() ==
              settings::POLYHEDRON_WITHOUT_HOLES) {
            t->addFace(item_id, surface_style_id,
                       std::vector<int>{dict[n1], dict[n2], dict[n3]});
          } else if (settings.get<settings::TriangulationType>().get() ==
                     settings::POLYHEDRON_WITH_HOLES) {
            t->addFace(
                item_id, surface_style_id,
                std::vector<std::vector<int>>{{dict[n1], dict[n2], dict[n3]}});
          } else {
      ...
      for (auto &p : edgecount) {
        // @todo should be != 2?
        if (p.second == 1 &&
            emitted_edges.find(p.first) == emitted_edges.end()) {
          ...
          if (settings.get<settings::WeldVertices>().get()) {
            // only relevant while welding, because otherwise vertices are not
            // shared among distinct faces
    ...
    if (polyhedral_output_without_holes || polyhedral_output_with_holes) {
      auto loops =
          IfcGeom::util::find_boundary_loops(t->verts(), triangle_indices);
  ...
  if (!t->normals().empty() && settings.get<settings::GenerateUvs>().get()) {
    t->uvs_ref() = IfcGeom::Representation::Triangulation::box_project_uvs(
        t->verts(), t->normals());
  }

  if (num_faces == 0) {
    // Edges are only emitted if there are no faces. A mixed representation of
    // faces and loose edges is discouraged by the standard. An alternative
    // would be to use TopExp_Explorer texp(s, TopAbs_EDGE, TopAbs_FACE) to find
    // edges that do not belong to any face.
    ...
    for (TopExp_Explorer texp(shape_, TopAbs_EDGE, TopAbs_WIRE); texp.More();
         texp.Next()) {
      edges.Append(texp.Current());
    }

    for (TopTools_ListIteratorOfListOfShape texp(edges); texp.More();
         texp.Next()) {
      BRepAdaptor_Curve crv(TopoDS::Edge(texp.Value()));
      GCPnts_QuasiUniformDeflection tessellater(
          crv, settings.get<settings::MesherLinearDeflection>().get());
      ...
      for (int i = (reversed ? n : 1); reversed ? (i >= 1) : (i <= n);
           i += reversed ? -1 : 1) {
        gp_XYZ p;
        if (std::fabs(tessellater.Parameter(i) - u0) < 1.e-7) {
          // Use the exact points from the topology when parameter is close to
          // the begin or end of the parametric range This guarantees points are
          // properly welded, because the GCPnts_QuasiUniformDeflection could
          // otherwise introduce minor differences between the approximated
          // points from shared vertices.
          // @todo Using GCPnts_QuasiUniformDeflection on linear edges is pure
          // lazyness
          p = p0.XYZ();
        ...
        if (settings.get<settings::EdgeArrows>().get()) {
          ...
          int left =
              t->addVertex(item_id, surface_style_id, p2.X(), p2.Y(), p2.Z());
          int right =
              t->addVertex(item_id, surface_style_id, p3.X(), p3.Y(), p3.Z());
...
void ifcopenshell::geometry::OpenCascadeShape::Serialize(
    const ifcopenshell::geometry::taxonomy::matrix4 &place,
    std::string &r) const {
...
OpaqueNumber *
ifcopenshell::geometry::OpenCascadeShape::OpenCascadeShape::length() {
  GProp_GProps prop;
...
OpaqueCoordinate<3> ifcopenshell::geometry::OpenCascadeShape::position() {
  if (shape_.ShapeType() == TopAbs_FACE) {
    ...
    if (plane) {
      ...
      return OpaqueCoordinate<3>(new NumberNativeDouble(loc.X()),
...
OpaqueCoordinate<3> ifcopenshell::geometry::OpenCascadeShape::axis() {
  if (shape_.ShapeType() == TopAbs_FACE) {
    ...
    if (plane) {
      ...
      return OpaqueCoordinate<3>(new NumberNativeDouble(dir.X()),
  ...
  if (shape_.ShapeType() == TopAbs_FACE) {
    ...
    if (plane) {
      ...
      return OpaqueCoordinate<4>(
          new NumberNativeDouble(a), new NumberNativeDouble(b),
          new NumberNativeDouble(c), new NumberNativeDouble(d));
    }
  }
  throw std::runtime_error("Invalid shape type");
}

std::vector<ConversionResultShape *>
ifcopenshell::geometry::OpenCascadeShape::convex_decomposition() {
...
std::vector<ConversionResultShape *>
ifcopenshell::geometry::OpenCascadeShape::vertices() {
...
std::vector<ConversionResultShape *>
ifcopenshell::geometry::OpenCascadeShape::edges() {
  TopTools_IndexedMapOfShape map;
...
std::vector<ConversionResultShape *>
ifcopenshell::geometry::OpenCascadeShape::facets() {
  TopTools_IndexedMapOfShape map;
...
namespace {
ConversionResultShape *boolean_op(BOPAlgo_Operation op,
                                  const TopoDS_Shape &shape_,
                                  const TopoDS_Shape &other_shape) {
...
ifcopenshell::geometry::OpenCascadeShape::add(ConversionResultShape *other) {
  return boolean_op(
      BOPAlgo_FUSE, shape_,
      ((ifcopenshell::geometry::OpenCascadeShape *)other)->shape_);
}

ConversionResultShape *ifcopenshell::geometry::OpenCascadeShape::subtract(
    ConversionResultShape *other) {
  return boolean_op(
      BOPAlgo_CUT, shape_,
      ((ifcopenshell::geometry::OpenCascadeShape *)other)->shape_);
}

ConversionResultShape *ifcopenshell::geometry::OpenCascadeShape::intersect(
    ConversionResultShape *other) {
  return boolean_op(
      BOPAlgo_COMMON, shape_,
      ((ifcopenshell::geometry::OpenCascadeShape *)other)->shape_);
}

std::pair<OpaqueCoordinate<3>, OpaqueCoordinate<3>>
ifcopenshell::geometry::OpenCascadeShape::bounding_box() const {
  throw std::runtime_error("Not implemented");
}

ConversionResultShape *ifcopenshell::geometry::OpenCascadeShape::moved(
    ifcopenshell::geometry::taxonomy::matrix4::ptr t) const {
  return new OpenCascadeShape(IfcGeom::util::apply_transformation(shape_, *t));
}

void ifcopenshell::geometry::OpenCascadeShape::map(OpaqueCoordinate<4> &,
                                                   OpaqueCoordinate<4> &) {
  throw std::runtime_error("Not implemented");
}

void ifcopenshell::geometry::OpenCascadeShape::map(
    const std::vector<OpaqueCoordinate<4>> &,
    const std::vector<OpaqueCoordinate<4>> &) {
  throw std::runtime_error("Not implemented");
}
