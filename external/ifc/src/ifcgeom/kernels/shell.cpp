...
bool OpenCascadeKernel::convert(const taxonomy::shell::ptr l,
                                TopoDS_Shape &shape) {
  ...
  for (auto &face : l->children) {
    ...
    if (!success) {
      Logger::Message(Logger::LOG_WARNING,
                      "Failed to convert face:", face->instance);
      continue;
    }
  ...
  if (!settings_.get<settings::ReorientShells>().get() ||
      !create_solid_from_faces(face_list, shape,
...
bool OpenCascadeKernel::convert_impl(const taxonomy::shell::ptr shell,
  ...
  results.emplace_back(ConversionResult(
      shell->instance->as<IfcUtil::IfcBaseEntity>()->id(), shell->matrix,
      new OpenCascadeShape(shape), shell->surface_style));
  return true;
}
