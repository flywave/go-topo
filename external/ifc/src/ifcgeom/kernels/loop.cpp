...
#include <ifcgeom/kernels/wire_builder.h>
...
struct curve_creation_visitor {
  ...
  OpenCascadeKernel::curve_creation_visitor_result_type
  operator()(const taxonomy::bspline_curve::ptr &bc) {
    ...
    // @tfk: it appears to be wrong to expect a period curve when the curve is
    // closed, see #586 Standard_Boolean     Periodic = l->ClosedCurve();

    int i;

    if (is_rational) {
      i = 0;
      for (auto it = bc->weights->begin(); it != bc->weights->end();
           ++it, ++i) {
        Weights(i) = *it;
      }
    }
    ...
    i = 0;
    for (auto it = bc->control_points.begin(); it != bc->control_points.end();
         ++it, ++i) {
      Poles(i) = OpenCascadeKernel::convert_xyz<gp_Pnt>(**it);
    }

    i = 0;
    for (auto it = bc->multiplicities.begin(); it != bc->multiplicities.end();
         ++it, ++i) {
      Mults(i) = *it;
    }
    ...
    if (is_rational) {
      return result = Handle(Geom_Curve)(new Geom_BSplineCurve(
                 Poles, Weights, Knots, Mults, Degree, Periodic));
    } else {
      return result = Handle(Geom_Curve)(
                 new Geom_BSplineCurve(Poles, Knots, Mults, Degree, Periodic));
    }
  }

  OpenCascadeKernel::curve_creation_visitor_result_type
  operator()(const taxonomy::line::ptr &l) {
    const auto &m = l->matrix->ccomponents();
    return result = Handle(Geom_Curve)(new Geom_Line(
               OpenCascadeKernel::convert_xyz2<gp_Pnt>(m.col(3)),
               OpenCascadeKernel::convert_xyz2<gp_Dir>(m.col(2))));
  }

  OpenCascadeKernel::curve_creation_visitor_result_type
  operator()(const taxonomy::circle::ptr &c) {
    const auto &m = c->matrix->ccomponents();
    return result = Handle(Geom_Curve)(new Geom_Circle(
               gp_Ax2(OpenCascadeKernel::convert_xyz2<gp_Pnt>(m.col(3)),
                      OpenCascadeKernel::convert_xyz2<gp_Dir>(m.col(2)),
  ...
  OpenCascadeKernel::curve_creation_visitor_result_type
  operator()(const taxonomy::ellipse::ptr &e) {
    const auto &m = e->matrix->ccomponents();
    return result = Handle(Geom_Curve)(new Geom_Ellipse(
               gp_Ax2(OpenCascadeKernel::convert_xyz2<gp_Pnt>(m.col(3)),
                      OpenCascadeKernel::convert_xyz2<gp_Dir>(m.col(2)),
                      OpenCascadeKernel::convert_xyz2<gp_Dir>(m.col(0))),
               e->radius, e->radius2));
  ...
  OpenCascadeKernel::curve_creation_visitor_result_type
  operator()(const taxonomy::loop::ptr &l) {
    TopoDS_Wire wire;
  ...
  OpenCascadeKernel::curve_creation_visitor_result_type
  operator()(const taxonomy::edge::ptr &e) {
    // @todo for polyloops/-lines we should probably construct edges based on
    // correct oriented TopoDS_Vertex instead.
    ...
    if (e_basis) {
      while (e_basis->kind() == taxonomy::EDGE && e_basis->instance &&
             e_basis->instance->declaration().name() == "IfcTrimmedCurve") {
        // @todo we still might have something to wrt orientation on periodic
        // curves to make sure we select the correct arc later on.
      ...
      if (crv_or_wire.which() == 0) {
        ...
        Logger::Warning(
            "Approximating BasisCurve due to possible discontinuities",
            e->instance);
        const auto &w = boost::get<TopoDS_Wire>(crv_or_wire);
#if OCC_VERSION_HEX < 0x70600
        BRepAdaptor_CompCurve cc(w, true);
        Handle(Adaptor3d_HCurve) hcc =
            Handle(Adaptor3d_HCurve)(new BRepAdaptor_HCompCurve(cc));
#else
        auto hcc = new BRepAdaptor_CompCurve(w, true);
#endif
        // @todo, arbitrary numbers here, note they cannot be too high as
        // contiguous memory is allocated based on them.
        Approx_Curve3d approx(hcc, precision_, GeomAbs_C0, 10, 10);
        curve = approx.Curve();
      }

      const bool is_conic = e_basis->kind() == taxonomy::ELLIPSE ||
                            e_basis->kind() == taxonomy::CIRCLE;
      ...
      if (e_start.which() == 0) {
        ...
        auto p1 = OpenCascadeKernel::convert_xyz<gp_Pnt>(
            *boost::get<taxonomy::point3::ptr>(e_start));
        auto p2 = OpenCascadeKernel::convert_xyz<gp_Pnt>(
            *boost::get<taxonomy::point3::ptr>(e_end));

        if (curve->IsClosed() &&
            p1.Distance(p2) <=
                kernel->settings().get<settings::Precision>().get()) {
          E = BRepBuilderAPI_MakeEdge(curve).Edge();
        } else {
  ...
  OpenCascadeKernel::curve_creation_visitor_result_type
  operator()(const taxonomy::offset_curve::ptr &) {
    // @todo
...
OpenCascadeKernel::curve_creation_visitor_result_type
OpenCascadeKernel::convert_curve(const taxonomy::ptr curve) {
...
bool OpenCascadeKernel::convert(const taxonomy::loop::ptr loop,
                                TopoDS_Wire &wire) {
  ...
  for (auto &segment : loop->children) {
    ...
    try {
      ...
      // @todo we should do some better logging here and catch specific
      // exceptions but most notably we just want to continue processing when
      // there are duplicate vertices in our loop (or remove them earlier in the
      // mapping?).
      continue;
    }
  ...
  if (converted_segments.Extent() == 0) {
    Logger::Message(Logger::LOG_ERROR,
                    "No segment successfully converted:", loop->instance);
    return false;
  }

  BRepBuilderAPI_MakeWire w;
  TopoDS_Vertex wire_first_vertex, wire_last_vertex, edge_first_vertex,
      edge_last_vertex;

  TopTools_ListIteratorOfListOfShape it(converted_segments);

  bool force_close = false;
  if (loop->instance && loop->instance->as<IfcUtil::IfcBaseEntity>() &&
      loop->instance->as<IfcUtil::IfcBaseEntity>()->file_) {
    auto *inst = loop->instance->as<IfcUtil::IfcBaseEntity>();
    auto *file = loop->instance->as<IfcUtil::IfcBaseEntity>()->file_;
    auto profile = file->getInverse(
        inst->id(), file->schema()->declaration_by_name("IfcProfileDef"), -1);
    force_close = profile && profile->size() > 0;
  }

  wire_builder bld(precision_,
                   loop->instance ? loop->instance->as<IfcUtil::IfcBaseEntity>()
                                  : nullptr);
  ...
  for (int i = 1; i <= map.Extent(); ++i) {
    ...
    if (edges.Extent() == 2) {
      ...
      auto has_circle = crv1->DynamicType() == STANDARD_TYPE(Geom_Circle) ||
                        crv2->DynamicType() == STANDARD_TYPE(Geom_Circle);
      auto has_line = crv1->DynamicType() == STANDARD_TYPE(Geom_Line) ||
                      crv2->DynamicType() == STANDARD_TYPE(Geom_Line);
      ...
      if (has_circle && has_line) {
        ...
        if (ang < 0.0314) {
          edges_to_tesselate.Add(
              crv1->DynamicType() == STANDARD_TYPE(Geom_Circle) ? edges.First()
                                                                : edges.Last());
          Logger::Notice("Sharp circular corner detecting, substituting with "
    ...
    for (; exp.More(); exp.Next()) {
      if (edges_to_tesselate.Contains(exp.Current())) {
        ...
        if (exp.Current().Orientation() == TopAbs_REVERSED) {
          for (int i = n - 1; i >= 1; --i) {
            mw.Add(BRepBuilderAPI_MakeEdge(tessellater.Value(i + 1),
                                           tessellater.Value(i))
                       .Edge());
          }
        } else {
...
bool OpenCascadeKernel::convert_impl(const taxonomy::loop::ptr loop,
  ...
  results.emplace_back(
      ConversionResult(loop->instance->as<IfcUtil::IfcBaseEntity>()->id(),
                       new OpenCascadeShape(shape), loop->surface_style));
  return true;
}

bool OpenCascadeKernel::convert_impl(const taxonomy::edge::ptr edge,
                                     IfcGeom::ConversionResults &results) {
  TopoDS_Wire shape = boost::get<TopoDS_Wire>(convert_curve(edge));

  results.emplace_back(
      ConversionResult(edge->instance->as<IfcUtil::IfcBaseEntity>()->id(),
                       new OpenCascadeShape(shape), edge->surface_style));
  return true;
}
