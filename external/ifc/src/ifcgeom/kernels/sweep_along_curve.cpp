...
bool OpenCascadeKernel::convert(const taxonomy::sweep_along_curve::ptr scs,
                                TopoDS_Shape &result) {
  ...
  if (face_.ShapeType() == TopAbs_FACE) {
    ...
    if (!IfcGeom::util::convert_wire_to_face(TopoDS::Wire(face_), face,
                                             settings)) {
      return false;
    }
  } else {
  ...
  const bool is_plane =
      surface && surface->DynamicType() == STANDARD_TYPE(Geom_Plane);
  ...
  if (is_plane) {
    ...
    // As per Informal propositions 2: The Directrix shall lie on the
    // ReferenceSurface. This is not always the case with the test files in the
    // repository. I am not sure how to deal with this and whether my
    // interpretation of the propositions is correct. However, if it has been
    // asserted that the vertices of the directrix do not conform to the
    // ReferenceSurface, the ReferenceSurface is ignored.
    {
      for (TopExp_Explorer exp(wire, TopAbs_VERTEX); exp.More(); exp.Next()) {
        if (pln.Distance(BRep_Tool::Pnt(TopoDS::Vertex(exp.Current()))) >
            ALMOST_ZERO) {
          directrix_on_plane = false;
          Logger::Message(Logger::LOG_WARNING,
                          "The Directrix does not lie on the ReferenceSurface",
                          scs->instance);
          break;
        }
      }
  ...
  if (is_plane && pln.Axis().Direction().IsNormal(directrix_tangent, 1.e-5) &&
      directrix_on_plane) {
    directrix.SetTransformation(
        gp_Ax3(directrix_origin, directrix_tangent, pln.Axis().Direction()),
        gp::XOY());
  } else if (!is_plane && surface) {
    ShapeAnalysis_Surface sas(surface);
    auto pnt2d = sas.ValueOfUV(directrix_origin,
                               settings_.get<settings::Precision>().get());
    ...
    */
    ...
    directrix.SetTransformation(
        gp_Ax3(directrix_origin, directrix_tangent, surface_normal), gp::XOY());
  } else {
    directrix.SetTransformation(gp_Ax3(directrix_origin, directrix_tangent),
                                gp::XOY());
  }
  ...
  if (surface) {
    surface_face = BRepBuilderAPI_MakeFace(
                       surface, settings_.get<settings::Precision>().get())
                       .Face();
  }
  ...
  if (!is_plane && surface) {
    ...
    for (; exp.More(); exp.Next()) {
      ShapeFix_Edge sfe;
      sfe.FixAddPCurve(TopoDS::Edge(exp.Current()), surface_face, false,
                       settings_.get<settings::Precision>().get());
    }
  }
  ...
  // NB: Note that StartParam and EndParam param are ignored and the assumption
  // is made that the parametric range over which to be swept matches the
  // IfcCurve in its entirety.

  // BRepOffsetAPI_MakePipeShell does not support FACE, so we need to manually
  // iterate over the wires, first processing the outer, then inner. Where the
  // cap face is constructed using MakeFace.
  ...
  for (int i = 0; i < 2; ++i) {
    for (TopExp_Explorer exp(face, TopAbs_WIRE); exp.More(); exp.Next()) {
      ...
      builder.Add(section);
      builder.SetTransitionMode(contains_circular_segments(wire) &&
                                        wire_is_c1_continuous(wire, 1.e-2)
      ...
      for (TopExp_Explorer exp(builder.Shape(), TopAbs_FACE); exp.More();
           exp.Next()) {
...
bool OpenCascadeKernel::convert_impl(const taxonomy::sweep_along_curve::ptr scs,
                                     IfcGeom::ConversionResults &results) {
  TopoDS_Shape shape;
  // For tiny radii occt will fail building the sweep, in which case we enlarge
  // the inputs to occt, and add a scale matrix to the output
  bool enlarged = false;
  ...
  if (scs->basis->kind() == taxonomy::FACE) {
    ...
    if (w->children.size() == 1 && w->children[0]->basis &&
        w->children[0]->basis->kind() == taxonomy::CIRCLE) {
      auto circ =
          std::static_pointer_cast<taxonomy::circle>(w->children[0]->basis);
      ...
      if (enlarged) {
        ...
        if (crv->matrix) {
          crv->matrix = taxonomy::make<taxonomy::matrix4>(
              Eigen::Scaling(enlarge_factor) * crv->matrix->ccomponents());
        } else {
          crv->matrix = taxonomy::make<taxonomy::matrix4>();
          crv->matrix->components().topLeftCorner<3, 3>() =
              Eigen::Scaling(enlarge_factor, enlarge_factor, enlarge_factor)
                  .toDenseMatrix();
        }
      }
    }
  ...
  if (enlarged) {
    m = taxonomy::make<taxonomy::matrix4>(Eigen::Scaling(1. / enlarge_factor) *
                                          scs->matrix->ccomponents());
  } else {
    m = scs->matrix;
  }
  results.emplace_back(
      ConversionResult(scs->instance->as<IfcUtil::IfcBaseEntity>()->id(), m,
                       new OpenCascadeShape(shape), scs->surface_style));
  return true;
}
