#include <ifcgeom/kernels/wire_utils.h>
...
bool IfcGeom::util::approximate_plane_through_wire(const TopoDS_Wire &wire,
                                                   gp_Pln &plane, double eps_) {
...
IfcGeom::util::triangulate_wire_result
IfcGeom::util::triangulate_wire(const std::vector<TopoDS_Wire> &wires,
  ...
  gp_Pln pln;
  if (!approximate_plane_through_wire(
          wires.front(), pln, std::numeric_limits<double>::infinity())) {
  ...
  for (auto it = wires.begin(); it != wires.end(); ++it) {
    ...
    for (; exp.More(); exp.Next()) {
      ...
      existing_edges.insert(
          std::make_pair(std::make_pair(uv1, uv0), TopoDS::Edge(e.Reversed())));
    }
  ...
  if (!tri.IsNull()) {
    ...
    for (int i = 1; i <= triangles.Length(); ++i) {
      if (face.Orientation() == TopAbs_REVERSED)
        ...
        triangles(i).Get(n123[0], n123[1], n123[2]);
      ...
      for (int j = 0; j < 3; ++j) {
        ...
        if (it != existing_edges.end()) {
          ...
          if (jt != new_edges.end()) {
            ...
            new_edges.insert(
                std::make_pair(std::make_pair(uvnodes[1], uvnodes[0]),
                               TopoDS::Edge(ne.Reversed())));
          }
        }
      }
...
namespace {
double get_wire_intersection_tolerance(
    const IfcGeom::util::wire_tolerance_settings &settings,
  ...
  if (settings.use_wire_intersection_tolerance) {
    ...
    if (settings.vertex_clustering_epsilon > 0.) {
      ...
      return (std::min)(IfcGeom::util::min_edge_length(wire) / 2.,
...
bool IfcGeom::util::wire_intersections(
    const TopoDS_Wire &wire, TopTools_ListOfShape &wires,
    const wire_tolerance_settings &settings) {
  ...
  for (int i = 2; i < n; ++i) {
    ...
    for (std::vector<int>::const_iterator it = js.begin(); it != js.end();
         ++it) {
      int j = *it;
      ...
      // segments.
      const bool unbounded_intersects =
          (!ecc.Extrema().IsParallel() && ecc.NbExtrema() == 1 &&
           ecc.Distance(1) < eps);
      if (unbounded_intersects) {
        ...
        // if (saw.CheckIntersectingEdges(i + 1, j + 1, points2d, points3d,
        // errors)) {
        ...
        if (u11 < U1 && U1 < u12 && u21 < U2 && U2 < u22) {
          ...
          for (int fb = 0; fb <= 1; ++fb) {
            ...
            // @todo this is a change in behaviour with eps precomputed from the
            // kernel instead of adaptively calculated for the successive
            // iterations.
...
void IfcGeom::util::select_largest(const TopTools_ListOfShape &shapes,
                                   TopoDS_Shape &largest) {
  double mass = 0.;
  ...
  for (; it.More(); it.Next()) {
    ...
    */
    ...
    bb.Get(xyz_min[0], xyz_min[1], xyz_min[2], xyz_max[0], xyz_max[1],
           xyz_max[2]);

    // @todo hard coded precision.
    // @todo this is a really strange measure for wire size. Why not use
    // newell's method to project to plane and then calculate size of the 2d
    // bbox?
...
bool IfcGeom::util::wire_to_sequence_of_point(const TopoDS_Wire &w,
                                              TColgp_SequenceOfPnt &p) {
...
void IfcGeom::util::sequence_of_point_to_wire(const TColgp_SequenceOfPnt &p,
                                              TopoDS_Wire &w, bool close) {
...
void IfcGeom::util::remove_collinear_points_from_loop(
    TColgp_SequenceOfPnt &polygon, bool closed, double tol) {
  const int start = closed ? 1 : 2;
  const int end = polygon.Length() - (closed ? 0 : 1);
  std::vector<bool> to_remove(polygon.Length(), false);
  for (int i = start; i <= end; ++i) {
    const gp_Pnt &a =
        polygon.Value(((i - 2 + polygon.Length()) % polygon.Length()) + 1);
...
void IfcGeom::util::remove_duplicate_points_from_loop(
    TColgp_SequenceOfPnt &polygon, bool closed, double tol) {
  tol *= tol;
...
// Returns the vertex part of an TopoDS_Edge edge that is not TopoDS_Vertex
// vertex
...
TopoDS_Edge
find_next(const TopTools_IndexedMapOfShape &edge_set,
          const TopTools_IndexedDataMapOfShapeListOfShape &vertex_to_edges,
          const TopoDS_Vertex &current, const TopoDS_Edge &previous_edge) {
...
bool IfcGeom::util::fill_nonmanifold_wires_with_planar_faces(
    TopoDS_Shape &shape, double tol) {
  ...
  for (int i = 1; i <= num_edges; ++i) {
    ...
    if (count == 1) {
      ...
      TopExp::MapShapesAndAncestors(edge, TopAbs_VERTEX, TopAbs_EDGE,
                                    vertex_to_edges);
      edge_set.Add(edge);
    }
  }
  ...
  for (int i = 1; i <= num_verts; ++i) {
    ...
    for (;;) {
      ...
      TopoDS_Edge edge =
          find_next(edge_set, vertex_to_edges, current, previous_edge);
      if (edge.IsNull()) {
        return false;
      }
...
bool IfcGeom::util::convert_curve_to_wire(const Handle(Geom_Curve) & curve,
                                          TopoDS_Wire &wire) {
  try {
...
bool IfcGeom::util::convert_wire_to_face(
    const TopoDS_Wire &w, TopoDS_Face &face,
    const IfcGeom::util::wire_tolerance_settings &settings) {
  TopoDS_Wire wire = w;

  TopTools_ListOfShape results;

  if (settings.use_wire_intersection_check &&
      util::wire_intersections(wire, results, settings)) {
    Logger::Warning("Self-intersections with " +
                    boost::lexical_cast<std::string>(results.Extent()) +
                    " cycles detected");
  ...
  for (; exp.More(); exp.Next()) {
    ...
    Handle(Geom_Curve) crv =
        BRep_Tool::Curve(TopoDS::Edge(exp.Current()), a, b);
    if (crv->DynamicType() != STANDARD_TYPE(Geom_Line)) {
      is_2d = false;
      break;
    }
  ...
  if (!is_2d) {
    // For 2d wires (e.g. profiles) a higher tolerance for plane fitting is
    // never required.
...
bool IfcGeom::util::convert_wire_to_faces(
    const TopoDS_Wire &w, TopoDS_Compound &faces,
    const IfcGeom::util::wire_tolerance_settings &settings) {
  bool is_2d = true;
  TopExp_Explorer exp(w, TopAbs_EDGE);
  for (; exp.More(); exp.Next()) {
  ...
  TopTools_ListOfShape results;
  if (settings.use_wire_intersection_check &&
      util::wire_intersections(w, results, settings)) {
    Logger::Warning("Self-intersections with " +
                    boost::lexical_cast<std::string>(results.Extent()) +
                    " cycles detected");
  } else {
  ...
  for (auto &p : face_list) {
    if (p.first >= max_area / 10.) {
      ...
      Logger::Warning("Ignoring self-intersection loop with area " +
