...
bool OpenCascadeKernel::convert(const taxonomy::solid::ptr solid,
                                TopoDS_Shape &result) {
  TopoDS_Shape S;
  ...
  if (solid->instance->declaration().is("IfcHalfSpaceSolid")) {
    ...
    const auto &m =
        taxonomy::cast<taxonomy::plane>(face->basis)->matrix->ccomponents();
    gp_Pln pln(convert_xyz2<gp_Pnt>(m.col(3)), convert_xyz2<gp_Dir>(m.col(2)));
    const gp_Pnt pnt = pln.Location().Translated(
        face->orientation.get_value_or(false) ? pln.Axis().Direction()
                                              : -pln.Axis().Direction());
    ...
    if (!face->children.empty()) {
      ...
      if (convert((taxonomy::loop::ptr)face->children[0], wire) &&
          wire.Closed() && convert(face->matrix, gtrsf)) {
        gp_Trsf trsf = gtrsf.Trsf();
        TopoDS_Shape prism = BRepPrimAPI_MakePrism(
            BRepBuilderAPI_MakeFace(wire), gp_Vec(0, 0, 200));
        gp_Trsf down;
        down.SetTranslation(gp_Vec(0, 0, -100.0));
    ...
    return true;
             ...
             solid->children[0]->children[0]->basis->kind() ==
                 taxonomy::SPHERE) {
    auto sphere = taxonomy::cast<taxonomy::sphere>(
        solid->children[0]->children[0]->basis);
    gp_GTrsf gtrsf;
...
bool OpenCascadeKernel::convert_impl(const taxonomy::solid::ptr solid,
  ...
  results.emplace_back(ConversionResult(
      solid->instance->as<IfcUtil::IfcBaseEntity>()->id(), solid->matrix,
      new OpenCascadeShape(shape), solid->surface_style));
  return true;
}
