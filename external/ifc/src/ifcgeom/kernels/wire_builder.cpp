...
// moved to p
TopoDS_Wire IfcGeom::util::adjust(const TopoDS_Wire &w, const TopoDS_Vertex &v,
                                  const gp_Pnt &p) {
  ...
  if (all_linear) {
    ...
    Handle(Geom_Curve) crv =
        BRep_Tool::Curve(TopoDS::Edge(edges.First()), a, b);
    crv->D0((a + b) / 2., p2);
...
double IfcGeom::util::deflection_for_approximating_circle(double radius,
                                                          double param) {
  return -radius * std::cos(1. / 2. * param) * std::cos(param) -
         radius * std::sin(1. / 2. * param) * std::sin(param) + radius;
}

bool IfcGeom::util::create_edge_over_curve_with_log_messages(
    const Handle_Geom_Curve &crv, const double eps, const gp_Pnt &p1,
    const gp_Pnt &p2, TopoDS_Edge &result) {
      ...
      for (int i = 0; i < 2; ++i) {
        ...
        if (extrema.IsDone()) {
          ...
          if (dmin == std::numeric_limits<double>::infinity()) {
            ...
            Logger::Error("Distance of " +
                          boost::lexical_cast<std::string>(std::sqrt(dmin)) +
                          " exceeds tolerance");
          }
        } else {
...
void IfcGeom::util::wire_builder::operator()(const TopoDS_Shape &a,
                                             const TopoDS_Shape &b, bool last) {
  ...
  // Distance is too large for attempting to move end points, add intermediate
  // edge
  if (dist > 1000. * p_) {
    mw_.Add(w1);
    mw_.Add(BRepBuilderAPI_MakeEdge(p1, p2));
    Logger::Warning("Added additional segment to close gap with length " +
                        boost::lexical_cast<std::string>(dist) + " to:",
                    inst_);
    goto check;
  }

  {
    ...
    if (last_edges.Extent() == 1 && first_edges.Extent() == 1) {
      Handle(Geom_Curve) c1 =
          BRep_Tool::Curve(TopoDS::Edge(last_edges.First()), _, __);
      Handle(Geom_Curve) c2 =
          BRep_Tool::Curve(TopoDS::Edge(first_edges.First()), _, __);
      ...
      if (is_line1 || (is_circle1 && !is_line2)) {
        ...
        Logger::Notice("Adjusted edge end-point with distance " +
                           boost::lexical_cast<std::string>(dist) + " on:",
                       inst_);
