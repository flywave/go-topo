...
bool apply_in_batches(
    IfcGeom::util::boolean_settings bst, const TopoDS_Shape &first_operand,
    std::vector<std::pair<double, TopoDS_Shape>> &opening_vector,
    BOPAlgo_Operation occ_op, TopoDS_Shape &result) {
  ...
  for (;; ++it) {
    if (it == opening_vector.end() || jt->first / it->first > 10.) {
      ...
      if (IfcGeom::util::boolean_operation(bst, result, opening_list, occ_op,
                                           intermediate_result)) {
...
bool OpenCascadeKernel::convert_impl(const taxonomy::boolean_result::ptr br,
  ...
  for (auto &c : br->children) {
    ...
    if (first && br->operation == taxonomy::boolean_result::SUBTRACTION) {
      ...
      IfcGeom::util::flatten_shape_list(
          cr, a, false, true, settings_.get<settings::Precision>().get());
      first_item_style = c->surface_style;
      if (!first_item_style && c->kind() == taxonomy::COLLECTION) {
        // @todo recursively right?
        first_item_style =
            taxonomy::cast<taxonomy::geom_item>(
                taxonomy::cast<taxonomy::collection>(c)->children[0])
                ->surface_style;
      }

      if (settings_.get<settings::DisableBooleanResult>().get()) {
        results.emplace_back(IfcGeom::ConversionResult(
            br->instance->as<IfcUtil::IfcBaseEntity>()->id(), br->matrix,
      ...
      for (auto &r : cr) {
        ...
        if (is_unbounded_halfspace(S)) {
          ...
          // #2665 we also set a precision-independent threshold, because in the
          // boolean op routine the working fuzziness might still be increased.
          if (d < tol * 20. || d < 0.00002) {
            Logger::Message(
                Logger::LOG_WARNING,
                "Halfspace subtraction yields unchanged volume:", c->instance);
            continue;
          } else {
            S = result;
          }
  ...
  if (br->operation == taxonomy::boolean_result::SUBTRACTION && !a.IsNull() &&
      a.ShapeType() == TopAbs_COMPOUND && TopoDS_Iterator(a).More() &&
      util::is_nested_compound_of_solid(a)) {
    TopoDS_Compound C;
    BRep_Builder B;
    B.MakeCompound(C);
    ...
    for (; it.More(); it.Next()) {
      TopoDS_Shape part;
      if (util::boolean_operation(bst, it.Value(), b, op_to_occt(br->operation),
                                  part)) {
        B.Add(C, part);
      } else {
        valid_result = false;
    ...
    valid_result =
        util::boolean_operation(bst, a, b, op_to_occt(br->operation), r);
  }
