...
namespace {
void find_neighbours(IfcGeom::impl::tree<int> &tree,
                     std::vector<std::unique_ptr<gp_Pnt>> &pnts,
                     std::set<int> &visited, int p, double eps) {
  visited.insert(p);
  ...
  for (int j : js) {
    ...
    if (visited.find(j) == visited.end()) {
      // @todo, making this recursive removes the dependence on the initial
      // ordering, but will likely result in empty results when all vertices are
      // within 1 eps from another point.
...
IfcGeom::OpenCascadeKernel::faceset_helper::faceset_helper(
    ...
    : kernel_(kernel), non_manifold_(false) {
  // @todo use pointers?
    ...
    for (auto &l : f->children) {
      ...
      for (auto &e : l->children) {
        for (size_t i = 0; i < 2; ++i) {
          ...
          auto &p = boost::get<ifcopenshell::geometry::taxonomy::point3::ptr>(
              i == 0 ? e->start : e->end);
          if (point_identities_visited.find(p->identity()) ==
              point_identities_visited.end()) {
  ...
  for (size_t i = 0; i < 3; ++i) {
    ...
    if (d > kernel->settings()
                    .get<ifcopenshell::geometry::settings::Precision>()
                    .get() *
                10. &&
        d < bdiff) {
      bdiff = d;
  ...
  eps_ = kernel->settings()
             .get<ifcopenshell::geometry::settings::Precision>()
             .get() *
         10. * (std::min)(1.0, bdiff);
  ...
  for (int i = 0; i < 3; ++i) {
    // Some times files, have large tolerance values specified collapsing too
    // many vertices. This case we detect below and re-run the loop with smaller
    // epsilon. Normally the body of this loop would only be executed once.
    ...
    for (int pnt_i = 0; pnt_i < (int)pnts.size(); ++pnt_i) {
      if (pnts[pnt_i]) {
        unique.insert(std::make_tuple((*pnts[pnt_i]).X(), (*pnts[pnt_i]).Y(),
                                      (*pnts[pnt_i]).Z()));
      }
    }
    ...
    if (unique.size() != num_retained) {
      Logger::Notice("Collapsed vertices from " + std::to_string(pnts.size()) +
                     " (" + std::to_string(unique.size()) + " unique) to " +
                     std::to_string(num_retained));
    }
  ...
  if (duplicates_.size() || loops_removed ||
      (non_manifold && shell->closed.get_value_or(false))) {
    Logger::Warning(boost::lexical_cast<std::string>(duplicate_faces) +
                    " duplicate faces removed, " +
                    boost::lexical_cast<std::string>(loops_removed) +
                    " degenerate loops eliminated and " +
                    boost::lexical_cast<std::string>(non_manifold) +
                    " non-manifold edges");
  }
}

void IfcGeom::OpenCascadeKernel::faceset_helper::loop_(
    const ifcopenshell::geometry::taxonomy::loop::ptr ps,
  ...
  for (auto &edge : ps->children) {
    auto A =
        boost::get<ifcopenshell::geometry::taxonomy::point3::ptr>(edge->start)
            ->identity();
    auto B =
        boost::get<ifcopenshell::geometry::taxonomy::point3::ptr>(edge->end)
            ->identity();
...
bool IfcGeom::OpenCascadeKernel::faceset_helper::edge(int A, int B,
                                                      TopoDS_Edge &e) {
...
bool IfcGeom::OpenCascadeKernel::faceset_helper::wire(
    const ifcopenshell::geometry::taxonomy::loop::ptr loop, TopoDS_Wire &w) {
...
bool IfcGeom::OpenCascadeKernel::faceset_helper::wires(
    const ifcopenshell::geometry::taxonomy::loop::ptr loop,
  ...
  if (count >= 3) {
    ...
    if (!kernel_->settings()
             .get<ifcopenshell::geometry::settings::NoWireIntersectionCheck>()
             .get() &&
        util::wire_intersections(
            wire, results,
            {!kernel_->settings()
                  .get<ifcopenshell::geometry::settings::
                           NoWireIntersectionCheck>()
                  .get(),
             !kernel_->settings()
                  .get<ifcopenshell::geometry::settings::
...
IfcGeom::OpenCascadeKernel::faceset_helper::~faceset_helper() {
  // @todo this is super ugly, but how else can we be notified that the
  // unique_ptr goes out of scope? Perhaps just supply a custom std::deleter?
