#include <ifcgeom/kernels/sweep_utils.h>
...
bool IfcGeom::util::wire_to_ax(const TopoDS_Wire &wire, gp_Ax2 &directrix) {
  ...
  if (v0.IsSame(v1) && map.Contains(v0) && map.FindFromKey(v0).Extent() == 2) {
    ...
    crv->D1(TopExp::FirstVertex(e1).IsSame(v0) ? u0 : u1, directrix_origin,
            directrix_tangent);

    accum += directrix_tangent;

    crv = BRep_Tool::Curve(e2, u0, u1);
    crv->D1(TopExp::FirstVertex(e2).IsSame(v0) ? u0 : u1, directrix_origin,
            directrix_tangent);
...
void IfcGeom::util::process_sweep_as_extrusion(const TopoDS_Wire &wire,
  ...
  // @todo we could be extruding the wire only when we know this is an
  // intermediate edge.
  TopoDS_Face face = BRepBuilderAPI_MakeFace(section).Face();
  result = BRepPrimAPI_MakePrism(face, depth * dir).Shape();
}

void IfcGeom::util::process_sweep_as_revolution(const TopoDS_Wire &wire,
                                                const TopoDS_Wire &section,
                                                TopoDS_Shape &result) {
  TopExp_Explorer exp(wire, TopAbs_EDGE);
  TopoDS_Edge e = TopoDS::Edge(exp.Current());
...
void IfcGeom::util::process_sweep_as_pipe(const TopoDS_Wire &wire,
                                          const TopoDS_Wire &section,
                                          TopoDS_Shape &result,
                                          bool force_transformed) {
  // This tolerance is fairly high due to the linear edge substitution for small
  // (or large radii) conical curves.
  const bool is_continuous = wire_is_c1_continuous(wire, 1.e-2);
  BRepOffsetAPI_MakePipeShell builder(wire);
  builder.Add(section);
  builder.SetTransitionMode(is_continuous || force_transformed
                                ? BRepBuilderAPI_Transformed
                                : BRepBuilderAPI_RightCorner);
  try {
    builder.Build();
  } catch (Standard_Failure &e) {
    // We fallback to BRepBuilderAPI_Transformed, but likely with visual
    // artefacts.
...
void IfcGeom::util::sort_edges(const TopoDS_Wire &wire,
  ...
  // @todo this probably still does not work on a closed wire consisting of one
  // (circular) edge.
...
void IfcGeom::util::break_closed(const TopoDS_Wire &wire,
...
void IfcGeom::util::segment_adjacent_non_linear(
    const TopoDS_Wire &wire, std::vector<TopoDS_Wire> &wires) {
  std::vector<TopoDS_Edge> sorted_edges;
  sort_edges(wire, sorted_edges);
...
void IfcGeom::util::process_sweep(const TopoDS_Wire &wire, double radius,
                                  TopoDS_Shape &result) {
