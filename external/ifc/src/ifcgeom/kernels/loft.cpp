...
bool OpenCascadeKernel::convert(const taxonomy::loft::ptr loft,
                                TopoDS_Shape &result) {
  ...
  if (non_polygonal) {
    if (loft->children.size() == 2) {
      ...
      TopoDS_Shape f0, f1;
      if (!convert(
              std::static_pointer_cast<taxonomy::face>(loft->children.front()),
              f0) ||
          !convert(
              std::static_pointer_cast<taxonomy::face>(loft->children.back()),
              f1)) {
        return false;
      }
  ...
  for (auto it = loft->children.begin(); it < loft->children.end() - 1; ++it) {
    ...
    for (int i = 0; i < 2; ++i) {
      if (fa[i]->kind() == taxonomy::FACE) {
        if (!convert(std::static_pointer_cast<taxonomy::face>(fa[i]),
                     shps[i])) {
          return false;
        }
      }
    ...
    for (; a.More() && b.More(); a.Next(), b.Next()) {
      ...
      // Documentation says unconnected edges are automatically connected, but
      ...
      */
      ...
      auto f = BRepBuilderAPI_MakeFace(
                   BRepBuilderAPI_MakePolygon(e1a, e1b, e3b, true).Wire())
                   .Face();
      BB.Add(comp, f);
      auto g = BRepBuilderAPI_MakeFace(
                   BRepBuilderAPI_MakePolygon(e3b, e3a, e1a, true).Wire())
                   .Face();
      BB.Add(comp, g);
    }
  }

  // create_solid_from_faces(faces, result,
...
bool OpenCascadeKernel::convert_impl(const taxonomy::loft::ptr loft,
  ...
  results.emplace_back(ConversionResult(
      loft->instance->as<IfcUtil::IfcBaseEntity>()->id(), loft->matrix,
      new OpenCascadeShape(shape), loft->surface_style));
  return true;
}
