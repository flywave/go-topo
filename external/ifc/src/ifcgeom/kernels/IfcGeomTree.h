...
#include <future>
#include <ifcgeom/kernels/clash_utils.h>
#include <mutex>
#include <stack>
...
double max_distance_inside(const TopoDS_Shape &volume,
...
template <typename T> class tree {
    ...
    while (exp.More()) {
      ...
      for (int i = 1; i <= edgeFaceMap.Extent(); ++i) {
        if (edgeFaceMap(i).Extent() < 2) {
          // This edge is not shared by two faces, indicating a potential
          // opening
          return false;
        }
      }
  ...
  bool is_point_in_shape(
      ...
      bool should_check_again = false) const {
    ray v_ray;
    v_ray.origin[0] = v.X();
    v_ray.origin[1] = v.Y();
    v_ray.origin[2] = v.Z();
      ...
      */
      ...
      if (bvh->IsOuter(i)) {
        ...
        for (int j = bvh->BegPrimitive(i); j <= bvh->EndPrimitive(i); ++j) {
          ...
          << " " << ray_origin.Z() << std::endl; std::cout << "inside-tri " <<
          ta.X() << " " << ta.Y() << " " << ta.Z() << std::endl; std::cout <<
          "inside-tri " << tb.X() << " " << tb.Y() << " " << tb.Z() <<
          std::endl; std::cout << "inside-tri " << tc.X() << " " << tc.Y() << "
          " << tc.Z() << std::endl;
          */
          double at, au, av;
          if (intersectRayTriangle(ray_origin, ray_vector, ta, tb, tc, at, au,
                                   av, false)) {
            if (std::abs(at) < 1e-4) {
              // The point is basically lying on a face so inside/outside is
              // ambiguous.
              return false;
            }
            // At is a signed intersection distance (positive is along
            // +ray_vector)
            if (at > -1e-5) {
              total_intersections++;
            }
  ...
  std::tuple<double, std::array<double, 3>, std::array<double, 3>> pierce_shape(
      const gp_Vec &e1, const gp_Vec &e2,
      const opencascade::handle<BVH_Tree<double, 3, BVH_BinaryTree>> &bvh,
      const std::vector<std::array<int, 3>> &tris,
      const std::vector<gp_Pnt> &verts,
      const std::vector<gp_Vec> &normals) const {
        ...
        for (int j = bvh->BegPrimitive(i); j <= bvh->EndPrimitive(i); ++j) {
          ...
          if (intersectRayTriangle(ray_origin, ray_vector, ta, tb, tc, at, au,
            ...
            if (at > 0 && at < edge_length) {
              ...
              gp_Vec int_vec =
                  aw * ta + au * tb + av * tc; // Intersection point

              if (is_point_on_line(int_vec, ta, tb) ||
  ...
  bool is_point_on_line(const gp_Pnt &point, const gp_Pnt &lineStart,
                        const gp_Pnt &lineEnd) const {
    // Create vectors
    ...
    // by checking if the cross product is (near) zero vector, indicating
    // collinearity.
  ...
  bool is_point_on_line(const gp_Vec &point, const gp_Vec &lineStart,
                        const gp_Vec &lineEnd) const {
    // Create vectors
  ...
  clash_bvh(opencascade::handle<BVH_Tree<double, 3, BVH_BinaryTree>> bvh_a,
            opencascade::handle<BVH_Tree<double, 3, BVH_BinaryTree>> bvh_b,
            double extend = 0.0) const {
    ...
    for (int i = 0; i < bvh_a->Length(); ++i) {
      ...
      while (!stack.empty()) {
        ...
        BVH_TreeBase<Standard_Real, 3>::BVH_VecNt bvh_b_min =
            bvh_b->MinPoint(j);
        BVH_TreeBase<Standard_Real, 3>::BVH_VecNt bvh_b_max =
            bvh_b->MaxPoint(j);
  ...
  clash test_intersection(const T &tA, const T &tB, double tolerance,
                          bool check_all = true) const {
    ...
    opencascade::handle<BVH_Tree<double, 3, BVH_BinaryTree>> bvh_a =
        bvhs_.find(tA)->second;
    opencascade::handle<BVH_Tree<double, 3, BVH_BinaryTree>> bvh_b =
        bvhs_.find(tB)->second;

    std::unordered_map<int, std::vector<int>> bvh_clashes =
        clash_bvh(bvh_a, bvh_b, max_protrusion);
    ...
    for (const auto &pair : bvh_clashes) {
      ...
      for (int i = bvh_a->BegPrimitive(bvh_a_i);
           i <= bvh_a->EndPrimitive(bvh_a_i); ++i) {
        ...
        if (points_in_b.empty()) {
          ...
          // Protrusions take priority over piercings. We only check for
          // piercings if:
          ...
          if (protrusion == -std::numeric_limits<double>::infinity() &&
              pierce != max_protrusion) {
            std::array<std::tuple<double, std::array<double, 3>,
                                  std::array<double, 3>>,
                       3>
                pierce_results = {pierce_shape(v1_a_vec, v2_a_vec, bvh_b,
                                               tris_b, verts_b, normals_b),
                                  pierce_shape(v1_a_vec, v3_a_vec, bvh_b,
                                               tris_b, verts_b, normals_b),
                                  pierce_shape(v2_a_vec, v3_a_vec, bvh_b,
                                               tris_b, verts_b, normals_b)};
          ...
          // Since there were no points in b, we don't need to check for
          // protrusions.
          continue;
        }
        ...
        for (const auto &bvh_b_i : bvh_b_is) {
          for (int j = bvh_b->BegPrimitive(bvh_b_i);
               j <= bvh_b->EndPrimitive(bvh_b_i); ++j) {
            ...
            for (const auto &v : points_in_b) {
              ...
              /*
              std::cout << "POINT IN B " << v.X() << " " << v.Y() << " " <<
              v.Z() << std::endl; std::cout << "dir-> " << normal_b.X() << " "
              << normal_b.Y() << " " << normal_b.Z() << std::endl; std::cout <<
              "->tri " << v1_b[0] << " " << v1_b[1] << " " << v1_b[2] <<
              std::endl; std::cout << "->tri " << v2_b[0] << " " << v2_b[1] << "
              " << v2_b[2] << std::endl; std::cout << "->tri " << v3_b[0] << " "
              << v3_b[1] << " " << v3_b[2] << std::endl;
              */

              // Do (cheaper) line check.
              double at, au, av;
              if (intersectRayTriangle(ray_origin, normal_b, ta, tb, tc, at, au,
                                       av, false)) {
                double current_v_protrusion = at;

                // std::cout << "We got a current protrusion " <<
                // current_v_protrusion << std::endl;
                if (current_v_protrusion < v_protrusion) {
                  double aw = 1.0f - au - av; // Barycentric coordinate for ta
                  gp_Vec point_on_b =
                      aw * ta + au * tb + av * tc; // Intersection point
                  // std::cout << "New v_protrusion winner of " <<
                  // current_v_protrusion << std::endl;
                  v_protrusion = current_v_protrusion;
                  v_protrusion_point = {v.X(), v.Y(), v.Z()};
                  v_surface_point = {point_on_b.X(), point_on_b.Y(),
                                     point_on_b.Z()};
        ...
        if (v_protrusion != std::numeric_limits<double>::infinity()) {
          if (v_protrusion > protrusion) {
            // std::cout << "New actual protrusion winner of " << v_protrusion
            // << std::endl;
      ...
      for (int i = bvh_a->BegPrimitive(bvh_a_i);
        ...
        for (const auto &bvh_b_i : bvh_b_is) {
          for (int j = bvh_b->BegPrimitive(bvh_b_i);
            ...
            gp_Vec int1, int2;
            if (trianglesIntersect(v1_a_vec, v2_a_vec, v3_a_vec, v1_b_vec,
                                   v2_b_vec, v3_b_vec, int1, int2,
                                   !allow_touching)) {
              if (allow_touching) {
                return {2,
                        tA,
  ...
  clash test_clearance(const T &tA, const T &tB, double clearance,
    ...
    std::unordered_map<int, std::vector<int>> bvh_clashes =
        clash_bvh(bvh_a, bvh_b, clearance);
    if (bvh_clashes.empty()) {
      return {-1, tA, tB, 0, {0, 0, 0}, {0, 0, 0}};
    }
  ...
  bool test(const TopoDS_Shape &A, const TopoDS_Shape &B,
            bool completely_within, double extend) const {
    if (extend > 0.) {
...
public:
  void add(const T &t, const Bnd_Box &b) { tree_.Add(t, b); }
  ...
  std::vector<T> select_box(const T &t, bool completely_within = false,
                            double extend = -1.e-5) const {
  ...
  std::vector<T> select_box(const Bnd_Box &b,
                            bool completely_within = false) const {
    selector s(b);
    tree_.Select(s);
    if (completely_within) {
  ...
  std::unique_ptr<BVH_BoxSet<double, 3>>
  build_box_set(const std::vector<T> &elements) const {
    double x, y, z, X, Y, Z;
    std::unique_ptr<BVH_BoxSet<double, 3>> box_set =
        std::make_unique<BVH_BoxSet<double, 3>>();
  ...
  std::vector<std::vector<clash_task>>
  allocate_tasks_to_threads(std::vector<clash_task> &task_queue) const {
    ...
    for (int i = 0; i < num_threads; ++i) {
      ...
      auto endIter = (i == num_threads - 1)
                         ? task_queue.end()
                         : std::next(startIter, tasks_per_thread);
      threaded_tasks[i] = std::vector<clash_task>(startIter, endIter);
    }
    return threaded_tasks;
  }

  std::vector<clash> clash_intersection_many(const std::vector<T> &set_a,
    ...
    const opencascade::handle<BVH_Tree<double, 3, BVH_BinaryTree>> &bvh_a =
        box_set_a->BVH();
    const opencascade::handle<BVH_Tree<double, 3, BVH_BinaryTree>> &bvh_b =
        box_set_b->BVH();

    std::unordered_map<int, std::vector<int>> bvh_clashes =
        clash_bvh(bvh_a, bvh_b, 0.0);
    ...
    std::vector<std::vector<clash_task>> threaded_tasks =
        allocate_tasks_to_threads(task_queue);

    std::vector<std::thread> threads;
    std::mutex results_mutex;

    for (auto &tasks : threaded_tasks) {
      threads.emplace_back([this, &tasks, &results, &results_mutex, tolerance,
                            check_all] {
        ...
        for (auto &task : tasks) {
          ...
          if (is_manifold_.find(task.b)->second) {
            is_manifold = true;
            clash intersection =
                test_intersection(task.a, task.b, tolerance, check_all);
          ...
          if (is_manifold_.find(task.a)->second) {
            is_manifold = true;
            clash intersection =
                test_intersection(task.b, task.a, tolerance, check_all);
            if (intersection.clash_type != -1) {
              // Replace the clash result if any of these criteria apply:
              // - We don't have a clash yet
              // - Our previous clash is piercing, and our new one is a
              // protrusion
        ...
        }
        {
          ...
          results.insert(results.end(), thread_results.begin(),
                         thread_results.end());
        }
      });
    }
  ...
  std::vector<clash> clash_collision_many(const std::vector<T> &set_a,
                                          const std::vector<T> &set_b,
                                          bool allow_touching = false) const {
    std::vector<clash_task> task_queue;
    std::vector<clash> results;
    ...
    for (auto &tasks : threaded_tasks) {
      threads.emplace_back(
          [this, &tasks, &results, &results_mutex, allow_touching] {
            std::vector<clash> thread_results;
            for (auto &task : tasks) {
              const auto &obb_a = obbs_.find(task.a)->second;
              auto obb_b = obbs_.find(task.b)->second;
  ...
  std::vector<clash> clash_clearance_many(const std::vector<T> &set_a,
                                          const std::vector<T> &set_b,
                                          double clearance = 0.05,
                                          bool check_all = false) const {
    std::vector<clash_task> task_queue;
    std::vector<clash> results;
    ...
    for (auto &tasks : threaded_tasks) {
      threads.emplace_back([this, &tasks, &results, &results_mutex, clearance,
                            check_all] {
        std::vector<clash> thread_results;
        for (auto &task : tasks) {
          const auto &obb_a = obbs_.find(task.a)->second;
  ...
  std::vector<T> select(const T &t, bool completely_within = false,
                        double extend = 0.0) const {
    distances_.clear();
  ...
  std::vector<T> select(const TopoDS_Shape &s, bool completely_within = false,
                        double extend = -1.e-5) const {
    distances_.clear();
    protrusion_distances_.clear();
  ...
  std::vector<T> select(const IfcGeom::BRepElement *elem,
                        bool completely_within = false,
                        double extend = -1.e-5) const {
    ...
    gp_Trsf tr;
    tr.SetValues(m(0, 0), m(0, 1), m(0, 2), m(0, 3), m(1, 0), m(1, 1), m(1, 2),
...
protected:
  ...
  std::map<const IfcUtil::IfcBaseClass *,
           ifcopenshell::geometry::taxonomy::matrix4::ptr>
      placements_;
  std::map<std::string, std::vector<double>> local_verts_;
  std::map<std::string, std::vector<int>> local_faces_;
  std::map<std::string,
           std::vector<ifcopenshell::geometry::taxonomy::style::ptr>>
      local_materials_;
  ...
  public:
    selector(const Bnd_Box &b) : tree_t::Selector(), bounds_(b) {}

    Standard_Boolean Reject(const Bnd_Box &b) const { return bounds_.IsOut(b); }
...
public:
  ...
  tree(IfcGeom::Iterator &it) { add_file(it); }

  void add_file(IfcParse::IfcFile &f,
                ifcopenshell::geometry::Settings settings) {
    ifcopenshell::geometry::Settings settings_ = settings;
    settings_.get<ifcopenshell::geometry::settings::IteratorOutput>().value =
        ifcopenshell::geometry::settings::NATIVE;
    settings_.get<ifcopenshell::geometry::settings::UseWorldCoords>().value =
        true;
    settings_.get<ifcopenshell::geometry::settings::ReorientShells>().value =
        true;
  ...
  template <typename T>
  void apply_matrix_to_flat_verts(
      const std::vector<T> &flat_list,
      const ifcopenshell::geometry::taxonomy::matrix4::ptr &matrix,
      std::vector<T> &result) {
    Eigen::Vector3d vin;
    result.clear();
    ...
    for (size_t i = 0; i < flat_list.size(); i += 3) {
      vin << flat_list[i], flat_list[i + 1], flat_list[i + 2];
  ...
  std::string uint8_to_b64(const std::vector<uint8_t> &uuids_array) {
    ...
    for (auto byte : uuids_array) {
      // Convert each byte to a two-digit hexadecimal string and append it to
      // the result
      char hex[3]; // Two characters for the hex value and one for the null
                   // terminator
  ...
  void add_element(IfcGeom::TriangulationElement *elem) {
...

    {
      ...
      gp_Trsf tr;
      tr.SetValues(m(0, 0), m(0, 1), m(0, 2), m(0, 3), m(1, 0), m(1, 1),
      ...
      std::unordered_map<std::tuple<int, int, int>, std::vector<size_t>,
                         boost::hash<std::tuple<int, int, int>>>
          quantized_normal_counts;
      ...
      for (size_t i = 0; i < fs.size(); i += 3) {
        ...
        auto quantized = std::make_tuple(static_cast<int>(cross.X() * 1000),
      ...
      std::vector<
          std::pair<double, decltype(quantized_normal_counts)::const_iterator>>
          area_to_it;

      for (auto it = quantized_normal_counts.cbegin();
           it != quantized_normal_counts.cend(); ++it) {
        double area_sum = 0.;
      ...
      std::sort(area_to_it.begin(), area_to_it.end(),
      ...
      for (auto it = ++area_to_it.rbegin();
           it != area_to_it.rend() && num_candidates < 10;
           ++it, ++num_candidates) {
    ...
    // Attempt to copy exactly what BRepExtrema_TriangleSet is doing under the
    // hood.
    const auto builder = new BVH_LinearBuilder<Standard_Real, 3>(
        BVH_Constants_LeafNodeSizeDefault, BVH_Constants_MaxTreeDepth);
    BVH_Triangulation<Standard_Real, 3> triangulation(builder);

    for (int i = 0; i < elem_verts.size(); i += 3) {
      triangulation.Vertices.push_back(
          BVH_Vec3d(elem_verts[i], elem_verts[i + 1], elem_verts[i + 2]));
      verts.push_back(
          gp_Pnt(elem_verts[i], elem_verts[i + 1], elem_verts[i + 2]));
    }
    ...
    for (int i = 0; i < elem_faces.size(); i += 3) {
      ...
      if (cross_product.Magnitude() > Precision::Confusion()) {
        triangulation.Elements.push_back(
            BVH_Vec4i(elem_faces[i], elem_faces[i + 1], elem_faces[i + 2],
                      original_tris_index));
        original_tris_index++;
    ...
    obbs_[t] = obb;
    max_protrusions_[t] =
        std::min(std::min(obb.XHSize(), obb.YHSize()), obb.ZHSize()) * 2;
  }
  ...
  void add_element(IfcGeom::BRepElement *elem) {
    ...
    auto compound =
        ((ifcopenshell::geometry::OpenCascadeShape *)compound_generic)->shape();
    ...
    if (enable_face_styles_) {
      ...
      for (; it.More(); it.Next(), ++git) {
        // Assumption is that the number of styles is small, so the linear
        // lookup time is not significant.
  ...
  std::vector<IfcGeom::ray_intersection_result>
  select_ray(const gp_Pnt &p0, const gp_Dir &d, double length = 1000.) const {
  ...
  bool enable_face_styles() const { return enable_face_styles_; }

  void enable_face_styles(bool b) { enable_face_styles_ = b; }
