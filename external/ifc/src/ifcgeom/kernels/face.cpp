...
#include <ifcgeom/kernels/face_definition.h>
...
struct surface_creation_visitor {
  ...
  Handle(Geom_Surface) operator()(const taxonomy::bspline_surface::ptr & bs) {
    ...
    i = 0;
    for (std::vector<int>::const_iterator it = umults.begin();
         it != umults.end(); ++it, ++i) {
      UMults(i) = *it;
    }
    i = 0;
    for (std::vector<int>::const_iterator it = vmults.begin();
         it != vmults.end(); ++it, ++i) {
      VMults(i) = *it;
    }
    return result = Handle(Geom_Surface)(new Geom_BSplineSurface(
  ...
  Handle(Geom_Surface) operator()(const taxonomy::cylinder::ptr & c) {
    ...
    return result = Handle(Geom_Surface)(new Geom_CylindricalSurface(
               gp_Ax3(OpenCascadeKernel::convert_xyz2<gp_Pnt>(m.col(3)),
                      OpenCascadeKernel::convert_xyz2<gp_Dir>(m.col(2)),
  ...
  Handle(Geom_Surface) operator()(const taxonomy::sphere::ptr & s) {
    ...
    return result = Handle(Geom_Surface)(new Geom_SphericalSurface(
               gp_Ax3(OpenCascadeKernel::convert_xyz2<gp_Pnt>(m.col(3)),
                      OpenCascadeKernel::convert_xyz2<gp_Dir>(m.col(2)),
                      OpenCascadeKernel::convert_xyz2<gp_Dir>(m.col(0))),
               s->radius));
  ...
  Handle(Geom_Surface) operator()(const taxonomy::torus::ptr & t) {
    ...
    return result = Handle(Geom_Surface)(new Geom_ToroidalSurface(
               gp_Ax3(OpenCascadeKernel::convert_xyz2<gp_Pnt>(m.col(3)),
                      OpenCascadeKernel::convert_xyz2<gp_Dir>(m.col(2)),
                      OpenCascadeKernel::convert_xyz2<gp_Dir>(m.col(0))),
               t->radius1, t->radius2));
  ...
  TopoDS_Wire get_edges_as_wire(const taxonomy::item::ptr &i) {
    // It's a bit more convenient to use high level BRepPrimAPI calls that
    // operate on topology. On a single edge that will create a
    // Geom_TrimmedCurve for us.
  ...
  Handle(Geom_Surface) operator()(const taxonomy::extrusion::ptr & e) {
    ...
    result = Handle(Geom_Surface)(new Geom_SurfaceOfLinearExtrusion(
        crv, OpenCascadeKernel::convert_xyz<gp_Dir>(*e->direction)));
  ...
  Handle(Geom_Surface) operator()(const taxonomy::revolve::ptr & e) {
    gp_Ax1 ax(OpenCascadeKernel::convert_xyz<gp_Pnt>(*e->axis_origin),
    ...
    if (e->basis &&
        (e->basis->kind() == taxonomy::EDGE ||
         (e->basis->kind() == taxonomy::LOOP &&
          taxonomy::cast<taxonomy::loop>(e->basis)->children.size() == 1))) {
      ...
      if (exp.More()) {
        Handle(Geom_Surface) surf =
            BRep_Tool::Surface(TopoDS::Face(exp.Current()));
        exp.Next();
        if (!exp.More()) {
          return result = surf;
        }
    ...
    auto crv = get_curve(e->basis);
    result = Handle(Geom_Surface)(new Geom_SurfaceOfRevolution(crv, ax));
...
Handle(Geom_Surface)
    OpenCascadeKernel::convert_surface(const taxonomy::ptr surface) {
  surface_creation_visitor v{this};
  if (dispatch_surface_creation<surface_creation_visitor, 0>::dispatch(surface,
                                                                       v)) {
    return v.result;
  } else {
...
bool OpenCascadeKernel::convert(const taxonomy::face::ptr face,
                                TopoDS_Shape &result, bool reversed_surface) {
#ifdef IFOPSH_DEBUG
  ...
  // The number of outer bounds should be one according to the schema. Also Open
  ...
  if (num_bounds > 1 && num_outer_bounds > 1 &&
    ...
    Logger::Message(Logger::LOG_ERROR,
                    "Invalid configuration of boundaries for:", face->instance);
    return false;
  }

  if (num_outer_bounds > 1) {
    Logger::Message(Logger::LOG_WARNING,
                    "Multiple outer boundaries for:", face->instance);
    fd.all_outer() = true;
  }
  ...
  for (int process_interior = 0; process_interior <= 1; ++process_interior) {
    for (auto &bound : face->children) {
      ...
      if (is_interior == !process_interior)
        continue;
      ...
      if (faceset_helper_ && bound->is_polyhedron()) {
        if (!faceset_helper_->wire(bound, wire)) {
          Logger::Message(Logger::LOG_WARNING,
                          "Face boundary loop not included", bound->instance);
          continue;
        }
      } else if (!convert(bound, wire)) {
        Logger::Message(Logger::LOG_ERROR,
                        "Failed to process face boundary loop",
                        bound->instance);
        return false;
      }
      ...
      wire_tolerance_settings settings{
          ...
          !settings_.get<settings::NoWireIntersectionTolerance>().get(), 0.,
      ...
      wire_senses.Bind(wire.Oriented(TopAbs_FORWARD),
                       same_sense ? TopAbs_FORWARD : TopAbs_REVERSED);
  ...
  if (fd.surface().IsNull()) {
    ...
    if (is_polyhedron(wire)) {
      ...
      if (count == 3) {
        ...
        double _, __;
        Handle(Geom_Line) c1 =
            Handle(Geom_Line)::DownCast(BRep_Tool::Curve(edges[0], _, __));
        Handle(Geom_Line) c2 =
            Handle(Geom_Line)::DownCast(BRep_Tool::Curve(edges[1], _, __));
  ...
  if (fd.surface().IsNull()) {
    ...
    Logger::Message(Logger::LOG_WARNING,
                    "Triangulating face boundaries for face", face->instance);
    ...
    if (mf.IsDone()) {
      ...
      if (std::distance(fd.inner_wires().first, fd.inner_wires().second)) {
        ...
        for (auto it = fd.inner_wires().first; it != fd.inner_wires().second;
             ++it) {
          mf.Add(*it);
        }
        ...
        ShapeExtend_DataMapIteratorOfDataMapOfShapeListOfMsg jt(
            msg->MapShape());
        for (; jt.More(); jt.Next()) {
  ...
  if (!fd.surface().IsNull()) {
    ...
    if (fd.surface()->DynamicType() != STANDARD_TYPE(Geom_Plane)) {
      ...
      for (TopTools_ListIteratorOfListOfShape it(face_list); it.More();
           it.Next()) {
        ...
        for (; jt.More(); jt.Next()) {
          ...
          for (; kt.More(); kt.Next()) {
            ...
            if (!reversed_surface && !fd.surface().IsNull() &&
                fd.surface()->IsUPeriodic() &&
                message == "Unknown message invoked with the keyword "
                           "FixAdvFace.FixOrientation.MSG0") {
              Logger::Notice("Detected reversed wire, reattempting with "
                             "reversed basis surface");
      ...
      for (; jt.More(); jt.Next()) {
        ...
        if (wire_map.IsBound(w) && wire_senses.IsBound(w)) {
          ...
          for (; kt.More(); kt.Next()) {
            ...
            wire_senses.Bind(kt.Value(), wire_senses.Find(w) == TopAbs_FORWARD
                                             ? TopAbs_REVERSED
                                             : TopAbs_FORWARD);
          }
        }
...
bool OpenCascadeKernel::convert_impl(const taxonomy::face::ptr face,
  ...
  results.emplace_back(
      ConversionResult(face->instance->as<IfcUtil::IfcBaseEntity>()->id(),
                       new OpenCascadeShape(shape), face->surface_style));
  return true;
}
