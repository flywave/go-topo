...
// Branchless slab method. Note that this can still be optimised further by
// batching boxes. From Tavian Barnes - MIT License
...
bool intersectRayTriangle(const gp_Vec &orig, const gp_Vec &dir,
                          const gp_Vec &vert0, const gp_Vec &vert1,
                          const gp_Vec &vert2, Standard_Real &at,
                          Standard_Real &au, Standard_Real &av, bool cull,
                          float enlarge) {
...
// With minor modifications to use gp_Vec type.
float distanceTriangleTriangleSquared(gp_Vec &cp, gp_Vec &cq,
  ...
  if (Snl > 1e-15f) {
    const std::array<double, 3> Tp = {
        (p[0] - q[0]).Dot(Sn), (p[0] - q[1]).Dot(Sn), (p[0] - q[2]).Dot(Sn)};
  ...
  if (Tnl > 1e-15f) {
    const std::array<double, 3> Sp = {
        (q[0] - p[0]).Dot(Tn), (q[0] - p[1]).Dot(Tn), (q[0] - p[2]).Dot(Tn)};
...
struct Interval {
  ...
  Interval()
      : min(FLT_MAX), max(-FLT_MAX), minPoint(gp_Vec(NAN, NAN, NAN)),
        maxPoint(gp_Vec(NAN, NAN, NAN)) {}
...
// https://github.com/NVIDIA-Omniverse/PhysX/blob/main/physx/source/geomutils/src/intersection/GuIntersectionTriangleTriangle.cpp
// With minor modifications to use gp_Vec type.
static Interval computeInterval(Standard_Real distanceA,
                                Standard_Real distanceB,
                                Standard_Real distanceC, const gp_Vec &a,
                                const gp_Vec &b, const gp_Vec &c,
                                const gp_Vec &dir) {
  Interval i;
  ...
  if (bA != bB) {
    const gp_Vec p = (distanceA / (distanceA + distanceB)) * b +
                     (distanceB / (distanceA + distanceB)) * a;
    i.include(dir.Dot(p), p);
  }
  if (bA != bC) {
    const gp_Vec p = (distanceA / (distanceA + distanceC)) * c +
                     (distanceC / (distanceA + distanceC)) * a;
    i.include(dir.Dot(p), p);
  }
  if (bB != bC) {
    const gp_Vec p = (distanceB / (distanceB + distanceC)) * c +
                     (distanceC / (distanceB + distanceC)) * b;
    i.include(dir.Dot(p), p);
  }
...
// https://github.com/NVIDIA-Omniverse/PhysX/blob/main/physx/source/geomutils/src/intersection/GuIntersectionTriangleTriangle.cpp
// With minor modifications to use gp_Vec type.
Standard_Real orient2d(const gp_Vec &a, const gp_Vec &b, const gp_Vec &c,
                       PxU32 x, PxU32 y) {
  return (a.Coord(y) - c.Coord(y)) * (b.Coord(x) - c.Coord(x)) -
         (a.Coord(x) - c.Coord(x)) * (b.Coord(y) - c.Coord(y));
}
...
// https://github.com/NVIDIA-Omniverse/PhysX/blob/main/physx/source/geomutils/src/intersection/GuIntersectionTriangleTriangle.cpp
// With minor modifications to use gp_Vec type.
Standard_Real pointInTriangle(const gp_Vec &a, const gp_Vec &b, const gp_Vec &c,
                              const gp_Vec &point, PxU32 x, PxU32 y) {
...
// https://github.com/NVIDIA-Omniverse/PhysX/blob/main/physx/source/geomutils/src/intersection/GuIntersectionTriangleTriangle.cpp
// With minor modifications to use gp_Vec type.
Standard_Real linesIntersect(const gp_Vec &startA, const gp_Vec &endA,
                             const gp_Vec &startB, const gp_Vec &endB, PxU32 x,
                             PxU32 y) {
...
void getProjectionIndices(gp_Vec normal, PxU32 &x, PxU32 &y) {
  normal.SetCoord(std::abs(normal.X()), std::abs(normal.Y()),
                  std::abs(normal.Z()));
...
// https://github.com/NVIDIA-Omniverse/PhysX/blob/main/physx/source/geomutils/src/intersection/GuIntersectionTriangleTriangle.cpp
// With minor modifications to use gp_Vec type.
bool trianglesIntersectCoplanar(const gp_Vec &p1_n, const gp_Vec &a1,
                                const gp_Vec &b1, const gp_Vec &c1,
                                const gp_Vec &a2, const gp_Vec &b2,
                                const gp_Vec &c2) {
  PxU32 x = 0;
  PxU32 y = 0;
  ...
  // A bit of the computations done inside the following functions could be
  // shared but it's kept simple since the difference is not very big and the
  // coplanar case is not expected to be the most common case
...
bool trianglesIntersect(const gp_Vec &a1, const gp_Vec &b1, const gp_Vec &c1,
                        const gp_Vec &a2, const gp_Vec &b2,
                        const gp_Vec &c2 /*, Segment* intersection*/,
                        gp_Vec &int1, gp_Vec &int2, bool ignoreCoplanar) {
  ...
  if (std::abs(p1ToA) < tolerance && std::abs(p1ToB) < tolerance &&
    ...
    return ignoreCoplanar
               ? false
               : trianglesIntersectCoplanar(p1_n, a1, b1, c1, a2, b2,
                                            c2); // Coplanar triangles

  if ((p1ToA > 0) == (p1ToB > 0) && (p1ToA > 0) == (p1ToC > 0))
    return false; // All points of triangle 2 on same side of triangle 1 -> no
                  // intersection
  ...
  if ((p2ToA > 0) == (p2ToB > 0) && (p2ToA > 0) == (p2ToC > 0))
    return false; // All points of triangle 1 on same side of triangle 2 -> no
                  // intersection
