...
#include <ifcgeom/kernels/IfcGeomTree.h>
...
void IfcGeom::util::copy_operand(const TopTools_ListOfShape &l,
...
double IfcGeom::util::min_vertex_edge_distance(const TopoDS_Shape &a,
                                               double min_search,
                                               double max_search) {
...
double IfcGeom::util::min_face_face_distance(const TopoDS_Shape &a,
                                             double max_search) {
  /*
  ...
  */
  ...
  for (int i = 1; i <= faces.Extent(); ++i) {
    if (BRep_Tool::Surface(TopoDS::Face(faces(i)))->DynamicType() ==
  ...
  for (int j = 1; j <= faces.Extent(); ++j) {
    ...
    for (; it != face_idxs.end(); ++it) {
      ...
      if (p0->Position().IsCoplanar(p1->Position(), max_search,
                                    asin(max_search))) {
        pgen.reset();
        ...
        while (pgen(test)) {
          ...
          // nb: TopAbs_ON is explicitly not considered to prevent matching
          // adjacent faces with similar orientations.
          if (cls.Perform(gp_Pnt2d(u, v)) == TopAbs_IN) {
            gp_Pnt test2;
            p1->D0(u, v, test2);
            double w = std::abs(gp_Vec(p1->Position().Direction().XYZ())
                                    .Dot(test2.XYZ() - test.XYZ()));
            if (w < M) {
              M = w;
            }
          }
...
int IfcGeom::util::bounding_box_overlap(double p, const TopoDS_Shape &a,
                                        const TopTools_ListOfShape &b,
                                        TopTools_ListOfShape &c) {
  int N = 0;
...
bool IfcGeom::util::is_subset(const TopTools_IndexedMapOfShape &lhs,
                              const TopTools_IndexedMapOfShape &rhs) {
...
bool IfcGeom::util::is_extrusion(const gp_Vec &v, const TopoDS_Shape &s,
                                 TopoDS_Face &base,
  ...
  for (TopTools_ListIteratorOfListOfShape it(parallel); it.More(); it.Next()) {
    TopoDS_Vertex v01[2];
    ...
    bool sets_equal = v_ortho_faces.Size() == ortho_faces.Size() &&
                      is_subset(v_ortho_faces, ortho_faces);
    if (!sets_equal) {
      return false;
    }
  }
...
int IfcGeom::util::eliminate_narrow_operands(double prec,
                                             const TopTools_ListOfShape &bs,
                                             TopTools_ListOfShape &c) {
  int N = 0;
...
int IfcGeom::util::eliminate_touching_operands(double prec,
...
bool IfcGeom::util::boolean_subtraction_2d_using_builder(
    const TopoDS_Shape &a_input, const TopTools_ListOfShape &b_input,
    TopoDS_Shape &result, double eps) {
  ...
  for (; it.More(); it.Next(), ++shape_index) {
    ...
    for (; exp.More(); exp.Next(), ++edge_index) {
      ...
      for (auto &i : edge_tree.select_box(b)) {
        ...
        GeomAPI_ExtremaCurveCurve ecc(BRep_Tool::Curve(e0, u11, u12),
                                      BRep_Tool::Curve(e1, u21, u22));

        if (!ecc.Extrema().IsParallel() && ecc.NbExtrema() >= 1) {
          // @todo: extend this to work in case of multiple extrema and curved
          // segments.
          ...
          for (int i = 1; i <= ecc.NbExtrema(); ++i) {
            ...
            // #3616 Only take into account orthogonal distance between closest
            // points on curve to see whether inside tolerance. Current DY is
            // hardcoded. The sensible default for walls.
            gp_Vec vec(p1, p2);
            ...
            if (unbounded_intersects) {
              ...
              /// @todo: tfk: probably need different thresholds on non-linear
              /// curves
              u11 -= eps;
              u12 += eps;
              u21 -= eps;
              ...
              if (u11 < U1 && U1 < u12 && u21 < U2 && U2 < u22) {
                // Edge curves belonging to different operands intersect, don't
                // process using builder.
  ...
  // Only inner wires are considered that are directly contained in the outer
  // wire Redundant subtractions are eliminated.
  ...
  for (auto &w : wires) {
    ...
    sass.push_back(std::make_unique<ShapeAnalysis_Surface>(
        BRep_Tool::Surface(wire_faces.back())));
  }
...
bool IfcGeom::util::boolean_operation(const boolean_settings &settings,
                                      const TopoDS_Shape &a_input,
                                      const TopTools_ListOfShape &b_input,
                                      BOPAlgo_Operation op,
                                      TopoDS_Shape &result, double fuzziness) {
  ...
  if (debug) {
    ...
    std::stringstream ss;
    ss << "bool-" << std::this_thread::get_id() << "-"
       << (operation_counter_++);
  ...
  */
  ...
  if (do_unify) {
    ...
    Logger::Message(Logger::LOG_DEBUG,
                    "Simplified operand A from "s +
                        std::to_string(count(a_input, TopAbs_FACE)) + " to "s +
  ...
  if (op == BOPAlgo_CUT) {
    ...
    if (do_subtraction_eliminate_disjoint_bbox) {
      ...
      if (N) {
        Logger::Notice("Eliminated " + std::to_string(N) +
                       " disjoint operands");
        std::swap(b, b_tmp);
      }
    }
  ...
  if (!is_2d && Logger::LOG_NOTICE >= Logger::Verbosity()) {
    ...
    if (!a.IsNull()) {
      Logger::Notice("Operand A is " + (is_manifold(a) ? ""s : "non-"s) +
                     "manifold");
    }

    TopTools_ListIteratorOfListOfShape it(b);
    for (int i = 0; it.More(); it.Next(), ++i) {
      Logger::Notice("Operand B " + std::to_string(i) + " is " +
                     (is_manifold(it.Value()) ? ""s : "non-"s) + "manifold");
    }
  }
...

  {
    ...
    TopTools_ListIteratorOfListOfShape it(b);
    for (; it.More(); it.Next()) {
      d = min_vertex_edge_distance(it.Value(), settings.precision,
                                   min_length_orig);
      if (d < min_length_orig) {
        min_length_orig = d;
      }
  ...
  const bool allow_retry =
      new_fuzziness - 1e-15 <= settings.precision * 10000. &&
      new_fuzziness < min_length_orig;
  ...
  if (debug) {
    ...
    for (int i = 0; i < 2; ++i) {
      ...
      for (int j = 0; it.More(); it.Next(), ++j) {
        std::string fn = debug_identifier + "-" + operand_names[i] + "-" +
                         std::to_string(j) + ".brep";
    ...
    if (is_extrusion_a) {
      ...
      for (int nb = 1; it.More(); it.Next(), ++nb) {
        ...
        bool is_extrusion_b;
        {
          ...
          is_extrusion_b =
              is_extrusion(gp::DY(), it.Value(), b_face, b_interval);
        }

        if (is_extrusion_b) {
          Logger::Notice("Operand B " + std::to_string(nb) + "/" +
                         std::to_string(b.Extent()) + " is an extrusion");

          if (b_interval.first < a_interval.first + (fuzz * 100.) &&
              b_interval.second > a_interval.second - (fuzz * 100.)) {
            Logger::Notice("Operand B creates a through hole");

            // Align b with a operand
            gp_Trsf trsf;
            trsf.SetTranslation(gp_Vec(gp::DY()) *
                                (a_interval.first - b_interval.first));
      ...
      if (b_faces.Extent()) {
        ...
        bool boolean_op_2d_success;
        {
          ...
          boolean_op_2d_success = boolean_subtraction_2d_using_builder(
              a_face, b_faces, face_result, fuzziness);
        }
        ...
        if (!boolean_op_2d_success) {
          ...
          boolean_op_2d_success = boolean_operation(settings, a_face, b_faces,
        ...
        if (boolean_op_2d_success) {
          ...
          BRepPrimAPI_MakePrism mp(face_result,
                                   gp_Vec(gp::DY()) *
                                       (a_interval.second - a_interval.first));
          if (mp.IsDone()) {
            if (b_remainder_3d.Extent()) {
              Logger::Notice(std::to_string(b_remainder_3d.Extent()) +
                             " operands remaining to process in 3D");
              b = b_remainder_3d;
              s1s.Clear();
              s1s.Append(mp.Shape());
            } else {
            ...
            Logger::Notice(
                "Failed to extrude 2D boolean result. Retrying in 3D.");
          }
        } else {
          Logger::Notice(
              "Failed to perform 2D boolean operation. Retrying in 3D.");
        }
      } else {
        Logger::Notice("No second operands can be processed as 2D inner "
                       "bounds. Retrying in 3D.");
      }
    }
  }
  ...
  if (builder->IsDone()) {
    if (false && builder->DSFiller()->HasWarning(
                     STANDARD_TYPE(BOPAlgo_AlertAcquiredSelfIntersection))) {
      Logger::Notice("Builder reports self-intersection in output");
      success = false;

      /*
      const auto& ws =
      builder->DSFiller()->GetReport()->GetAlerts(Message_Warning); for (const
      auto& w : ws) { if (w->DynamicType() ==
      STANDARD_TYPE(BOPAlgo_AlertAcquiredSelfIntersection)) { const auto& x =
      Handle(BOPAlgo_AlertAcquiredSelfIntersection)::DownCast(w)->GetShape();
                      BRepTools::Write(x, "debug_x.brep");
                      BRepTools::Write(*builder, "debug_r.brep");
              }
      }
      */
    } else if (builder->DSFiller()->HasWarning(
                   STANDARD_TYPE(BOPAlgo_AlertBadPositioning)) &&
               !TopoDS_Iterator(*builder).More()) {
      ...
      if (success) {
        ...
        if (!success) {
          ...
          // An exemption for the requirement to be manifold: When the cut
          // operands have overlapping edge belonging to faces that do not
          // overlap.
          bool operands_nonmanifold = false;
          if (op == BOPAlgo_CUT) {
            TopTools_IndexedMapOfShape edges;
            TopTools_IndexedDataMapOfShapeListOfShape map;
            for (TopTools_ListIteratorOfListOfShape it2(b); it2.More();
                 it2.Next()) {
              auto &bb = it2.Value();
            ...
            for (int i = 1; i <= edges.Extent(); ++i) {
              ...
              for (int j : ii) {
                if (j != i) {
                  ...
                  double f = fuzziness;
                  bool edges_overlapping =
                      sae.CheckOverlapping(ei, ej, f, 0.) ||
                  ...
                  if (edges_overlapping) {
                    ...
                    bool overlap = false;
                    for (TopTools_ListIteratorOfListOfShape it4(faces_i);
                         it4.More(); it4.Next()) {
                      auto &fi = it4.Value();
                      for (TopTools_ListIteratorOfListOfShape it2(faces_j);
                           it2.More(); it2.Next()) {
                        auto &fj = it2.Value();
        ...
        if (success) {
          ...
          if (op == BOPAlgo_CUT) {
            ...
            for (TopExp_Explorer exp(a, TopAbs_SHELL); exp.More(); exp.Next()) {
              if (!exp.Current().Closed()) {
                // This 'face addition check' is only done when the first
                // operand contains open shells (which was initially the aim of
                // this check see #1472). Later in #1914 we found that the logic
                // to apply openings in groups of similar edge lengths can
                // create a situation of inner voids, which trigger a false
                // positive in this check. This could have also been solved
                // below by checking whether the opening(s) are included as a
                // unmodified (interior) shell within a solid of multiple
                // shells. Checking for open shells in first operand was quicker
                // and more straightforward. The question still is whether in
                // cases like #1472 we need to first try the boolean union as
                // solid/solid interference to trigger this case or whether we
                // can immediately proceed to a face/ solid operation.
            ...
            if (has_open_shells) {
              ...
              for (TopExp_Explorer exp(a, TopAbs_FACE); exp.More();
                   exp.Next()) {
          ...
          if (op == BOPAlgo_CUT && has_open_shells &&
              all_faces_included_in_result &&
              result_n_faces > first_op_n_faces) {
            success = false;
            Logger::Notice("Boolean result discarded because subtractions "
                           "results in only the addition of faces");
          } else {
            // when there are edges or vertex-edge distances close to the used
            // fuzziness, the output is not trusted and the operation is
            // attempted with a higher fuzziness.
            int reason = 0;
            double v;
...

            {
              ...
              if ((v = min_vertex_edge_distance(r, settings.precision,
                                                fuzziness * 3.)) <
                  fuzziness * 3.) {
                reason = 1;
                success = false;
...

            {
              ...
              if ((v = min_face_face_distance(r, 1.e-4)) < 1.e-4) {
                // #2095 Check if this distance wasn't already realized in the
                // input first operand.
          ...
          skip_further_checks:
            if (!success) {
              static const char *const reason_strings[] = {
                  "edge length", "vertex-edge", "face-face"};
              std::stringstream str;
              str << "Boolean operation result failing "
                  << reason_strings[reason]
                  << " interference check, with fuzziness " << fuzziness
                  << " with length " << v;
              Logger::Notice(str.str());
            }
          }
  ...
  if (!success) {
    if (allow_retry) {
      ...
      Logger::Notice(
          "No longer attempting boolean operation with higher fuzziness");
    }
  }
...
TopoDS_Shape
IfcGeom::util::ensure_fit_for_subtraction(const TopoDS_Shape &shape,
                                          double tol) {
