...
#include <ifcgeom/kernels/layerset.h>
...
bool split(const TopoDS_Shape &input, const TopTools_ListOfShape &operands,
           double eps, std::vector<TopoDS_Shape> &slices) {
  if (operands.Extent() < 2) {
    // Needs to have at least two cutting surfaces for the ordering based on
    // surface containment to work.
    return false;
  }
  ...
  if (!split.IsDone()) {
    ...
    int i = 1;
    for (TopTools_ListIteratorOfListOfShape it(operands); it.More();
         it.Next(), ++i) {
      TopExp_Explorer exp(it.Value(), TopAbs_FACE);
      for (; exp.More(); exp.Next()) {
        surfaces.insert(std::make_pair(
            BRep_Tool::Surface(TopoDS::Face(exp.Current())).get(), i));
      }
    }
    ...
    // Sometimes there is more nesting of compounds, so when we find a single
    // compound we again try to explode it into a list.
    if (subs.size() == 1 && (subs.front().ShapeType() == TopAbs_COMPSOLID ||
                             subs.front().ShapeType() == TopAbs_COMPOUND)) {
      auto s = subs.front();
      subs.clear();
      subshapes(s, subs);
    }
...
#else
bool split(const TopoDS_Shape &input, const TopTools_ListOfShape &operands,
           double, std::vector<TopoDS_Shape> &slices) {
  ...
  for (; it.More(); it.Next()) {
    ...
    if ((s.ShapeType() == TopAbs_FACE &&
         IfcGeom::util::split_solid_by_surface(i, surf, a, b)) ||
        (s.ShapeType() == TopAbs_SHELL &&
         IfcGeom::util::split_solid_by_shell(i, s, a, b))) {
      slices.push_back(b);
      i = a;
    } else {
      return false;
...
bool IfcGeom::util::apply_folded_layerset(
    const ConversionResults &items,
    const std::vector<std::vector<Handle_Geom_Surface>> &surfaces,
    const std::vector<ifcopenshell::geometry::taxonomy::style::ptr> &styles,
    ConversionResults &result, double tol) {
  Bnd_Box bb;
  TopoDS_Shape input;
  flatten_shape_list(items, input, false, false, tol);

  typedef std::vector<std::vector<Handle_Geom_Surface>> folded_surfaces_t;
  typedef std::vector<std::pair<TopoDS_Face, std::pair<gp_Pnt, gp_Pnt>>>
      faces_with_mass_t;

  TopTools_ListOfShape shells;

  for (folded_surfaces_t::const_iterator it = surfaces.begin();
       it != surfaces.end(); ++it) {
    if (it->empty()) {
      continue;
    } else if (it->size() == 1) {
      ...
      for (folded_surfaces_t::value_type::const_iterator jt = it->begin();
           jt != it->end(); ++jt) {
        ...
        TopoDS_Face face =
            BRepBuilderAPI_MakeFace(surface, u1, u2, v1, v2, 1.e-7).Face();
        gp_Pnt p, p1, p2;
        gp_Vec vu, vv, n;
      ...
      for (++jt; jt != solids.end(); ++jt) {
        ...
        TopoDS_Shape Bn =
            BRepPrimAPI_MakeHalfSpace(B, jt->second.second).Solid();
      ...
      for (faces_with_mass_t::const_iterator kt = solids.begin();
           kt != solids.end(); ++kt) {
  ...
  if (shells.Extent() == 0) {
    ...
    for (ConversionResults::const_iterator it = items.begin();
         it != items.end(); ++it) {
      TopoDS_Shape a, b;
      if (split_solid_by_shell(
              std::static_pointer_cast<OpenCascadeShape>(it->Shape())->shape(),
              shells.First(), a, b, tol)) {
        result.push_back(ConversionResult(
            it->ItemId(), it->Placement(), new OpenCascadeShape(b),
            (!!styles[0] ? styles[0] : it->StylePtr())));
        result.push_back(ConversionResult(
            it->ItemId(), it->Placement(), new OpenCascadeShape(a),
            (!!styles[1] ? styles[1] : it->StylePtr())));
      } else {
        continue;
      }
    }
    ...
    for (ConversionResults::const_iterator it = items.begin();
      ...
      if (split(s, shells, tol, slices) && slices.size() == styles.size()) {
        for (size_t i = 0; i < slices.size(); ++i) {
          result.push_back(ConversionResult(
              it->ItemId(), it->Placement(), new OpenCascadeShape(slices[i]),
              (!!styles[i] ? styles[i] : it->StylePtr())));
        }
      } else {
        return false;
      }
...
bool IfcGeom::util::apply_layerset(
    const ConversionResults &items,
    const std::vector<Handle_Geom_Surface> &surfaces,
    const std::vector<ifcopenshell::geometry::taxonomy::style::ptr> &styles,
    ConversionResults &result, double tol) {
  if (surfaces.size() < 3) {
    ...
    /*
    // Determine whether sequence of surfaces is consistent with surface normal,
    so that
    // layer operations are applied in the correct order. This seems to be
    always the case. Bnd_Box bb; for (ConversionResults::const_iterator it =
    items.begin(); it != items.end(); ++it) { BRepBndLib::Add(it->Shape(), bb);
    }
    ...
    */
    ...
    for (ConversionResults::const_iterator it = items.begin();
      ...
      for (unsigned i = 1; i < surfaces.size() - 1; ++i) {
        ...
        TopoDS_Face face =
            BRepBuilderAPI_MakeFace(surfaces[i], u1, u2, v1, v2, 1.e-7).Face();

        operands.Append(face);
      }

      /*
      // enable this is you want to see how IfcOpenShell has placed the layer
      surfaces for (auto& x : operands) {
              result.push_back(ConversionResult(it->ItemId(), it->Placement(),
      x, nullptr));
      }
      */
...
bool IfcGeom::util::split_solid_by_surface(const TopoDS_Shape &input,
                                           const Handle_Geom_Surface &surface,
                                           TopoDS_Shape &front,
                                           TopoDS_Shape &back, double tol) {
...
bool IfcGeom::util::split_solid_by_shell(const TopoDS_Shape &input,
  ...
  BOPAlgo_PaveFiller filler(
      new NCollection_IncAllocator); // TODO: Does this need to be freed?
  ...
  if (is_null[0] || is_null[1]) {
    Logger::Message(Logger::LOG_ERROR,
                    "Null result obtained from layerset slicing");
    if (is_null[0] && is_null[1]) {
      return false;
    }
  }
