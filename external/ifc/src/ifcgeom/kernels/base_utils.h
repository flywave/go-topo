...
gp_Trsf combine_offset_and_rotation(const gp_Vec &offset,
                                    const gp_Quaternion &rotation);

bool is_nested_compound_of_solid(const TopoDS_Shape &s, int depth = 0);

bool create_solid_from_compound(const TopoDS_Shape &compound,
                                TopoDS_Shape &solid, double tol);
bool shape_to_face_list(const TopoDS_Shape &s, TopTools_ListOfShape &li);
bool create_solid_from_faces(const TopTools_ListOfShape &face_list,
                             TopoDS_Shape &solid, double tol,
                             bool force_sewing = false);
...
bool fit_halfspace(const TopoDS_Shape &a, const TopoDS_Shape &b,
                   TopoDS_Shape &box, double &height, double tol);
const Handle_Geom_Curve intersect(const Handle_Geom_Surface &,
                                  const Handle_Geom_Surface &);
const Handle_Geom_Curve intersect(const Handle_Geom_Surface &,
                                  const TopoDS_Face &);
const Handle_Geom_Curve intersect(const TopoDS_Face &,
                                  const Handle_Geom_Surface &);
...
bool intersect(const Handle_Geom_Curve &, const TopoDS_Shape &,
               std::vector<gp_Pnt> &);
bool intersect(
    const Handle_Geom_Surface &, const TopoDS_Shape &,
    std::vector<std::pair<Handle_Geom_Surface, Handle_Geom_Curve>> &);
bool closest(const gp_Pnt &, const std::vector<gp_Pnt> &, gp_Pnt &);
bool project(const Handle_Geom_Curve &, const gp_Pnt &, gp_Pnt &p, double &u,
             double &d);
bool project(const Handle_Geom_Surface &, const TopoDS_Shape &, double &u1,
             double &v1, double &u2, double &v2, double widen = 0.1);

double shape_volume(const TopoDS_Shape &s);
double face_area(const TopoDS_Face &f);

TopoDS_Shape
apply_transformation(const TopoDS_Shape &,
                     const ifcopenshell::geometry::taxonomy::matrix4 &t);
...
bool flatten_shape_list(const IfcGeom::ConversionResults &shapes,
                        TopoDS_Shape &result, bool fuse, bool create_shell,
                        double tol);
