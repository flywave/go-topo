...
bool OpenCascadeKernel::convert(const taxonomy::extrusion::ptr extrusion,
                                TopoDS_Shape &shape) {
  const double &height = extrusion->depth;

  if (height < settings_.get<settings::Precision>().get()) {
    Logger::Error("Non-positive extrusion height encountered for:",
                  extrusion->instance);
    return false;
  }
...
bool OpenCascadeKernel::convert_impl(const taxonomy::extrusion::ptr extrusion,
  ...
  results.emplace_back(
      ConversionResult(extrusion->instance->as<IfcUtil::IfcBaseEntity>()->id(),
                       extrusion->matrix, new OpenCascadeShape(shape),
