...
std::vector<double> ifcopenshell::geometry::helmert_curve_point(double A0,
                                                                double A1,
                                                                double A2,
                                                                double s) {
...
struct functor_fn_evaluator : public fn_evaluator {
  functor_fn_evaluator(taxonomy::functor_item::const_ptr fn,
                       const ifcopenshell::geometry::Settings &settings)
      : fn_evaluator(settings), fn_(fn) {}

  fn_evaluator *clone() const override {
    return new functor_fn_evaluator(*this);
  }
  double start() const override { return fn_->start(); }
  double end() const override { return fn_->end(); }

  Eigen::Matrix4d evaluate(double u) const override { return (*fn_)(u); }

  taxonomy::functor_item::const_ptr fn_;
};

struct piecewise_fn_evaluator : public fn_evaluator {
  piecewise_fn_evaluator(taxonomy::piecewise_function::const_ptr fn,
                         const ifcopenshell::geometry::Settings &settings)
      : fn_evaluator(settings), fn_(fn) {}

  fn_evaluator *clone() const override {
    return new piecewise_fn_evaluator(*this);
  }
  double start() const override { return fn_->start(); }
  double end() const override { return fn_->end(); }
  ...
  Eigen::Matrix4d evaluate(double u) const override {
    ...
    if (current_span_fn_ == nullptr ||
        (u < current_span_start_ || current_span_end_ < u)) {
      // there isn't a current span or u is outside the range of the current
      // span get a new "current span"
      std::tie(current_span_start_, current_span_end_, current_span_fn_) =
          get_span(u);
    }
  ...
  std::tuple<double, double, taxonomy::function_item::const_ptr>
  get_span(double u) const {
    ...
    for (auto &fn : fn_->spans()) {
      ...
      auto tolerance =
          settings_.get<ifcopenshell::geometry::settings::Precision>().get();
...
struct gradient_fn_evaluator : public fn_evaluator {
  gradient_fn_evaluator(taxonomy::gradient_function::const_ptr fn,
                        const ifcopenshell::geometry::Settings &settings)
      : fn_evaluator(settings), fn_(fn),
  ...
  fn_evaluator *clone() const override {
    return new gradient_fn_evaluator(*this);
  }
  double start() const override { return fn_->start(); }
  double end() const override { return fn_->end(); }
  ...
  Eigen::Matrix4d evaluate(double u) const override {
    ...
    // curvature is stored in row 3 - capture it and remove it from the xy and
    // uz matrices so the matrix operations (ie multiplication) works correct.y
    ...
    uz(0, 3) = 0.0; // x is distance along. zero it out so it doesn't add to the
                    // x from horizontal
    uz.col(1).swap(uz.col(2)); // uz is 2D in distance along - y plane, swap y
                               // and z so elevations become z
...
struct cant_fn_evaluator : public fn_evaluator {
  cant_fn_evaluator(taxonomy::cant_function::const_ptr fn,
                    const ifcopenshell::geometry::Settings &settings)
      : fn_evaluator(settings), fn_(fn),
  ...
  Eigen::Matrix4d evaluate(double u) const override {
    ...
    // curvature is stored in row 3 - capture it and remove it from the xy and
    // uz matrices so the matrix operations (ie multiplication) works correctly
    ...
    // cant_curvature is cant alone. this needs to be combined with gradient in
    // column 3
...
struct offset_fn_evaluator : public fn_evaluator {
  offset_fn_evaluator(taxonomy::offset_function::const_ptr fn,
                      const ifcopenshell::geometry::Settings &settings)
      : fn_evaluator(settings), fn_(fn),
  ...
  fn_evaluator *clone() const override {
    return new offset_fn_evaluator(*this);
  }
  double start() const override { return fn_->start(); }
  double end() const override { return fn_->end(); }
...
function_item_evaluator::function_item_evaluator(
    const ifcopenshell::geometry::Settings &settings,
    taxonomy::function_item::const_ptr fn) {
  auto kind = fn->kind();
  if (kind == taxonomy::FUNCTOR_ITEM) {
    fn_evaluator_ = new functor_fn_evaluator(
        std::dynamic_pointer_cast<const taxonomy::functor_item>(fn), settings);
  } else if (kind == taxonomy::PIECEWISE_FUNCTION) {
    fn_evaluator_ = new piecewise_fn_evaluator(
        std::dynamic_pointer_cast<const taxonomy::piecewise_function>(fn),
        settings);
  } else if (kind == taxonomy::GRADIENT_FUNCTION) {
    fn_evaluator_ = new gradient_fn_evaluator(
        std::dynamic_pointer_cast<const taxonomy::gradient_function>(fn),
        settings);
  } else if (kind == taxonomy::CANT_FUNCTION) {
    fn_evaluator_ = new cant_fn_evaluator(
        std::dynamic_pointer_cast<const taxonomy::cant_function>(fn), settings);
  } else if (kind == taxonomy::OFFSET_FUNCTION) {
    fn_evaluator_ = new offset_fn_evaluator(
        std::dynamic_pointer_cast<const taxonomy::offset_function>(fn),
        settings);
  } else {
...
function_item_evaluator::function_item_evaluator(
    const function_item_evaluator &other) {
  fn_evaluator_ = other.fn_evaluator_->clone();
  eval_points_ = other.eval_points_;
}

function_item_evaluator::~function_item_evaluator() { delete fn_evaluator_; }

std::vector<double> function_item_evaluator::evaluation_points() const {
  if (!eval_points_.is_initialized()) {
    double curve_length = fn_evaluator_->length();

    auto param_type =
        fn_evaluator_->settings_
            .get<ifcopenshell::geometry::settings::FunctionStepType>()
            .get();
    auto param = fn_evaluator_->settings_
                     .get<ifcopenshell::geometry::settings::FunctionStepParam>()
                     .get();
    unsigned num_steps = 0;
    if (param_type ==
        ifcopenshell::geometry::settings::FunctionStepMethod::MAXSTEPSIZE) {
    ...
    eval_points_ = evaluation_points(fn_evaluator_->start(),
                                     fn_evaluator_->end(), num_steps);
  }
  return *eval_points_;
}

std::vector<double>
function_item_evaluator::evaluation_points(double ustart, double uend,
...
taxonomy::item::ptr function_item_evaluator::evaluate(double ustart,
                                                      double uend,
                                                      unsigned nsteps) const {
...
taxonomy::item::ptr
function_item_evaluator::evaluate(const std::vector<double> &dist) const {
  std::vector<taxonomy::point3::ptr> polygon;
  polygon.reserve(dist.size());
  for (auto &u : dist) {
    Eigen::Matrix4d m = evaluate(u);
    polygon.push_back(
        taxonomy::make<taxonomy::point3>(m(0, 3), m(1, 3), m(2, 3)));
  }
...
Eigen::Matrix4d function_item_evaluator::evaluate(double u) const {
  ...
  if (!fn_evaluator_->settings_
           .get<ifcopenshell::geometry::settings::ComputeCurvature>()
           .get()) {
