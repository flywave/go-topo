...
namespace IfcGeom {
/// The filter function (free or member function) or function object (use
/// boost::ref() to reference to it) should return true if the geometry for the
/// product is wanted to be included in the output.
...
struct filter {
  ...
  filter(bool incl, bool trav, bool trav_openings = false)
      : include(incl), traverse(trav), traverse_openings(trav_openings) {}
  /// Should the product be included (true) or excluded (false).
  bool include;
  /// If traversal requested, traverse to the parents to see if they satisfy the
  /// criteria. E.g. we might be looking for children of a storey named "Level
  /// 20", or children of entities that have no representation, e.g.
  /// IfcCurtainWall.
  bool traverse;
  ...
  bool traverse_match(IfcUtil::IfcBaseEntity *prod,
                      const filter_t &pred) const {
    ...
    static auto mapping =
        ifcopenshell::geometry::impl::mapping_implementations().construct(
            prod->file_, s);
    while ((parent = mapping->get_decomposing_entity(
                current, traverse_openings)) != nullptr) {
      if (pred(parent)) {
        return true;
      }
      current = parent;
...
struct wildcard_filter : public filter {
  ...
  wildcard_filter(bool include, bool traverse,
  ...
  static bool match_values(const std::set<boost::regex> &values,
...
struct attribute_filter : public wildcard_filter {
  ...
  bool operator()(IfcUtil::IfcBaseEntity *prod) const {
    return filter::match(
        prod, std::bind(&attribute_filter::match, this, std::placeholders::_1));
  }
...
struct layer_filter : public wildcard_filter {
  ...
  layer_filter() {}
  layer_filter(bool include, bool traverse,
               const std::set<std::string> &patterns)
  ...
  bool match(IfcUtil::IfcBaseEntity *prod) const {
    ...
    return std::find_if(layers.begin(), layers.end(),
                        wildcards_match(values)) != layers.end();
  }

  bool operator()(IfcUtil::IfcBaseEntity *prod) const {
    return filter::match(
        prod, std::bind(&layer_filter::match, this, std::placeholders::_1));
  }
...
struct entity_filter : public filter {
  ...
  entity_filter(bool include, bool traverse,
                const std::set<std::string> &entity_names)
      : filter(include, traverse), entity_names(entity_names) {}
  ...
  bool operator()(IfcUtil::IfcBaseEntity *prod) const {
    return filter::match(
        prod, std::bind(&entity_filter::match, this, std::placeholders::_1));
  }
...
struct instance_id_filter : public filter {
  ...
  instance_id_filter() {}
  instance_id_filter(bool include, bool traverse,
                     const std::set<int> &instance_ids)
      : filter(include, traverse), instance_ids_(instance_ids) {}
  ...
  bool operator()(IfcUtil::IfcBaseEntity *prod) const {
    return filter::match(prod, std::bind(&instance_id_filter::match, this,
                                         std::placeholders::_1));
  }
