...
/// Returns (x,y,theta) at L/2. The results are in a vector so they can be
/// returned to python
std::vector<double> helmert_curve_point(double A0, double A1, double A2,
                                        double s);

/// @brief Abstract class for evaluating a function_item. This class is
/// specialized for each of the function_item types.
struct fn_evaluator {
  fn_evaluator(const ifcopenshell::geometry::Settings &settings)
      : settings_(settings) {}
...
public:
  function_item_evaluator(const ifcopenshell::geometry::Settings &settings,
                          taxonomy::function_item::const_ptr fn);
  function_item_evaluator(const function_item_evaluator &other);
  ~function_item_evaluator();

  /// @brief returns a vector of "distance along" points where the evaluate
  /// function computes loop points
  ...
  std::vector<double> evaluation_points(double ustart, double uend,
  ...
  taxonomy::item::ptr evaluate(double ustart, double uend,
                               unsigned nsteps) const;

  /// @brief evaluates the function at u
  /// @param u u is constrained to be between start_ and start_+length
  /// @return 4x4 placement matrix. Curvature values for horizontal, vertical,
  /// and vertical + cant are stored in the last row.
...
private:
  ...
  mutable boost::optional<std::vector<double>>
      eval_points_; // cache evaluation points
};
