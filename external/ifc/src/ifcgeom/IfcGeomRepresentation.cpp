...
namespace {
void accumulate(const gp_Ax3 &ax, const gp_Dir &normal, double area,
...
void surface_area_along_direction(double tol, const TopoDS_Shape &s,
                                  const gp_Ax3 &ax, double &along_x,
                                  double &along_y, double &along_z) {
  ...
  for (; exp.More(); exp.Next()) {
    ...
    if (surf->DynamicType() == STANDARD_TYPE(Geom_Plane)) {
      ...
      accumulate(ax, plane->Position().Direction(), area, along_x, along_y,
                 along_z);
    } else {
      ...
      if (!tri.IsNull()) {
        ...
        for (int i = 1; i <= triangles.Length(); ++i) {
          ...
          if (normal_vector.Magnitude() > 1.e-7) {
            ...
            double edge_lengths[3] = {v1.Magnitude(), v2.Magnitude(),
                                      v3.Magnitude()};
            ...
            const double area = 0.25 * sqrt((a + (b + c)) * (c - (a - b)) *
                                            (c + (a - b)) * (a + (b - c)));
...
IfcGeom::Representation::Serialization::Serialization(const BRep &brep)
  ...
  for (auto it = brep.begin(); it != brep.end(); ++it) {
    ...
    if (it->hasStyle()) {
      ...
      sid = it->Style().instance
                ? it->Style().instance->as<IfcUtil::IfcBaseEntity>()->id()
                : -1;
    } else {
  ...
  if (brep.begin() != brep.end()) {
    if (std::dynamic_pointer_cast<ifcopenshell::geometry::OpenCascadeShape>(
            brep.begin()->Shape())) {
...
IfcGeom::Representation::BRep::as_compound(bool force_meters) const {
  ...
  for (auto it = begin(); it != end(); ++it) {
    ...
    if (!force_meters &&
        settings()
            .get<ifcopenshell::geometry::settings::ConvertBackUnits>()
            .get()) {
      gp_Trsf scale;
      scale.SetScaleFactor(
          1.0 /
          settings().get<ifcopenshell::geometry::settings::LengthUnit>().get());
...
bool IfcGeom::Representation::BRep::calculate_surface_area(double &area) const {
  try {
    ...
    for (IfcGeom::ConversionResults::const_iterator it = begin(); it != end();
         ++it) {
      GProp_GProps prop;
    ...
    for (IfcGeom::ConversionResults::const_iterator it = begin(); it != end();
      ...
      if (util::is_manifold(
              *std::static_pointer_cast<
                  ifcopenshell::geometry::OpenCascadeShape>(it->Shape()))) {
        GProp_GProps prop;
...
bool IfcGeom::Representation::BRep::calculate_projected_surface_area(
    const ifcopenshell::geometry::taxonomy::matrix4 &place, double &along_x,
    double &along_y, double &along_z) const {
  try {
    gp_GTrsf trsf;
    ...
    for (IfcGeom::ConversionResults::const_iterator it = begin(); it != end();
         ++it) {
      double x, y, z;
      surface_area_along_direction(
          settings()
              .get<ifcopenshell::geometry::settings::MesherLinearDeflection>()
              .get(),
          *std::static_pointer_cast<ifcopenshell::geometry::OpenCascadeShape>(
              it->Shape()),
          ax, x, y, z);
...
IfcGeom::Representation::Triangulation::Triangulation(const BRep &shape_model)
    : Representation(shape_model.settings(), shape_model.entity(),
                     shape_model.id()),
      weld_offset_(0) {
  for (IfcGeom::ConversionResults::const_iterator iit = shape_model.begin();
       iit != shape_model.end(); ++iit) {

    // Don't weld vertices that belong to different items to prevent
    ...
    if (iit->hasStyle()) {
      auto jt =
          std::find(materials_.begin(), materials_.end(), iit->StylePtr());
    ...
    if (settings()
            .get<ifcopenshell::geometry::settings::ApplyDefaultMaterials>()
            .get() &&
        surface_style_id == -1) {
    ...
    iit->Shape()->Triangulate(settings(), *iit->Placement(), this,
                              iit->ItemId(), surface_style_id);
  }
}
...
std::vector<double> IfcGeom::Representation::Triangulation::box_project_uvs(
    const std::vector<double> &vertices, const std::vector<double> &normals) {
  ...
  for (size_t uv_idx = 0, v_idx = 0;
    ...
    double n_x = normals[v_idx], n_y = normals[v_idx + 1],
           n_z = normals[v_idx + 2];
    double v_x = vertices[v_idx], v_y = vertices[v_idx + 1],
           v_z = vertices[v_idx + 2];
...
int IfcGeom::Representation::Triangulation::addVertex(int item_id,
                                                      int material_index,
                                                      double pX, double pY,
                                                      double pZ) {
  const bool convert =
...
void IfcGeom::Representation::Triangulation::registerEdgeCount(
    int n1, int n2, std::map<std::pair<int, int>, int> &edgecount) {
  const Edge e = Edge((std::min)(n1, n2), (std::max)(n1, n2));
  edgecount[e]++;
}

const IfcGeom::ConversionResultShape *
IfcGeom::Representation::BRep::item(int i) const {
