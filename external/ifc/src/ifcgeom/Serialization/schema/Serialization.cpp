...
#define INCLUDE_PARENT_PARENT_DIR(x)                                           \
  STRINGIFY(../../../ifcparse/x-definitions.h)
#include INCLUDE_PARENT_PARENT_DIR(IfcSchema)
...
template <>
int convert_to_ifc(const TopoDS_Vertex &v, IfcSchema::IfcCartesianPoint *&p,
                   bool advanced) {
...
template <>
int convert_to_ifc(const TopoDS_Vertex &v, IfcSchema::IfcVertex *&vertex,
                   bool advanced) {
...
template <>
int convert_to_ifc(const gp_Ax2 &a, IfcSchema::IfcAxis2Placement3D *&ax,
                   bool advanced) {
  IfcSchema::IfcCartesianPoint *p;
  IfcSchema::IfcDirection *x, *z;
  if (!(convert_to_ifc(a.Location(), p, advanced) &&
        convert_to_ifc(a.Direction(), z, advanced) &&
        convert_to_ifc(a.XDirection(), x, advanced))) {
    ax = 0;
    return 0;
  }
...
namespace {
IfcSchema::IfcKnotType::Value
opencascade_knotspec_to_ifc(GeomAbs_BSplKnotDistribution bspline_knot_spec) {
  IfcSchema::IfcKnotType::Value knot_spec =
      IfcSchema::IfcKnotType::IfcKnotType_UNSPECIFIED;
...
template <>
int convert_to_ifc(const Handle_Geom_Curve &c, IfcSchema::IfcCurve *&curve,
                   bool advanced) {
  if (c->DynamicType() == STANDARD_TYPE(Geom_TrimmedCurve)) {
    ...
    curve = new IfcSchema::IfcEllipse(ax, ellipse->MajorRadius(),
                                      ellipse->MinorRadius());
    ...
    IfcSchema::IfcKnotType::Value knot_spec =
        IfcSchema::IfcKnotType::IfcKnotType_QUASI_UNIFORM_KNOTS;

    IfcSchema::IfcCartesianPoint::list::ptr points(
        new IfcSchema::IfcCartesianPoint::list);
    ...
    curve = new IfcSchema::IfcRationalBSplineCurveWithKnots(
        bezier->Degree(), points,
        IfcSchema::IfcBSplineCurveForm::IfcBSplineCurveForm_UNSPECIFIED,
        bezier->IsClosed() != 0, false, mults, knots, knot_spec, weights);
    ...
    IfcSchema::IfcKnotType::Value knot_spec =
        opencascade_knotspec_to_ifc(bspline->KnotDistribution());
    ...
    bool rational = false;
    for (std::vector<double>::const_iterator it = weights.begin();
         it != weights.end(); ++it) {
      if ((*it) != 1.) {
        rational = true;
        break;
    ...
    if (rational) {
      curve = new IfcSchema::IfcRationalBSplineCurveWithKnots(
          bspline->Degree(), points,
          IfcSchema::IfcBSplineCurveForm::IfcBSplineCurveForm_UNSPECIFIED,
          bspline->IsClosed() != 0, false, mults, knots, knot_spec, weights);
    } else {
      curve = new IfcSchema::IfcBSplineCurveWithKnots(
          bspline->Degree(), points,
          IfcSchema::IfcBSplineCurveForm::IfcBSplineCurveForm_UNSPECIFIED,
          bspline->IsClosed() != 0, false, mults, knots, knot_spec);
    }
...
template <>
int convert_to_ifc(const Handle_Geom_Surface &s,
                   IfcSchema::IfcSurface *&surface, bool advanced) {
  if (s->DynamicType() == STANDARD_TYPE(Geom_Plane)) {
    ...
    Handle_Geom_CylindricalSurface cyl =
        Handle_Geom_CylindricalSurface::DownCast(s);
    IfcSchema::IfcAxis2Placement3D *place;
    /// @todo: Note that the Ax3 is converted to an Ax2 here
    ...
    Handle_Geom_BSplineSurface bspline =
        Handle_Geom_BSplineSurface::DownCast(s);
    ...
    IfcSchema::IfcKnotType::Value knot_spec_u =
        opencascade_knotspec_to_ifc(bspline->UKnotDistribution());
    IfcSchema::IfcKnotType::Value knot_spec_v =
        opencascade_knotspec_to_ifc(bspline->VKnotDistribution());
    ...
    TColStd_Array2OfReal bspline_weights(1, bspline->NbUPoles(), 1,
                                         bspline->NbVPoles());
    ...
    bool rational = false;
    for (std::vector<std::vector<double>>::const_iterator it = weights.begin();
         it != weights.end(); ++it) {
      for (std::vector<double>::const_iterator jt = it->begin();
           jt != it->end(); ++jt) {
        if ((*jt) != 1.) {
          rational = true;
          break;
    ...
    if (rational) {
      surface = new IfcSchema::IfcRationalBSplineSurfaceWithKnots(
          bspline->UDegree(), bspline->VDegree(), points,
          IfcSchema::IfcBSplineSurfaceForm::IfcBSplineSurfaceForm_UNSPECIFIED,
          bspline->IsUClosed() != 0, bspline->IsVClosed() != 0, false, umults,
          vmults, uknots, vknots, knot_spec_u, weights);
    } else {
      surface = new IfcSchema::IfcBSplineSurfaceWithKnots(
          bspline->UDegree(), bspline->VDegree(), points,
          IfcSchema::IfcBSplineSurfaceForm::IfcBSplineSurfaceForm_UNSPECIFIED,
          bspline->IsUClosed() != 0, bspline->IsVClosed() != 0, false, umults,
          vmults, uknots, vknots, knot_spec_u);
    }
...
template <>
int convert_to_ifc(const TopoDS_Edge &e, IfcSchema::IfcCurve *&c,
                   bool advanced) {
  double a, b;
  ...
  IfcSchema::IfcTrimmingSelect::list::ptr trim1(
      new IfcSchema::IfcTrimmingSelect::list);
  IfcSchema::IfcTrimmingSelect::list::ptr trim2(
      new IfcSchema::IfcTrimmingSelect::list);
  trim1->push(new IfcSchema::IfcParameterValue(a));
  trim2->push(new IfcSchema::IfcParameterValue(b));

  c = new IfcSchema::IfcTrimmedCurve(
      base, trim1, trim2, true,
...
template <>
int convert_to_ifc(const TopoDS_Edge &e, IfcSchema::IfcEdge *&edge,
                   bool advanced) {
  double a, b;
  ...
  if (!(convert_to_ifc(v1, vertex1, advanced) &&
        convert_to_ifc(v2, vertex2, advanced))) {
    return 0;
  }
  ...
  if (crv->DynamicType() == STANDARD_TYPE(Geom_Line) && !advanced) {
    ...
    IfcSchema::IfcEdge *edge2 =
        new IfcSchema::IfcEdgeCurve(vertex1, vertex2, curve, true);
...
template <>
int convert_to_ifc(const TopoDS_Wire &wire, IfcSchema::IfcLoop *&loop,
                   bool advanced) {
  bool polygonal = true;
  ...
  if (!polygonal && !advanced) {
    ...
    IfcSchema::IfcOrientedEdge::list::ptr edges(
        new IfcSchema::IfcOrientedEdge::list);
    BRepTools_WireExplorer exp(wire);
    for (; exp.More(); exp.Next()) {
      IfcSchema::IfcEdge *edge;
      // With advanced set to true convert_to_ifc(TopoDS_Edge&) will always
      // create an IfcOrientedEdge
...
template <>
int convert_to_ifc(const TopoDS_Face &f, IfcSchema::IfcFace *&face,
                   bool advanced) {
  ...
  if (is_planar && !advanced) {
    ...
    face = new IfcSchema::IfcAdvancedFace(bounds, surface,
...
int convert_to_ifc(const TopoDS_Shape &s, U *&item, bool advanced) {
  ...
  for (TopExp_Explorer exp(s, TopAbs_FACE); exp.More(); exp.Next()) {
    if (convert_to_ifc(TopoDS::Face(exp.Current()), f, advanced)) {
      ...
      /// Cleanup:
      for (IfcSchema::IfcFace::list::it it = faces->begin(); it != faces->end();
           ++it) {
        aggregate_of_instance::ptr data = IfcParse::traverse(*it)->unique();
        for (aggregate_of_instance::it jt = data->begin(); jt != data->end();
             ++jt) {
          delete *jt;
        }
      }
...
IfcUtil::IfcBaseClass *POSTFIX_SCHEMA(serialise)(const TopoDS_Shape &shape,
                                                 bool advanced) {
  ...
  IfcSchema::IfcRepresentationItem::list::ptr items(
      new IfcSchema::IfcRepresentationItem::list);

  // First check if there is a solid with one or more shells
  for (TopExp_Explorer exp(shape, TopAbs_SOLID); exp.More(); exp.Next()) {
    IfcSchema::IfcClosedShell *outer = 0;
    IfcSchema::IfcClosedShell::list::ptr inner(
        new IfcSchema::IfcClosedShell::list);
    for (TopExp_Explorer exp2(exp.Current(), TopAbs_SHELL); exp2.More();
         exp2.Next()) {
      IfcSchema::IfcClosedShell *shell;
      if (!convert_to_ifc(exp2.Current(), shell, advanced)) {
        return 0;
      }
      /// @todo Are shells always in this order or does Orientation() needs to
      /// be checked? > #4216, no, consider using BRepClass3d::OuterShell()
      if (outer) {
        inner->push(shell);
      } else {
        outer = shell;
    ...
    if (advanced) {
      ...
      /// @todo this is not necessarily correct as the shell is not necessarily
      /// facetted.
      if (inner->size()) {
  ...
  if (items->size() > 0) {
    rep = new IfcSchema::IfcShapeRepresentation(
        0, std::string("Body"),
        advanced ? std::string("AdvancedBrep") : std::string("Brep"), items);
  } else {
    ...
    if (shells->size() > 0) {
      ...
      // If not, see if there is are one of more faces. Note that they will be
      // grouped into a shell.
      IfcSchema::IfcOpenShell *shell;
      ...
      if (face_count > 0) {
        ...
        rep = new IfcSchema::IfcShapeRepresentation(
            0, std::string("Body"),
            advanced ? std::string("AdvancedBrep") : std::string("Brep"),
            items);
      } else {
        ...
        if (edges->size() == 0) {
          ...
          rep = new IfcSchema::IfcShapeRepresentation(
              0, std::string("Axis"), std::string("Curve2D"),
              edges->as<IfcSchema::IfcRepresentationItem>());
        } else {
          // A geometric set is created as that probably (?) makes more sense in
          // IFC
          IfcSchema::IfcGeometricCurveSet *curves =
              new IfcSchema::IfcGeometricCurveSet(
                  edges->as<IfcSchema::IfcGeometricSetSelect>());
          items->push(curves);
          rep = new IfcSchema::IfcShapeRepresentation(
              0, std::string("Axis"), std::string("GeometricCurveSet"),
              items->as<IfcSchema::IfcRepresentationItem>());
        }
      }
    }
  }
  ...
  reps->push(rep);
  return new IfcSchema::IfcProductDefinitionShape(boost::none, boost::none,
                                                  reps);
}

IfcUtil::IfcBaseClass *POSTFIX_SCHEMA(tesselate)(const TopoDS_Shape &shape,
                                                 double deflection) {
  ...
  for (TopExp_Explorer exp(shape, TopAbs_FACE); exp.More(); exp.Next()) {
    ...
    if (!tri.IsNull()) {
      ...
      for (int i = 1; i <= tri->NbNodes(); ++i) {
        ...
        xyz.push_back(pnt.Y());
        xyz.push_back(pnt.Z());
        IfcSchema::IfcCartesianPoint *cpnt =
            new IfcSchema::IfcCartesianPoint(xyz);
      ...
      for (int i = 1; i <= triangles.Length(); ++i) {
        ...
        IfcSchema::IfcFaceOuterBound *bound = new IfcSchema::IfcFaceOuterBound(
            loop, face.Orientation() != TopAbs_REVERSED);
        IfcSchema::IfcFaceBound::list::ptr bounds(
            new IfcSchema::IfcFaceBound::list);
        bounds->push(bound);
  ...
  IfcSchema::IfcConnectedFaceSet::list::ptr shells(
      new IfcSchema::IfcConnectedFaceSet::list);
  shells->push(shell);
  IfcSchema::IfcFaceBasedSurfaceModel *surface_model =
      new IfcSchema::IfcFaceBasedSurfaceModel(shells);
  ...
  IfcSchema::IfcShapeRepresentation *rep =
      new IfcSchema::IfcShapeRepresentation(0, std::string("Facetation"),
                                            std::string("SurfaceModel"), items);

  reps->push(rep);
  IfcSchema::IfcProductDefinitionShape *shapedef =
      new IfcSchema::IfcProductDefinitionShape(boost::none, boost::none, reps);
