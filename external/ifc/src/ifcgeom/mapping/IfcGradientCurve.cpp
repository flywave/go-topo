...
taxonomy::ptr mapping::map_impl(const IfcSchema::IfcGradientCurve *inst) {
  if (!inst->BaseCurve()->as<IfcSchema::IfcCompositeCurve>())
    Logger::Warning(
        "Expected IfcGradientCurve.BaseCurve to be IfcCompositeCurve",
        inst); // CT 4.1.7.1.1.2
  ...
  // Get starting position of gradient curve, which is relative to the base
  // curve The gradient curve can start before or after the start of the base
  // curve
  ...
  double gradient_start = m(0, 3); // start of vertical (row 0, col 3) -
                                   // "Distance Along" horizontal curve

  // create the vertical pwf
  auto vertical =
      taxonomy::make<taxonomy::piecewise_function>(gradient_start, spans);

  // create the horizontal pwf
  auto horizontal =
      taxonomy::cast<taxonomy::piecewise_function>(map(inst->BaseCurve()));

  // create the composite gradient curve function
  auto gradient_function =
      taxonomy::make<taxonomy::gradient_function>(horizontal, vertical, inst);

  // check to see if there is valid overlap of the horizontal and vertical
  // domains
  if (!(0 < gradient_function->length())) {
    Logger::Error(
        "IfcGradientCurve does not have a common domain with BaseCurve");
