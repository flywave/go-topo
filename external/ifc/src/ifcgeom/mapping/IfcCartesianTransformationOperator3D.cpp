...
taxonomy::ptr
mapping::map_impl(const IfcSchema::IfcCartesianTransformationOperator3D *inst) {
  ...
  if (inst->Axis3()) {
    taxonomy::direction3::ptr ax3 =
        taxonomy::cast<taxonomy::direction3>(map(inst->Axis3()));
    axis3 << *ax3->components_, 0.0;
  }

  auto m4 = taxonomy::make<taxonomy::matrix4>(origin.head<3>(), axis3.head<3>(),
                                              axis1.head<3>());
  ...
  if (inst->as<IfcSchema::IfcCartesianTransformationOperator3DnonUniform>()) {
    auto nu =
        inst->as<IfcSchema::IfcCartesianTransformationOperator3DnonUniform>();
