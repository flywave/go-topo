...
taxonomy::ptr mapping::map_impl(const IfcSchema::IfcIndexedPolyCurve *inst) {
  ...
  if (point_list->as<IfcSchema::IfcCartesianPointList2D>()) {
    coordinates =
        point_list->as<IfcSchema::IfcCartesianPointList2D>()->CoordList();
  } else if (point_list->as<IfcSchema::IfcCartesianPointList3D>()) {
    coordinates =
        point_list->as<IfcSchema::IfcCartesianPointList3D>()->CoordList();
  }
  ...
  if (inst->Segments()) {
    ...
    for (auto it = segments->begin(); it != segments->end(); ++it) {
      ...
      if (segment->as<IfcSchema::IfcLineIndex>()) {
        ...
        for (std::vector<int>::const_iterator jt = indices.begin();
          ...
          if (*jt < 1 || *jt > max_index) {
            throw IfcParse::IfcException(
                "IfcIndexedPolyCurve index out of bounds for index " +
        ...
        auto circ = taxonomy::circle::from_3_points(
            a->ccomponents(), b->ccomponents(), c->ccomponents());
        if (circ) {
        ...
        throw IfcParse::IfcException(
            "Unexpected IfcIndexedPolyCurve segment of type " +
            segment->as<IfcUtil::IfcBaseClass>()->declaration().name());
      }
    }
