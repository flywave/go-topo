...
taxonomy::ptr
mapping::map_impl(const IfcSchema::IfcFixedReferenceSweptAreaSolid *inst) {
  ...
  if (auto fn = taxonomy::dcast<taxonomy::function_item>(dir)) {
    ...
    // Invocation of mapping on pre-4.3 models can never result in a
    // piecewise_function.
    if (inst->StartParam() &&
        inst->StartParam()->as<IfcSchema::IfcLengthMeasure>()) {
    ...
    if (inst->EndParam() &&
        inst->EndParam()->as<IfcSchema::IfcLengthMeasure>()) {
...
std::wcout << "determinant: " << m4.determinant() << std::endl;
      */
      ...
      if (is_directrix_derived) {
        ...
        Eigen::Vector3d proj =
            (ref->components() - tangent * tangent.dot(ref->components()));
        proj.normalize();
        ...
        if (condition_number > 1.e10) {
                Logger::Error("Non-invertible matrix at " +
        std::to_string(distalong) + " conversion will likely fail.");
        }
        */
      }

      // @todo taxonomy::clone() does not actually clone. That's really
      // confusing. loft->children.push_back(taxonomy::clone(profile));
      loft->children.push_back(taxonomy::face::ptr(profile->clone_()));
      loft->children.back()->matrix = taxonomy::make<taxonomy::matrix4>();

      if (profile->matrix) {
        loft->children.back()->matrix->components() =
            (m4b * profile->matrix->ccomponents()).eval();
      } else {
