...
taxonomy::ptr mapping::map_impl(const IfcSchema::IfcTShapeProfileDef *inst) {
  ...
  const double flangeSlope =
      hasFlangeSlope ? (*inst->FlangeSlope() * angle_unit_) : 0.;
  ...
  if (hasWebSlope || hasFlangeSlope) {
    ...
    const double y1e = y - d2;
    const double x2s = x;
    const double y2s = y - d2 + dy2;
    const double x2e = d1 / 2.;
    const double y2e = y - d2 - dy1;
    ...
    if (std::fabs(det) < 1.e-5) {
      Logger::Message(Logger::LOG_NOTICE,
                      "Web and flange do not intersect for:", inst);
      return nullptr;
    }
  ...
  return profile_helper(m4, {{{d1 / 2. - dx2, -y}, {f2}},
                             {{xx, xy}, {f1}},
                             {{x, y - d2 + dy2}, {f3}},
                             {{x, y}},
                             {{-x, y}},
