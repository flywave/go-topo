...
taxonomy::ptr
mapping::map_impl(const IfcSchema::IfcOffsetCurveByDistances *inst) {
  auto offset_values = inst->OffsetValues();
  if (offset_values->size() == 0) {
    Logger::Error(
        "IfcOffsetCurveByDistances must have at least one offset value");
  }
  ...
  if (!curve) {
    ...
    Logger::Warning("IfcOffsetCurveByDistances is only implemented for "
                    "BasisCurves curves based on taxonomy::function_item",
                    inst);
    return nullptr;
  }
  ...
  double first_distance =
      *first_offset_value->DistanceAlong()->as<IfcSchema::IfcLengthMeasure>();
#endif

  if (first_distance < 0.0) {
    Logger::Warning("IfcOffsetCurveByDistance first offset value is before the "
                    "start of the curve.");
  }

  if (0.0 < first_distance) {
    // First offset is defined after the start of the curve so the lateral and
    // vertical offsets implicitly continue with the same value towards the
    // start of the basis curve
    ...
    offset_spans.emplace_back(
        taxonomy::make<taxonomy::functor_item>(first_distance, fn));
  }
  ...
  for (; next != end; prev++, next++) {
    ...
    if ((*prev)->BasisCurve() != basis_curve ||
        (*next)->BasisCurve() != basis_curve) {
      Logger::Error("All offsets from a IfcOffsetCurveByDistances must refer "
                    "to the same BasisCurve");
    }
#endif
...
#endif
    if ((dp < 0.0 || basis_curve_length < dp) or
        (dn < 0.0 || basis_curve_length < dn) or (dn < dp)) {
      Logger::Warning(
          "IfcOffsetCurveByDistance offset value is out of bounds.");
      continue;
    }
  ...
  double last_distance =
      *(*prev)->DistanceAlong()->as<IfcSchema::IfcLengthMeasure>() *
      length_unit_;
#endif

  if (basis_curve_length < last_distance) {
    Logger::Warning("IfcOffsetCurveByDistance last offset value is after the "
                    "end of the curve.");
  }

  if (last_distance < basis_curve_length) {
    // Last offset is defined before the end of the curve so the lateral and
    // vertical offsets implicitly continue with the same value towards the end
    // of the basis curve
  ...
  auto offsets =
      taxonomy::make<taxonomy::piecewise_function>(start, offset_spans);
