...
struct POSTFIX_SCHEMA(factory_t) {
  abstract_mapping *operator()(IfcParse::IfcFile *file,
                               Settings &settings) const {
    ifcopenshell::geometry::POSTFIX_SCHEMA(mapping) *m =
        new ifcopenshell::geometry::POSTFIX_SCHEMA(mapping)(file, settings);
    return m;
  }
};
} // namespace

void MAKE_INIT_FN(MappingImplementation)(
    ifcopenshell::geometry::impl::MappingFactoryImplementation *mapping) {
...
IfcSchema::IfcProduct::list::ptr mapping::products_represented_by(
    const IfcSchema::IfcRepresentation *representation,
    IfcSchema::IfcRepresentationMap *&rmap, bool only_direct) {
  IfcSchema::IfcProduct::list::ptr products(new IfcSchema::IfcProduct::list);

  IfcSchema::IfcProductRepresentation::list::ptr prodreps =
      representation->OfProductRepresentation();

  for (IfcSchema::IfcProductRepresentation::list::it it = prodreps->begin();
       it != prodreps->end(); ++it) {
    // http://buildingsmart-tech.org/ifc/IFC2x3/TC1/html/ifcrepresentationresource/lexical/ifcproductrepresentation.htm
    // IFC2x Edition 3 NOTE  Users should not instantiate the entity
    // IfcProductRepresentation from IFC2x Edition 3 onwards. It will be changed
    // into an ABSTRACT supertype in future releases of IFC.

    // IfcProductRepresentation also lacks the INVERSE relation to IfcProduct
    // Let's find the IfcProducts that reference the IfcProductRepresentation
    // anyway
    products->push(
        (*it)
            ->file_
            ->getInverse((*it)->id(), &IfcSchema::IfcProduct::Class(), -1)
            ->as<IfcSchema::IfcProduct>());
  }
  ...
  if (maps->size() == 1) {
    ...
    taxonomy::matrix4::ptr origin =
        taxonomy::cast<taxonomy::matrix4>(map(rmap->MappingOrigin()));
    if (origin->is_identity()) {
      IfcSchema::IfcMappedItem::list::ptr items = rmap->MapUsage();
      for (IfcSchema::IfcMappedItem::list::it it = items->begin();
           it != items->end(); ++it) {
        ...
        try {
          target =
              taxonomy::cast<taxonomy::matrix4>(map(item->MappingTarget()));
        ...
        IfcSchema::IfcRepresentation::list::ptr reps =
            item->file_
                ->getInverse(item->id(),
                             (&IfcSchema::IfcRepresentation::Class()), -1)
                ->as<IfcSchema::IfcRepresentation>();
        for (IfcSchema::IfcRepresentation::list::it jt = reps->begin();
             jt != reps->end(); ++jt) {
          IfcSchema::IfcRepresentation *rep = *jt;
          if (rep->Items()->size() != 1)
            continue;
          IfcSchema::IfcProductRepresentation::list::ptr prodreps_mapped =
              rep->OfProductRepresentation();
          for (IfcSchema::IfcProductRepresentation::list::it kt =
                   prodreps_mapped->begin();
               kt != prodreps_mapped->end(); ++kt) {
            IfcSchema::IfcProduct::list::ptr ps =
                (*kt)
                    ->file_
                    ->getInverse((*kt)->id(), (&IfcSchema::IfcProduct::Class()),
                                 -1)
                    ->as<IfcSchema::IfcProduct>();
            products->push(ps);
          }
...
namespace {
IfcSchema::IfcProduct::list::ptr
filter_products(IfcSchema::IfcProduct::list::ptr unfiltered_products,
...
bool mapping::reuse_ok_(const IfcSchema::IfcProduct::list::ptr &products) {
  ...
  for (IfcSchema::IfcProduct::list::it it = products->begin();
    ...
    if (!settings_.get<settings::DisableOpeningSubtractions>().get() &&
        find_openings(product)->size()) {
      return false;
    }
    ...
    if (settings_.get<settings::ApplyLayerSets>().get()) {
      ...
      for (IfcSchema::IfcRelAssociates::list::it jt = associations->begin();
        ...
        if (assoc) {
          if (assoc->RelatingMaterial()->declaration().is(
    ...
    // Note that this can be a nullptr (!), but the fact that set size should be
    // one still holds
...
mapping::find_openings(const IfcUtil::IfcBaseEntity *inst) {
  ...
  if (inst->as<IfcSchema::IfcElement>() &&
      !inst->as<IfcSchema::IfcFeatureElementSubtraction>()) {
  ...
  // Is the IfcElement a decomposition of an IfcElement with any
  // IfcOpeningElements?
  const IfcSchema::IfcObjectDefinition *obdef =
      inst->as<IfcSchema::IfcObjectDefinition>();
  if (obdef != nullptr) {
    for (;;) {
      ...
      if ((*decomposes->begin())->as<IfcSchema::IfcRelAggregates>() ==
          nullptr) {
      ...
      IfcSchema::IfcObjectDefinition *rel_obdef =
          (*decomposes->begin())
              ->as<IfcSchema::IfcRelAggregates>()
              ->RelatingObject();
      if (rel_obdef->as<IfcSchema::IfcElement>() &&
          !rel_obdef->as<IfcSchema::IfcFeatureElementSubtraction>()) {
...
void mapping::get_representations(std::vector<geometry_conversion_task> &tasks,
                                  std::vector<filter_t> &filters) {
  ...
  for (auto representation : *representations) {
    ...
    IfcSchema::IfcProduct::list::ptr ifcproducts = filter_products(
        products_represented_by(representation, rmap, false), filters);
    ...
    IfcSchema::IfcRepresentation *representation_mapped_to_result =
        representation_mapped_to(representation);
    if (representation_mapped_to_result) {
      representation_processed_as_mapped_item =
          geometry_reuse_ok_for_current_representation_ &&
          (ok_mapped_representations->contains(
               representation_mapped_to_result) ||
           reuse_ok_(
               products_represented_by(representation_mapped_to_result, rmap)));
    }
    ...
    if (!geometry_reuse_ok_for_current_representation_ &&
        ifcproducts->size() > 1) {
      // reuse_ok is taken into account in products_represented_by(), but not
      // when the same IfcRepresentation is directly assigned to multiple
      // products.
...
mapping::get_product_type(const IfcUtil::IfcBaseEntity *product_) {
  ...
  for (auto it = rels->begin(); it != rels->end(); ++it) {
    ...
    auto rel = *it;
#else // IFC2X3.
    IfcSchema::IfcRelDefinesByType *rel =
        (*it)->as<IfcSchema::IfcRelDefinesByType>();
    if (rel == nullptr) {
      continue;
    }
#endif
...
const IfcUtil::IfcBaseEntity *mapping::get_single_material_association(
  ...
  IfcSchema::IfcRelAssociatesMaterial::list::ptr associated_materials =
      product->HasAssociations()->as<IfcSchema::IfcRelAssociatesMaterial>();
  if (associated_materials->size() == 1) {
    IfcSchema::IfcMaterialSelect *associated_material = nullptr;

    try {
      associated_material =
          (*associated_materials->begin())->RelatingMaterial();
    ...
    if (associated_material) {
      ...
      // NB: Single-layer layersets are also considered, regardless of
      // --enable-layerset-slicing, this in accordance with other viewers.
      if (!single_material) {
        if (associated_material->as<IfcSchema::IfcMaterialLayerSetUsage>() ||
            associated_material->as<IfcSchema::IfcMaterialLayerSet>()) {
          IfcSchema::IfcMaterialLayerSet *layerset;
          if (auto *m = associated_material
                            ->as<IfcSchema::IfcMaterialLayerSetUsage>()) {
          ...
          if (settings_.get<settings::LayersetFirst>().value
                  ? layerset->MaterialLayers()->size() >= 1
                  : layerset->MaterialLayers()->size() == 1) {
            IfcSchema::IfcMaterialLayer *layer =
                (*layerset->MaterialLayers()->begin());
        ...
        if (associated_material->as<IfcSchema::IfcMaterialProfileSetUsage>() ||
            associated_material->as<IfcSchema::IfcMaterialProfileSet>()) {
          IfcSchema::IfcMaterialProfileSet *profileset;
          if (auto *m = associated_material
                            ->as<IfcSchema::IfcMaterialProfileSetUsage>()) {
          ...
          if (settings_.get<settings::LayersetFirst>().value
                  ? profileset->MaterialProfiles()->size() >= 1
                  : profileset->MaterialProfiles()->size() == 1) {
            IfcSchema::IfcMaterialProfile *profile =
                (*profileset->MaterialProfiles()->begin());
...
IfcSchema::IfcRepresentation *mapping::representation_mapped_to(
    const IfcSchema::IfcRepresentation *representation) {
  ...
  if (items->size() == 1) {
    ...
    if (item->declaration().is(IfcSchema::IfcMappedItem::Class())) {
      if (item->StyledByItem()->size() == 0) {
        IfcSchema::IfcMappedItem *mapped_item =
            item->as<IfcSchema::IfcMappedItem>();
        taxonomy::matrix4::ptr target;
        try {
          target = taxonomy::cast<taxonomy::matrix4>(
              map(mapped_item->MappingTarget()));
        } catch (const std::exception &e) {
          Logger::Error(e);
        }
...
find_item_carrying_style(const IfcSchema::IfcRepresentationItem *item) {
  ...
  while (auto booleanresult = item->as<IfcSchema::IfcBooleanClippingResult>()) {
    // All instantiations of IfcBooleanOperand (type of FirstOperand) are
    // subtypes of IfcGeometricRepresentationItem
    item =
        booleanresult->FirstOperand()->as<IfcSchema::IfcRepresentationItem>();
    if (item->StyledByItem()->size()) {
      return item;
    }
  }

  // TODO: Ideally this would be done for other entities (such as IfcCsgSolid)
  // as well. But neither are these very prevalent, nor does the current
  // IfcOpenShell style mechanism enable to conveniently style subshapes, which
  // would be necessary for distinctly styled union operands.
...
get_surface_style(const IfcSchema::IfcStyledItem *si) {
  ...
  for (auto kt = style_assignments->begin(); kt != style_assignments->end();
       ++kt) {
    // Using IfcPresentationStyleAssignment is deprecated, use the direct
    // assignment of a subtype of IfcPresentationStyle instead.
    ...
    auto style_assignment =
        (*kt)->as<IfcSchema::IfcPresentationStyleAssignment>();
    if (!style_assignment) {
      continue;
    }
  ...
  IfcSchema::IfcPresentationStyleAssignment::list::ptr style_assignments =
      si->Styles();
  for (IfcSchema::IfcPresentationStyleAssignment::list::it kt =
           style_assignments->begin();
...
#if defined(SCHEMA_HAS_IfcStyleAssignmentSelect) ||                            \
    defined(SCHEMA_HAS_IfcPresentationStyleAssignment)
  }
#endif
  ...
  for (auto &style : prs_styles) {
    if (auto surface_style = style->as<IfcSchema::IfcSurfaceStyle>()) {
      if (surface_style->Side() !=
          IfcSchema::IfcSurfaceSide::IfcSurfaceSide_NEGATIVE) {
        surface_style_ = surface_style;
        auto styles_elements = surface_style->Styles();
        for (auto mt = styles_elements->begin(); mt != styles_elements->end();
             ++mt) {
          if ((*mt)->template as<T>()) {
...
bool process_colour(IfcSchema::IfcColourOrFactor *colour_or_factor,
                    double *rgb) {
  if (colour_or_factor == 0) {
    return false;
  } else if (colour_or_factor->declaration().is(
                 IfcSchema::IfcColourRgb::Class())) {
    return process_colour(
        static_cast<IfcSchema::IfcColourRgb *>(colour_or_factor), rgb);
  } else if (colour_or_factor->declaration().is(
                 IfcSchema::IfcNormalisedRatioMeasure::Class())) {
    return process_colour(
        static_cast<IfcSchema::IfcNormalisedRatioMeasure *>(colour_or_factor),
        rgb);
  } else {
    return false;
  }
...
const IfcSchema::IfcStyledItem *mapping::find_style(
    const IfcSchema::IfcRepresentationItem *representation_item) {
  ...
  IfcSchema::IfcStyledItem::list::ptr styled_items =
      representation_item->StyledByItem();
  if (styled_items->size()) {
    // StyledByItem is a SET [0:1] OF IfcStyledItem, so we return after the
    // first IfcStyledItem:
...
taxonomy::ptr mapping::map_impl(const IfcSchema::IfcMaterial *material) {
  IfcSchema::IfcMaterialDefinitionRepresentation::list::ptr defs =
      material->HasRepresentation();
  for (IfcSchema::IfcMaterialDefinitionRepresentation::list::it jt =
           defs->begin();
       jt != defs->end(); ++jt) {
    IfcSchema::IfcRepresentation::list::ptr reps = (*jt)->Representations();
    IfcSchema::IfcStyledItem::list::ptr styles(
        new IfcSchema::IfcStyledItem::list);
    for (IfcSchema::IfcRepresentation::list::it it = reps->begin();
         it != reps->end(); ++it) {
    ...
    if (styles->size() == 1) {
      ...
      Logger::Warning("Skipping unsupported material style for material: ",
                      material);
    }
  }

  // When material does not have a representation we don't create a style from
  // it
  return nullptr;
  ...
  */
  ...
  // @todo
  // IfcGeom::SurfaceStyle material_style =
  // IfcGeom::SurfaceStyle(material->data().id(), material->Name()); return
  // &(style_cache[material->data().id()] = material_style);
}
...
taxonomy::ptr mapping::map_impl(const IfcSchema::IfcStyledItem *inst) {
  ...
  if (style == nullptr) {
    ...
    Logger::Warning("Only IfcSurfaceStyle is supported, couldn't find it in "
                    "IfcStyledItem: ",
                    inst);
...
taxonomy::ptr mapping::map_impl(const IfcSchema::IfcSurfaceStyle *style) {
  ...
  surface_style->use_surface_color =
      settings_.get<settings::SurfaceColour>().get();
  ...
  if (auto rendering_style =
          shading->as<IfcSchema::IfcSurfaceStyleRendering>()) {
    if (rendering_style->DiffuseColour() &&
        process_colour(rendering_style->DiffuseColour(), rgb)) {
      const taxonomy::colour &old_diffuse =
          surface_style->diffuse ? surface_style->diffuse : white;
      surface_style->diffuse =
          taxonomy::colour(old_diffuse.r() * rgb[0], old_diffuse.g() * rgb[1],
                           old_diffuse.b() * rgb[2]);
    }
    ...
    if (rendering_style->SpecularColour() &&
        process_colour(rendering_style->SpecularColour(), rgb)) {
      surface_style->specular = taxonomy::colour(rgb[0], rgb[1], rgb[2]);
    }
    if (rendering_style->SpecularHighlight()) {
      IfcSchema::IfcSpecularHighlightSelect *highlight =
          rendering_style->SpecularHighlight();
      if (highlight->declaration().is(
              IfcSchema::IfcSpecularRoughness::Class())) {
        double roughness = *((IfcSchema::IfcSpecularRoughness *)highlight);
        if (roughness >= 1e-9) {
          surface_style->specularity = (1.0 / roughness);
        }
      } else if (highlight->declaration().is(
                     IfcSchema::IfcSpecularExponent::Class())) {
        surface_style->specularity =
            (*((IfcSchema::IfcSpecularExponent *)highlight));
      }
    }
...
taxonomy::ptr mapping::map(const IfcBaseInterface *inst) {
  ...
  // @todo we should check whether there is a notice performance impact on the
  // large sequence of if-statements and whether a switch on e.g
  // inst->declaration()->index_in_schema() isn't more efficient (which would
  // disable inheritance though).

  bool matched = false;

#include <ifcgeom/mapping/bind_convert_impl.i>
...
get_RelatingObject(IfcSchema::IfcRelDecomposes *decompose) {
#ifdef SCHEMA_IfcRelDecomposes_HAS_RelatingObject
  return decompose->RelatingObject();
#else
  IfcSchema::IfcRelAggregates *aggr =
      decompose->as<IfcSchema::IfcRelAggregates>();
  if (aggr != nullptr) {
...
IfcUtil::IfcBaseEntity *
mapping::get_decomposing_entity(const IfcUtil::IfcBaseEntity *inst,
  ...
  if (include_openings &&
      product->declaration().is(IfcSchema::IfcOpeningElement::Class())) {
    IfcSchema::IfcOpeningElement *opening =
        (IfcSchema::IfcOpeningElement *)product;
    ...
    if (fills->size() && include_openings) {
      for (IfcSchema::IfcRelFillsElement::list::it it = fills->begin();
           it != fills->end(); ++it) {
    ...
    if (!parent) {
      IfcSchema::IfcRelContainedInSpatialStructure::list::ptr parents =
          element->ContainedInStructure();
      if (parents->size()) {
        IfcSchema::IfcRelContainedInSpatialStructure *container =
            *parents->begin();
  ...
  if (!parent) {
    aggregate_of_instance::ptr parents = product->file_->getInverse(
        product->id(), (&IfcSchema::IfcRelAggregates::Class()), -1);
    parents->push(product->file_->getInverse(
        product->id(), (&IfcSchema::IfcRelNests::Class()), -1));
    for (aggregate_of_instance::it it = parents->begin(); it != parents->end();
         ++it) {
      IfcSchema::IfcRelDecomposes *decompose =
          (*it)->as<IfcSchema::IfcRelDecomposes>();
      IfcUtil::IfcBaseEntity *ifc_objectdef;

      ifc_objectdef = get_RelatingObject(decompose);

      if (!ifc_objectdef || product == ifc_objectdef)
        continue;
...
mapping::get_layers(IfcUtil::IfcBaseEntity *inst) {
  ...
  if (prod->Representation()) {
    ...
    IfcSchema::IfcRepresentation::list::ptr representations =
        r->as<IfcSchema::IfcRepresentation>();
    for (IfcSchema::IfcRepresentation::list::it it = representations->begin();
         it != representations->end(); ++it) {
      IfcSchema::IfcPresentationLayerAssignment::list::ptr a =
          (*it)->LayerAssignments();
      for (IfcSchema::IfcPresentationLayerAssignment::list::it jt = a->begin();
           jt != a->end(); ++jt) {
        layers[(*jt)->Name()] = *jt;
      }
    }
...
void mapping::initialize_units_() {
    ...
    if (!units || !units->size()) {
      ...
      for (auto it = units->begin(); it != units->end(); ++it) {
        ...
        if (base->declaration().is(IfcSchema::IfcNamedUnit::Class())) {
          IfcSchema::IfcNamedUnit *named_unit =
              base->as<IfcSchema::IfcNamedUnit>();
          if (named_unit->UnitType() ==
                  IfcSchema::IfcUnitEnum::IfcUnit_LENGTHUNIT ||
              named_unit->UnitType() ==
                  IfcSchema::IfcUnitEnum::IfcUnit_PLANEANGLEUNIT) {
            std::string current_unit_name;
            const double current_unit_magnitude =
                IfcParse::get_SI_equivalent<IfcSchema>(named_unit);
            if (current_unit_magnitude != 0.) {
              if (named_unit->declaration().is(
                      IfcSchema::IfcConversionBasedUnit::Class())) {
                IfcSchema::IfcConversionBasedUnit *u =
                    (IfcSchema::IfcConversionBasedUnit *)base;
                current_unit_name = u->Name();
              } else if (named_unit->declaration().is(
                             IfcSchema::IfcSIUnit::Class())) {
                IfcSchema::IfcSIUnit *si_unit =
                    named_unit->as<IfcSchema::IfcSIUnit>();
                if (si_unit->Prefix()) {
                  current_unit_name =
                      IfcSchema::IfcSIPrefix::ToString(*si_unit->Prefix());
                }
                current_unit_name +=
                    IfcSchema::IfcSIUnitName::ToString(si_unit->Name());
              }
              if (named_unit->UnitType() ==
                  IfcSchema::IfcUnitEnum::IfcUnit_LENGTHUNIT) {
  ...
  if (settings_.get<settings::BuildingLocalPlacement>().get()) {
    placement_rel_to_type_ =
        file_->schema()->declaration_by_name("IfcBuilding");
  }
  ...
  if (settings_.get<ModelOffset>().has()) {
    ...
    if (vs.size() == 3) {
      offset_and_rotation_ *=
          Eigen::Affine3d(Eigen::Translation3d(vs[0], vs[1], vs[2])).matrix();
    } else {
  ...
  if (settings_.get<ModelRotation>().has()) {
    ...
    if (vs.size() == 4) {
      // @nb W, X, Y, Z
      auto m3 =
          Eigen::Quaterniond(vs[3], vs[0], vs[1], vs[2]).normalized().matrix();
...
void mapping::initialize_settings() {
  ...
  IfcSchema::IfcGeometricRepresentationContext::list::ptr contexts =
      file_->instances_by_type_excl_subtypes<
  ...
  for (it = contexts->begin(); it != contexts->end(); ++it) {
    ...
    if (context->Precision() && (*context->Precision() * length_unit_ * fp) <
...
bool mapping::get_layerset_information(const IfcUtil::IfcBaseInterface *p,
                                       layerset_information &info, int &) {
  ...
  IfcSchema::IfcRelAssociates::list::ptr associations =
      product->HasAssociations();
  for (IfcSchema::IfcRelAssociates::list::it it = associations->begin();
       it != associations->end(); ++it) {
    IfcSchema::IfcRelAssociatesMaterial *associates_material =
        (**it).as<IfcSchema::IfcRelAssociatesMaterial>();
    if (associates_material) {
      usage = associates_material->RelatingMaterial()
                  ->as<IfcSchema::IfcMaterialLayerSetUsage>();
      break;
    }
  }
  ...
  IfcSchema::IfcRepresentation *body_representation =
      find_representation(product, "Body");
  ...
  if (product->declaration().is(IfcSchema::IfcWall::Class())) {
    ...
    for (IfcSchema::IfcMaterialLayer::list::it it = material_layers->begin();
         it != material_layers->end(); ++it) {
      info.styles.push_back(
          *taxonomy::cast<taxonomy::style>(map((*it)->Material())));
    ...
    IfcSchema::IfcExtrudedAreaSolid::list::ptr extrusions =
        IfcParse::traverse(body_representation)
            ->as<IfcSchema::IfcExtrudedAreaSolid>();

    if (extrusions->size() != 1) {
      Logger::Message(
          Logger::LOG_WARNING,
          "No single extrusion found in body representation for:", product);
      return false;
    }
    ...
    if (has_position) {
      ...
      if (!m4) {
        Logger::Message(
            Logger::LOG_ERROR,
            "Failed to convert placement for extrusion of:", product);
        return false;
      } else {
    ...
    taxonomy::direction3::ptr extrusion_direction =
        taxonomy::cast<taxonomy::direction3>(
            map(extrusion->ExtrudedDirection()));

    if (!extrusion_direction) {
      Logger::Message(Logger::LOG_ERROR,
                      "Failed to convert direction for extrusion of:", product);
      return false;
    }

    // @todo I don't think this is correct actually. This shouldn't take into
    // account extrusion direction? reference_surface = new
    // Geom_Plane(extrusion_position.TranslationPart(), extrusion_direction);
...
bool mapping::get_wall_neighbours(const IfcUtil::IfcBaseInterface *,
                                  std::vector<endpoint_connection> &) {
  return false;
}

IfcSchema::IfcRepresentation *
mapping::find_representation(const IfcSchema::IfcProduct *product,
  ...
  for (IfcSchema::IfcRepresentation::list::it it = reps->begin();
       it != reps->end(); ++it) {
    if ((**it).RepresentationIdentifier() &&
        (*(**it).RepresentationIdentifier()) == identifier) {
      return *it;
    }
  }
  return 0;
...
void mapping::addRepresentationsFromContextIds(
  ...
  for (auto context_id : settings_.get<settings::ContextIds>().get()) {
    ...
    try {
      context = file_->instance_by_id(context_id)
                    ->as<IfcSchema::IfcGeometricRepresentationContext>();
    } catch (IfcParse::IfcException &e) {
      Logger::Error(e);
      continue;
    }
    ...
    if (!context) {
      Logger::Error("Failed to process context ID " +
...
void mapping::addRepresentationsFromDefaultContexts(
  ...
  if (this->settings_.get<settings::OutputDimensionality>().get() !=
      settings::CURVES) {
  ...
  for (it = contexts->begin(); it != contexts->end(); ++it) {
    IfcSchema::IfcGeometricRepresentationContext *context = *it;
    if (context->declaration().is(
            IfcSchema::IfcGeometricRepresentationSubContext::Class())) {
    ...
    try {
      if (context->ContextType()) {
        ...
        if (allowed_context_types.find(context_type) ==
          ...
          Logger::Warning(std::string("ContextType '") +
  ...
  // In case no contexts are identified based on their ContextType, all contexts
  // are considered. Note that sub contexts are excluded as they are considered
  // later on.
  ...
  if (representations->size() == 0) {
    Logger::Warning(
        "No representations encountered in relevant contexts, using all");
    representations->push(
        file_->instances_by_type<IfcSchema::IfcRepresentation>());
  }
}

IfcUtil::IfcBaseEntity *
mapping::representation_of(const IfcUtil::IfcBaseEntity *product) {
  // @todo correct, but very inefficient
  IfcSchema::IfcRepresentation::list::ptr representations(
      new IfcSchema::IfcRepresentation::list);
  IfcSchema::IfcRepresentation::list::ptr of_product(
      new IfcSchema::IfcRepresentation::list);
  IfcSchema::IfcRepresentation::list::ptr intersection(
      new IfcSchema::IfcRepresentation::list);
  IfcSchema::IfcRepresentation::list::ptr intersection_no_box(
      new IfcSchema::IfcRepresentation::list);
  ...
  if (product->as<IfcSchema::IfcProduct>()->Representation()) {
    of_product->push(product->as<IfcSchema::IfcProduct>()
                         ->Representation()
                         ->Representations());
  }
  ...
  if (intersection->size() == 0) {
    ...
    if (intersection_no_box->size() > 1) {
      Logger::Warning("Multiple applicable representations found for element, "
                      "selecting arbitrary");
    }
