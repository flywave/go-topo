...
taxonomy::ptr mapping::map_impl(const IfcSchema::IfcLShapeProfileDef *inst) {
  ...
  const double x =
      inst->Width().get_value_or(inst->Depth()) / 2.0f * length_unit_;
  ...
  if (hasSlope) {
    ...
    const double x1s = x;
    const double y1s = -y + d - dy1;
    const double x1e = -x + d;
    const double y1e = -y + d + dy2;
    const double x2s = -x + d - dx1;
    const double y2s = y;
    const double x2e = -x + d + dx2;
    const double y2e = -y + d;
