...
namespace {
taxonomy::boolean_result::operation_t
boolean_op_type(IfcSchema::IfcBooleanOperator::Value op) {
  if (op == IfcSchema::IfcBooleanOperator::IfcBooleanOperator_DIFFERENCE) {
    return taxonomy::boolean_result::SUBTRACTION;
  } else if (op ==
             IfcSchema::IfcBooleanOperator::IfcBooleanOperator_INTERSECTION) {
...
taxonomy::ptr mapping::map_impl(const IfcSchema::IfcBooleanResult *inst) {
  ...
  if (op == taxonomy::boolean_result::SUBTRACTION) {
    ...
    while (true) {
      ...
      if (res1 && res1->SecondOperand()->as<IfcSchema::IfcHalfSpaceSolid>() &&
          ++n_half_space_operands > 8) {
        // There is something peculiar about many half space subtraction
        // operands that OCCT does not like. Often these are used to create a
        // semi-curved arch, as is the case in 693. Supplying all these operands
        // at once apparently leads to too many edge-edge interference checks.

        // @todo this should probably be moved to the opencascade kernel and not
        // the mapping.
