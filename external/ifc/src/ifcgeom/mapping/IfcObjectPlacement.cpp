...
taxonomy::ptr mapping::map_impl(const IfcSchema::IfcObjectPlacement *inst) {
  ...
  if (relative_to && (placement_rel_to_type_ || placement_rel_to_instance_)) {
    IfcSchema::IfcProduct::list::ptr parent_places =
        relative_to->PlacesObject();
    for (auto iter = parent_places->begin(); iter != parent_places->end();
         ++iter) {
      if ((placement_rel_to_type_ &&
           (*iter)->declaration().is(*placement_rel_to_type_)) ||
          (placement_rel_to_instance_ &&
           (*iter)->as<IfcUtil::IfcBaseEntity>() ==
               placement_rel_to_instance_)) {
...
if (gridp = inst->as<IfcSchema::IfcGridPlacement>()) {
        ...
        auto grids =
(*axes->begin())->data().file->getInverse<IfcSchema::IfcGrid>((*axes->begin())->data().id(),
-1); if (grids && grids->size()) { grid = *grids->begin(); if
(grid->ObjectPlacement()) { convert(grid->ObjectPlacement(), grid_position);
                }
        }
#endif

        auto get_point = [this, &c1, &c2,
&ecc](IfcSchema::IfcVirtualGridIntersection const* x, gp_Pnt& P) { auto axes =
x->IntersectingAxes(); auto offsets = x->OffsetDistances();

                if (axes->size() != 2) {
                        Logger::Message(Logger::LOG_WARNING, "Unexpected grid
axes count:" + std::to_string(axes->size()), x); return false;
                }
                if (offsets.size() != 3) {
                        Logger::Message(Logger::LOG_WARNING, "Unexpected offset
count:" + std::to_string(offsets.size()), x); return false;
                }
                auto first = *axes->begin();
                auto second = *++axes->begin();
                TopoDS_Wire w;
                // Might display a lot of 'No operation defined for ...'
messages if (!convert_curve(first->AxisCurve(), c1)) { if
(!convert_wire(first->AxisCurve(), w)) { return false;
                        }
                        double a, b;
                        c1 =
BRep_Tool::Curve(TopoDS::Edge(TopoDS_Iterator(w).Value()), a, b);
                }
...
BRep_Tool::Curve(TopoDS::Edge(TopoDS_Iterator(w).Value()), a, b);
                ...
                if (pp1.Distance(pp2) > getValue(GV_PRECISION)) {
                        Logger::Message(Logger::LOG_WARNING, "No axis
intersection:", x); return false;
                }
                P = pp1;
                return true;
        };
        ...
        if (gridp->PlacementRefDirection()) {
                ...
                if ((dir =
gridp->PlacementRefDirection()->as<IfcSchema::IfcDirection>())) { if
(!convert(dir, D)) { return false;
                        }
                } else if ((refx =
gridp->PlacementRefDirection()->as<IfcSchema::IfcVirtualGridIntersection>())) {
                        gp_Pnt P;
                        if (!get_point(refx, P)) {
                                return false;
                        }
                        ...
                        if (V.Magnitude() > 1.e-9) {
                                ...
                                Logger::Message(Logger::LOG_ERROR, "Unable to
obtain ref direction:", l); return false;
                        }
                }
        } else {
                gp_Pnt tmp_;
                ...
                if (V.Magnitude() > 1.e-9) {
                        ...
                        Logger::Message(Logger::LOG_ERROR, "Unable to obtain ref
direction:", l); return false;
                }
        }
