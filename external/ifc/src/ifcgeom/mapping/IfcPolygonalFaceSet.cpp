...
taxonomy::ptr mapping::map_impl(const IfcSchema::IfcPolygonalFaceSet *inst) {
  ...
  for (auto &f : *polygonal_faces) {
      ...
      for (std::vector<int>::const_iterator jt = indices.begin();
        ...
        if (*jt < 1 || *jt > max_index) {
          throw IfcParse::IfcException(
              "IfcPolygonalFaceSet index out of bounds for index " +
    ...
    if (f->as<IfcSchema::IfcIndexedPolygonalFaceWithVoids>()) {
      auto indices = f->as<IfcSchema::IfcIndexedPolygonalFaceWithVoids>()
                         ->InnerCoordIndices();
      {
        taxonomy::point3::ptr previous;
        for (auto &li : indices) {
          ...
          for (std::vector<int>::const_iterator jt = li.begin(); jt != li.end();
               ++jt) {
            if (*jt < 1 || *jt > max_index) {
              throw IfcParse::IfcException(
                  "IfcPolygonalFaceSet index out of bounds for index " +
