...
enum segment_type_t { ST_HORIZONTAL, ST_VERTICAL, ST_CANT };

// @todo use std::numbers::pi when upgrading to C++ 20
static const double PI = boost::math::constants::pi<double>();

double translate_to_length_measure(const IfcSchema::IfcCurve *crv,
                                   double param_value) {
...
double translate_if_param_value(const IfcSchema::IfcCurve *crv,
                                IfcSchema::IfcCurveMeasureSelect *val) {
...
// vector of parent curve types that are supported for
// IfcCurveSegment.ParentCurve
typedef boost::mpl::vector<IfcSchema::IfcLine, IfcSchema::IfcCircle,
...
struct parent_curve_function {
  ...
  parent_curve_function(std::function<Eigen::Matrix4d(double)> fn,
                        std::function<Eigen::Matrix4d(double)> cfn)
      : fn_(fn), cfn_(cfn) {}
...
struct curve_segment_function {
  curve_segment_function(const Eigen::Matrix4d &curve_segment_placement,
                         const Eigen::Matrix4d &remove_parent_curve_rotation,
                         const Eigen::Matrix4d &remove_parent_curve_translation,
                         std::shared_ptr<parent_curve_function> parent_curve_fn)
  ...
  Eigen::Matrix4d operator()(double u) const {
    ...
    Eigen::Matrix4d curve_segment_point =
        curve_segment_placement_ * remove_parent_curve_rotation_ *
        remove_parent_curve_translation_ * parent_curve_point;
...
struct cant_curve_segment_function {
  cant_curve_segment_function(
      const Eigen::Matrix4d &curve_segment_placement,
      const Eigen::Matrix4d &parent_curve_start_point,
      std::shared_ptr<parent_curve_function> parent_curve_fn)
      : curve_segment_placement_(curve_segment_placement),
  ...
  Eigen::Matrix4d operator()(double u) const {
    // The parent curve function returns the cant rotation and superelevation
    // for the parent curve. Subtract the parent_curve_start_point to get the
    // incremental cant rotation and superelevation Add the incremental cant
    // rotation and superelevation to curve_segment_placement to get the
    // curve_segment_point
    Eigen::Matrix4d parent_curve_point = (*parent_curve_fn_)(u);
    Eigen::Matrix4d cant_increment =
        parent_curve_point - parent_curve_start_point_;
    Eigen::Matrix4d curve_segment_point =
        curve_segment_placement_ + cant_increment;
    return curve_segment_point + parent_curve_fn_->curvature(u);
  }
...
private:
  ...
  const IfcSchema::IfcCurveSegment *inst_ =
      nullptr; // this curve segment instance
  double length_unit_;
  double start_;
  double
      length_; // length along the curve, as provided from the IfcCurveSegment
  segment_type_t segment_type_;
  const IfcSchema::IfcCurve *parent_curve_ = nullptr;

  double projected_length_; // for vertical segments, this is the length of
                            // curve projected onto the "Distance Along" axis

  std::shared_ptr<parent_curve_function>
      parent_curve_fn_; // function for the parent curve. Function takes
                        // distances along, u, and returns the 4x4 position
                        // matrix
  std::optional<Eigen::Matrix4d>
      parent_curve_start_point_; // placement matrix for the parent curve

  std::optional<Eigen::Matrix4d>
      curve_segment_placement_; // placement of this segment
  std::optional<Eigen::Matrix4d>
      next_segment_placement_; // placement of the next segment

public:
  curve_segment_evaluator(mapping *mapping,
                          const IfcSchema::IfcCurveSegment *inst,
                          double length_unit)
      : mapping_(mapping), inst_(inst), length_unit_(length_unit),
    ...
    if (composite_curves) {
      if (composite_curves->size() == 1) {
        auto segments = (*composite_curves->begin())
        ...
        Logger::Warning("IfcCurveSegment belongs to multiple IfcCompositeCurve "
                        "instances. Cannot determine the next segment.");
      }
    }
    ...
    if ((is_horizontal + is_vertical + is_cant) != 1) {
      ...
      Logger::Error(
          std::runtime_error("multiple uses of IfcSegmentCurve not supported"),
          inst_);
    }

    segment_type_ = is_horizontal ? ST_HORIZONTAL
                    : is_vertical ? ST_VERTICAL
                    : is_cant     ? ST_CANT
                                  : ST_HORIZONTAL;

    start_ =
        translate_if_param_value(inst->ParentCurve(), inst->SegmentStart()) *
        length_unit;
    length_ =
        translate_if_param_value(inst->ParentCurve(), inst->SegmentLength()) *
        length_unit;
    projected_length_ = length_; // initialize with something reasonable

    if (inst) {
      curve_segment_placement_ =
          taxonomy::cast<taxonomy::matrix4>(mapping_->map(inst->Placement()))
              ->ccomponents();
    }

    if (next_inst) {
      next_segment_placement_ = taxonomy::cast<taxonomy::matrix4>(
                                    mapping_->map(next_inst->Placement()))
                                    ->ccomponents();
    } else {
      // there is not a next segment, however IfcGradientCurve and
      // IfcSegmentReferenceCurve have an optional EndPoint which services the
      // same purpose as the zero-length last segment.
      ...
      if (composite_curves->size() == 1) {
        ...
        if (segment_type_ == ST_VERTICAL) {
          ...
          auto segmented_reference_curve =
              cc->as<IfcSchema::IfcSegmentedReferenceCurve>();
          end_point = segmented_reference_curve->EndPoint();
        }
      } else {
        Logger::Warning("IfcCurveSegment belongs to multiple IfcCompositeCurve "
                        "instances. Cannot determine the end point.");
      }
      if (end_point) {
        next_segment_placement_ =
            taxonomy::cast<taxonomy::matrix4>(mapping_->map(end_point))
                ->ccomponents();
      }
    }
  }

  // Take the boost::type value from mpl::for_each and test it against our curve
  // instance
  ...
  double length() const {
    return (segment_type_ == ST_HORIZONTAL || segment_type_ == ST_CANT)
               ? length_
               : projected_length_;
  }
  ...
  taxonomy::ptr get_segment_curve_function() {
    if (!parent_curve_fn_ || !parent_curve_start_point_) {
      Logger::Error(
          std::runtime_error(inst_->ParentCurve()->declaration().name() +
                             " not implemented"),
          inst_);
    }
    ...
    if (segment_type_ == ST_CANT) {
      auto fn = cant_curve_segment_function(*curve_segment_placement_,
                                            *parent_curve_start_point_,
                                            parent_curve_fn_);
      ...
      // Subtract the parent curve start point (remove the translation and
      // rotation) to get the incremental translation and rotation. Apply the
      // incremental translation and rotation to the curve_segment_placement to
      // get the curve_segment_point

      // Do a negative translation of the parent curve point relative to the
      // start of the parent curve. This moves parent_curve_fn(u=0.0) to
      // coordinate (0,0). This is done so the curve_segment_placement is
      // applied relative to (0,0)
      Eigen::Matrix4d remove_parent_curve_translation =
          Eigen::Matrix4d::Identity();
      remove_parent_curve_translation.col(3) =
          -1.0 * (*parent_curve_start_point_).col(3);
      remove_parent_curve_translation(3, 3) = 1.0;

      // Do a rotation so that the tangent of the parent curve is in the
      // direction (1,0) Example: if the parent curve IfcLine is at a 30 degree
      // clockwise angle, this does a 30 degree counter-clockwise rotation
      ...
      Eigen::Matrix4d remove_parent_curve_rotation =
          (*parent_curve_start_point_);
      remove_parent_curve_rotation(0, 1) *= -1.0;
      remove_parent_curve_rotation(1, 0) *= -1.0;
      remove_parent_curve_rotation.col(3) = Eigen::Vector4d(
          0, 0, 0, 1); // remove the parent curve placement point

      auto fn = curve_segment_function(
          *curve_segment_placement_, remove_parent_curve_rotation,
          remove_parent_curve_translation, parent_curve_fn_);
      return taxonomy::make<taxonomy::functor_item>(length, fn);
    }
  }

  void set_spiral_function(double s, std::function<double(double)> fnX,
                           std::function<double(double)> fnY,
    ...
    if (segment_type_ == ST_HORIZONTAL || segment_type_ == ST_VERTICAL) {
      ...
      parent_curve_fn_ = std::make_shared<spiral_parent_curve>(
          [start = start_, s, convert_u, fnX,
           fnY](double u) -> Eigen::Matrix4d {
            u = convert_u(u + start);
      ...
      if (segment_type_ == ST_VERTICAL) {
        ...
        // projected_length_ is the domain of the curve, measured in the
        // horizontal "Distance Along" coordinate The quickest way to get the
        // projected length is the difference of the i-ordinates at the start
        // and end of the spiral.
        ...
        auto dy = -(*curve_segment_placement_)(
            1, 0);                   // -1 to rotation in opposite direction
        auto X1 = x1 * dx - y1 * dy; // X of start point in global coordinates
        auto X2 = x2 * dx - y2 * dy; // X of end point in global coordinates
        projected_length_ =
            X2 - X1; // distance between points on the global X-axis
      } else {
        projected_length_ = length_;
      }
    } else if (segment_type_ == ST_CANT) {
      Logger::Error(std::runtime_error(
          "Unexpected segment type encountered - cant is handled in "
          "set_cant_spiral_function - should never get here"));
  ...
  void
  set_cant_spiral_function(std::function<double(double)> Superelevation,
                           std::function<double(double)> SuperelevationSlope,
                           std::function<double(double)> Cant) {
    ...
    dy = (next_segment_placement_.has_value() ? (*next_segment_placement_)(1, 2)
                                              : 0.0);
    dz = (next_segment_placement_.has_value() ? (*next_segment_placement_)(2, 2)
                                              : 1.0);
    ...
    parent_curve_fn_ = std::make_shared<spiral_parent_curve>(
        [start_angle, delta_angle, start_cant, delta_cant, Superelevation,
         SuperelevationSlope, Cant](double u) -> Eigen::Matrix4d {
          ...
          auto cant = Cant(u);
          auto tilt_angle =
              start_angle + delta_angle * (cant - start_cant) / delta_cant;
  ...
  // returns function for super elevation and the slope of the super elevation
  // curve if the super elevation is constant over the length of the segment.
  // otherwise, no functions are returned because they are the same as the cant
  // tilt angle functions.
  std::pair<boost::optional<std::function<double(double)>>,
            boost::optional<std::function<double(double)>>>
  get_superelevation_functions() {
    ...
    if (curve_segment_placement_.has_value() &&
        next_segment_placement_.has_value()) {
  ...
  void operator()(const IfcSchema::IfcClothoid *c) {
    ...
    if (segment_type_ == ST_CANT) {
      ...
      auto cant = [A, L](double t) -> double {
        return A ? L * L * A * t / fabs(pow(A, 3)) : 0.0;
      };
      ...
      if (!slope.is_initialized()) {
        slope = [A, L](double /*t*/) -> double {
      ...
      auto fn_x = [A, s](double t) -> double {
        return A ? s * cos(PI * A * t * t / (2 * fabs(A))) : 0.0;
      };
      auto fn_y = [A, s](double t) -> double {
        return A ? s * sin(PI * A * t * t / (2 * fabs(A))) : 0.0;
      };
      auto curvature = [A](double t) -> double {
        return A ? A * t / fabs(A * A * A) : 0.0;
      };
  ...
  void operator()(const IfcSchema::IfcCosineSpiral *c) {
    ...
    if (constant_term.is_initialized()) {
    ...
    if (segment_type_ == ST_HORIZONTAL) {
      ...
      auto theta = [constant_term, cosine_term, L](double t) -> double {
        auto a0 = constant_term.is_initialized() ? t / constant_term.value() : 0.0;
      ...
      auto curvature = [constant_term, cosine_term, L](double t) -> double {
        auto a0 = constant_term.is_initialized() ? L / constant_term.value() : 0.0;
      ...
      auto cant = [constant_term, cosine_term, L](double t) -> double {
        auto a0 = constant_term.is_initialized() ? 1 / constant_term.value() : 0.0;
      ...
      Logger::Error(std::runtime_error(
          "IfcCosineSpiral cannot be used for vertical alignment"));
      parent_curve_fn_ = std::make_shared<parent_curve_function>(
          [](double /*u*/) -> Eigen::Matrix4d {
            return Eigen::Matrix4d::Identity();
          },
  ...
  void operator()(const IfcSchema::IfcSineSpiral *c) {
    ...
    if (linear_term.is_initialized()) {
      linear_term.value() *= length_unit_;
    }
    auto sine_term = c->SineTerm() * length_unit_;
    auto L = length(); // already converted to internal units by constructor
    if (segment_type_ == ST_HORIZONTAL) {
      auto theta = [constant_term, linear_term, sine_term,
                    L](double t) -> double {
        auto a0 = constant_term.is_initialized() ? t / constant_term.value() : 0.0;
        auto a1 = linear_term.is_initialized()
                      ? (linear_term.value() / fabs(linear_term.value())) *
                            pow(t / linear_term.value(), 2.0) / 2.0
                      : 0.0;
        auto a2 =
            -1.0 * (L / (2 * PI * sine_term)) * (cos(2 * PI * t / L) - 1.0);
        return a0 + a1 + a2;
      };
      auto fn_x = [theta](double t) -> double { return cos(theta(t)); };
      auto fn_y = [theta](double t) -> double { return sin(theta(t)); };
      auto curvature = [constant_term, linear_term, sine_term,
                        L](double t) -> double {
        auto a0 = constant_term.is_initialized() ? L / constant_term.value() : 0.0;
        auto a1 = linear_term.is_initialized()
                      ? (linear_term.value() / fabs(linear_term.value())) *
                            pow(L / linear_term.value(), 2.0) * (t / L)
                      : 0.0;
      ...
      auto cant = [constant_term, linear_term, sine_term,
                   L](double t) -> double {
        auto a0 = constant_term.is_initialized() ? 1 / constant_term.value() : 0.0;
        auto a1 = linear_term.is_initialized()
                      ? (linear_term.value() / fabs(linear_term.value())) *
                            pow(1 / linear_term.value(), 2.0) * t
                      : 0.0;
      ...
      if (!slope.is_initialized()) {
        slope = [linear_term, sine_term, L](double t) -> double {
          auto a1 = linear_term.is_initialized()
                        ? sign(linear_term.value()) *
                              pow(L / linear_term.value(), 2.0) * (1.0 / L)
                        : 0.0;
      ...
      Logger::Error(std::runtime_error(
          "IfcSineSpiral cannot be used for vertical alignment"));
      parent_curve_fn_ = std::make_shared<parent_curve_function>(
          [](double /*u*/) -> Eigen::Matrix4d {
            return Eigen::Matrix4d::Identity();
          },
  ...
  void polynomial_spiral(boost::optional<double> A0, boost::optional<double> A1,
                         boost::optional<double> A2, boost::optional<double> A3,
                         boost::optional<double> A4, boost::optional<double> A5,
                         boost::optional<double> A6,
                         boost::optional<double> A7) {
    auto theta = [A0, A1, A2, A3, A4, A5, A6, A7, start = start_ * length_unit_,
                  lu = length_unit_](double t) -> double {
      auto a0 = A0.get_value_or(0.0) != 0.0 ? t / (A0.value() * lu) : 0.0;
      auto a1 = A1.get_value_or(0.0) != 0.0
                    ? A1.value() * lu * std::pow(t, 2) /
                          (2 * fabs(std::pow(A1.value() * lu, 3)))
                    : 0.0;
      auto a2 = A2.get_value_or(0.0) != 0.0
                    ? std::pow(t, 3) / (3 * std::pow(A2.value() * lu, 3))
                    : 0.0;
      auto a3 = A3.get_value_or(0.0) != 0.0
                    ? A3.value() * lu * std::pow(t, 4) /
                          (4 * fabs(std::pow(A3.value() * lu, 5)))
                    : 0.0;
      auto a4 = A4.get_value_or(0.0) != 0.0
                    ? std::pow(t, 5) / (5 * std::pow(A4.value() * lu, 5))
                    : 0.0;
      auto a5 = A5.get_value_or(0.0) != 0.0
                    ? A5.value() * lu * std::pow(t, 6) /
                          (6 * fabs(std::pow(A5.value() * lu, 7)))
                    : 0.0;
      auto a6 = A6.get_value_or(0.0) != 0.0
                    ? std::pow(t, 7) / (7 * std::pow(A6.value() * lu, 7))
                    : 0.0;
      auto a7 = A7.get_value_or(0.0) != 0.0
                    ? A7.value() * lu * std::pow(t, 8) /
                          (8 * fabs(std::pow(A7.value() * lu, 9)))
                    : 0.0;
    ...
    auto curvature = [A0, A1, A2, A3, A4, A5, A6, A7, start = start_,
                      L = length_, lu = length_unit_,
                      length = length_](double t) -> double {
      t += start;
      auto a0 = A0.get_value_or(0.0) != 0.0 ? 1 / (A0.value() * lu) : 0.0;
      auto a1 = A1.get_value_or(0.0) != 0.0
                    ? A1.value() * lu * t / fabs(std::pow(A1.value() * lu, 3))
                    : 0.0;
      auto a2 = A2.get_value_or(0.0) != 0.0
                    ? std::pow(t, 2) / std::pow(A2.value() * lu, 3)
                    : 0.0;
      auto a3 = A3.get_value_or(0.0) != 0.0
                    ? A3.value() * lu * std::pow(t, 3) /
                          fabs(std::pow(A3.value() * lu, 5))
                    : 0.0;
      auto a4 = A4.get_value_or(0.0) != 0.0
                    ? std::pow(t, 4) / std::pow(A4.value() * lu, 5)
                    : 0.0;
      auto a5 = A5.get_value_or(0.0) != 0.0
                    ? A5.value() * lu * std::pow(t, 5) /
                          fabs(std::pow(A5.value() * lu, 7))
                    : 0.0;
      auto a6 = A6.get_value_or(0.0) != 0.0
                    ? std::pow(t, 6) / std::pow(A6.value() * lu, 7)
                    : 0.0;
      auto a7 = A7.get_value_or(0.0) != 0.0
                    ? A7.value() * lu * std::pow(t, 7) /
                          fabs(std::pow(A7.value() * lu, 9))
                    : 0.0;
  ...
  void
  polynomial_cant_spiral(boost::optional<double> A0, boost::optional<double> A1,
                         boost::optional<double> A2, boost::optional<double> A3,
                         boost::optional<double> A4, boost::optional<double> A5,
                         boost::optional<double> A6,
                         boost::optional<double> A7) {
    ...
    auto cant = [A0, A1, A2, A3, A4, A5, A6, A7, start = start_, L = length_,
                 lu = length_unit_, length = length_](double t) -> double {
      t += start;
      auto a0 = A0.get_value_or(0.0) != 0.0 ? 1 / (A0.value() * lu) : 0.0;
      auto a1 = A1.get_value_or(0.0) != 0.0
                    ? A1.value() * lu * t / fabs(std::pow(A1.value() * lu, 3))
    ...
    if (!slope.is_initialized()) {
      slope = [A1, A2, A3, A4, A5, A6, A7, start = start_, L = length_,
               lu = length_unit_, length = length_](double t) -> double {
        t += start;
        auto a1 = A1.get_value_or(0.0) != 0.0
                      ? A1.value() * lu / fabs(std::pow(A1.value() * lu, 3))
                      : 0.0;
        auto a2 = A2.get_value_or(0.0) != 0.0
                      ? 2 * t / std::pow(A2.value() * lu, 3)
                      : 0.0;
        auto a3 = A3.get_value_or(0.0) != 0.0
                      ? 3 * A3.value() * lu * std::pow(t, 2) /
                            fabs(std::pow(A3.value() * lu, 5))
                      : 0.0;
        auto a4 = A4.get_value_or(0.0) != 0.0
                      ? 4 * std::pow(t, 3) / std::pow(A4.value() * lu, 5)
                      : 0.0;
        auto a5 = A5.get_value_or(0.0) != 0.0
                      ? 5 * A5.value() * lu * std::pow(t, 4) /
                            fabs(std::pow(A5.value() * lu, 7))
                      : 0.0;
        auto a6 = A6.get_value_or(0.0) != 0.0
                      ? 6 * std::pow(t, 5) / std::pow(A6.value() * lu, 7)
                      : 0.0;
        auto a7 = A7.get_value_or(0.0) != 0.0
                      ? 7 * A7.value() * lu * std::pow(t, 6) /
                            fabs(std::pow(A7.value() * lu, 9))
                      : 0.0;
  ...
  void operator()(const IfcSchema::IfcCircle *c) {
    if (segment_type_ == ST_HORIZONTAL || segment_type_ == ST_VERTICAL) {
      ...
      auto parent_curve_position =
          taxonomy::cast<taxonomy::matrix4>(mapping_->map(c->Position()))
              ->ccomponents();
      ...
      // sweep angle from the parent curve X-axis to the first point on the
      // trimmed curve
      ...
      if (segment_type_ == ST_HORIZONTAL) {
        ...
        auto curve_segment_placement =
            taxonomy::cast<taxonomy::matrix4>(mapping_->map(inst_->Placement()))
                ->ccomponents();
        ...
        convert_u = [csStartX, csStartY, csCenterX, csCenterY, R,
                     sign_l](double u) {
          // for vertical, u is measured along the horizonal but we need it to
          // be an arc length

          // x and y are coordinates on the curve segment for horizontal
          // distance u from the start point u is a horizontal distance so x =
          // csStartX + u Recognizing the triangle R^2 = (u + csStartX -
          // csCenterX)^2 + (y - csCenterY)^2 solve for y (y - csCenterY) =
          // sqrt( R^2 - (u + csStartX - csCenterX)^2 ) y = csCenterY + sqrt(
          // R^2 - (u + csStartX - csCenterX)^2 )
          auto x = csStartX + u;
          auto y = csCenterY -
                   sign_l * sqrt(pow(R, 2) - pow(u + csStartX - csCenterX, 2));
      ...
      parent_curve_fn_ = std::make_shared<circle_parent_curve>(
          [segment_type = segment_type_, R, pcCenterX, pcCenterY, start_angle,
           sign_l, convert_u](double u) -> Eigen::Matrix4d {
            u = convert_u(u);
      ...
      if (segment_type_ == ST_HORIZONTAL) {
        ...
        // For vertical circle "u" is a horizontal measurement and it needs to
        // be converted to a distance along the circle. However, when evaluating
        // the start point, start_ is distance along. parent_curve_fn_ will call
        // it's convert_u method which would be wrong in this case. For this
        // reason, the start point of the parent curve is explicitly computed
        // here. This code is a little redundant with parent_curve_fn_.
      ...
      Logger::Warning(
          std::runtime_error("Use of IfcCircle for cant is not supported"));
      parent_curve_fn_ = std::make_shared<parent_curve_function>(
          [](double /*u*/) -> Eigen::Matrix4d {
            return Eigen::Matrix4d::Identity();
          },
  ...
  void operator()(const IfcSchema::IfcLine *l) {
    ...
    // 8.9.3.75 IfcVector
    // https://standards.buildingsmart.org/IFC/RELEASE/IFC4_3/HTML/lexical/IfcVector.htm
    // 8.9.3.30 IfcDirection
    // https://standards.buildingsmart.org/IFC/RELEASE/IFC4_3/HTML/lexical/IfcDirection.htm
    // "The IfcDirection does not imply a vector length, and the direction
    // ratios does not have to be normalized."
    //
    // Therefore, the direction ratios need to be normalized to compute points
    // on the line.
    //
    // Magnitude is not used because it relates to the parameterization of the
    // line, which isn't currently done for IfcCurveSegment
    // @todo - parameterization was recently added so Magnitude needs to be
    // taking into consideration
    auto dr = l->Dir()->Orientation()->DirectionRatios();

    // normalize the direction ratios
    double m_squared =
        std::inner_product(dr.begin(), dr.end(), dr.begin(), 0.0);
    double m = sqrt(m_squared);
    std::transform(dr.begin(), dr.end(), dr.begin(),
                   [m](auto &d) { return d / m; });
    auto pcDx = dr[0];
    auto pcDy = dr[1];

    if (segment_type_ == ST_VERTICAL && curve_segment_placement_) {
      // the general algorithm for mapping parent curve onto curve segment
      // doesn't exactly work for IfcLine. This is easily overcome by using the
      // curve segment placement for the IfcLine direction
      pcDx = (*curve_segment_placement_)(0, 0);
      pcDy = (*curve_segment_placement_)(1, 0);

      // projected length along the x-axis is the 'i' component of the total
      // length
      projected_length_ = length_ * pcDx;
    }

    if (segment_type_ == ST_HORIZONTAL || segment_type_ == ST_VERTICAL ||
        segment_type_ == ST_CANT) {
      std::function<double(double)> convert_u;
      ...
      Logger::Warning(
          std::runtime_error("Unexpected segment type encountered"));
      parent_curve_fn_ = std::make_shared<parent_curve_function>(
          [](double /*u*/) -> Eigen::Matrix4d {
            return Eigen::Matrix4d::Identity();
          },
  ...
  void operator()(const IfcSchema::IfcPolynomialCurve *pc) {
    // see
    // https://forums.buildingsmart.org/t/ifcpolynomialcurve-clarification/4716
    // for discussion on IfcPolynomialCurve
    ...
    if (!coeffZ.empty()) {
      Logger::Warning("Expected IfcPolynomialCurve.CoefficientsZ to be "
                      "undefined for alignment geometry. Coefficients ignored.",
                      pc);
    }
    ...
    if (segment_type_ == ST_HORIZONTAL || segment_type_ == ST_VERTICAL) {
      ...
      // There is one significant difference between IfcPolynomialCurve used for
      // horizontal and vertical alignments. For horizontal alignment, u is the
      // distance along the curve. For vertical alignment, u is the horizontal
      // distance. From 4.2.2.2.8 the polynomial curve equation is in the form
      // of y = Ax^3 for horizontal parabolic transition segments. To evaluate
      // the horizontal function, the value of x that corresponds to the
      // distance along the curve u is needed. This is what the convert_u
      // functor does. For vertical curves, the convert_u functor simply returns
      // x = u.
      std::function<double(double)> convert_u;

      // A numerical method is required to return the x-coordinate that
      // corresponds to a 'u' value (distance along the curve)
      ...
      auto curve_length_fn = [df](double x) -> double {
        auto fs = [df](double x) -> double { return sqrt(pow(df(x), 2) + 1); };
        auto s = boost::math::quadrature::trapezoidal(fs, 0.0, x);
        return s;
      };
      // There isn't a closed form solution to get x that corresponds to a
      // distance along the curve, u A numerical solution is required. This
      // functor finds the value of x such that s(x) - u = 0, where u is the
      // input value and s is the computed curve length.
      x_at_dist_along = [curve_length_fn](double u) -> double {
        ...
        try {
          ...
          auto f = [curve_length_fn, u](double x) -> double {
            return curve_length_fn(x) - u;
          };
          // use a root finder to get x
          auto result = boost::math::tools::bracket_and_solve_root(
              f, x, 2.0, true, tol, max_iter);
          x = result.first;
        } catch (...) {
      ...
      parent_curve_fn_ = std::make_shared<polynomial_parent_curve>(
          [start = start_, lu = length_unit_, coeffX, coeffY,
           convert_u](double u) -> Eigen::Matrix4d {
            auto x = convert_u(u + start); // find x for u
            // evaluate the polynomial at x
            std::array<const std::vector<double> *, 2> coefficients{&coeffX,
                                                                    &coeffY};
            std::array<double, 2> position{0.0, 0.0}; // = SUM(coeff*u^pos)
            std::array<double, 2> slope{
                0.0, 0.0}; // slope is derivative of the curve = SUM(
                           // coeff*pos*u^(pos-1) )
          ...
          [start = start_, lu = length_unit_, coeffX, coeffY,
           convert_u](double u) -> Eigen::Matrix4d {
            auto x = convert_u(u + start); // find x for u
            Eigen::Matrix4d c = Eigen::Matrix4d::Zero();
            c(3, 0) = coeffY[2]; // this may need a unit conversion (also assume
                                 // there is only 3 coefficients)
            return c;
          });

      parent_curve_start_point_ = (*parent_curve_fn_)(
          0.0); // start is added to u in parent_curve_fn_, so use 0.0 here
    } else if (segment_type_ == ST_CANT) {
      Logger::Warning(std::runtime_error(
...
taxonomy::ptr mapping::map_impl(const IfcSchema::IfcCurveSegment *inst) {
  ...
  boost::mpl::for_each<curve_seg_types, boost::type<boost::mpl::_>>(
      std::ref(cse));
