...
taxonomy::ptr
mapping::map_impl(const IfcSchema::IfcCenterLineProfileDef *inst) {
  return nullptr;
  ...
  */
  ...
  if (!exp.More()) {
          ...
          Handle(Geom_OffsetCurve) c1 = new Geom_OffsetCurve(trim,  d,
  gp::DZ()); Handle(Geom_OffsetCurve) c2 = new Geom_OffsetCurve(trim, -d,
  gp::DZ());
  ...
  offset(BRepBuilderAPI_MakeFace(gp_Pln(gp::Origin(), gp::DZ())));
          offset.AddWire(wire);
          offset.Perform(d);
