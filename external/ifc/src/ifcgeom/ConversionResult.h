...
struct EdgeKey {
  ...
  EdgeKey(int a, int b) : ov1(a), ov2(b) {
    if (a < b) {
      v1 = a;
      v2 = b;
    } else {
...
template <typename T> constexpr T add_(T a, T b) { return a + b; }

template <typename T> constexpr T subtract_(T a, T b) { return a - b; }

template <typename T> constexpr T multiply_(T a, T b) { return a * b; }

template <typename T> constexpr T divide_(T a, T b) { return a / b; }

template <typename T> constexpr bool equals_(T a, T b) { return a == b; }

template <typename T> constexpr bool less_than_(T a, T b) { return a < b; }

template <typename T> constexpr T negate_(T a) { return -a; }
...
// @todo this can simply be a template class, to remove the need for the
// NumberEpeck in CGAL kernel.
...
private:
  ...
  template <double (*Fn)(double)> OpaqueNumber *unary_op() const {
    return new NumberNativeDouble(Fn(value_));
  }

public:
  NumberNativeDouble(double v) : value_(v) {}

  virtual double to_double() const { return value_; }
  ...
  virtual OpaqueNumber *clone() const { return new NumberNativeDouble(value_); }
};

template <size_t N> struct IFC_GEOM_API OpaqueCoordinate {
private:
  std::array<OpaqueNumber *, N> values;

  static void copy_(std::array<OpaqueNumber *, N> &dest,
                    const std::array<OpaqueNumber *, N> &src) {
...
public:
  template <typename... Args> OpaqueCoordinate(Args... args) {
...
public:
  ...
  Triangulate(ifcopenshell::geometry::Settings settings,
              ...
              Representation::Triangulation *t, int item_id,
              int surface_style_id) const = 0;
  IfcGeom::Representation::Triangulation *
  Triangulate(const ifcopenshell::geometry::Settings &settings) const;
  ...
  virtual void map(const std::vector<OpaqueCoordinate<4>> &from,
                   const std::vector<OpaqueCoordinate<4>> &to) = 0;
  virtual ConversionResultShape *
      moved(ifcopenshell::geometry::taxonomy::matrix4::ptr) const = 0;
...
public:
  ConversionResult(int id,
                   ifcopenshell::geometry::taxonomy::matrix4::ptr placement,
  ...
  ConversionResult(int id, ConversionResultShape *shape,
                   ifcopenshell::geometry::taxonomy::style::ptr style)
      : id(id), placement_(ifcopenshell::geometry::taxonomy::make<
                           ifcopenshell::geometry::taxonomy::matrix4>()),
        shape_(shape), style_(style) {}
  ConversionResult(int id, ConversionResultShape *shape)
      : id(id), placement_(ifcopenshell::geometry::taxonomy::make<
  ...
  ifcopenshell::geometry::taxonomy::matrix4::ptr Placement() const {
    return placement_;
  }
  bool hasStyle() const { return !!style_; }
  const ifcopenshell::geometry::taxonomy::style &Style() const {
    return *style_;
  }
  ifcopenshell::geometry::taxonomy::style::ptr StylePtr() const {
    return style_;
  }
  void setStyle(ifcopenshell::geometry::taxonomy::style::ptr newStyle) {
    style_ = newStyle;
  }
...
// bool flatten_shape_list(const IfcGeom::ConversionResults& shapes,
// TopoDS_Shape& result, bool fuse, double tol);

// Function to find boundary loops from triangles
template <typename NT>
std::vector<std::vector<int>>
find_boundary_loops(const std::vector<NT> &positions,
