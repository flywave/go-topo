...
ifcopenshell::geometry::Converter::Converter(
    IfcParse::IfcFile *file, ifcopenshell::geometry::Settings &s) {
  mapping_ = impl::mapping_implementations().construct(file, s);
  kernel_ = kernels::construct(file, mapping_->settings());
...
namespace {
void substitute_with_box_based_on_density(IfcGeom::ConversionResults &items,
                                          double &density) {
  int nv = 0;
  void *box = nullptr;
  double volume = 0.;
  ...
  if (density > 1e5) {
    ...
    Logger::Notice("Substituted element with " +
                   boost::lexical_cast<std::string>(density) +
                   " vertices / m3 with a bounding box");
  }
}
} // namespace

IfcGeom::BRepElement *
ifcopenshell::geometry::Converter::create_brep_for_representation_and_product(
    taxonomy::ptr representation_node, const IfcUtil::IfcBaseEntity *product,
    const taxonomy::matrix4::ptr &place_) {
  std::stringstream representation_id_builder;

  auto place = place_;

  representation_id_builder
      << representation_node->instance->as<IfcUtil::IfcBaseEntity>()->id();
  ...
  if (settings_.get<ifcopenshell::geometry::settings::ApplyLayerSets>().get()) {
    ...
    = product->HasAssociations(); for (IfcSchema::IfcRelAssociates::list::it it
    = associations->begin(); it != associations->end(); ++it) {
                                    IfcSchema::IfcRelAssociatesMaterial*
    associates_material = (**it).as<IfcSchema::IfcRelAssociatesMaterial>(); if
    (associates_material) { unsigned layerset_id =
    associates_material->RelatingMaterial()->data().id();
                                            representation_id_builder <<
    "-layerset-" << layerset_id; break;
                                    }
                            }

                            if (styles.size() > 1) {
                                    // If there's only a single layer there is
    no need to manipulate geometries. bool success = true; if
    (product->as<IfcSchema::IfcWall>() &&
    fold_layers(product->as<IfcSchema::IfcWall>(), shapes, layers, thickness,
    folded_layers)) { if (util::apply_folded_layerset(shapes, folded_layers,
    styles, shapes2, getValue(GV_PRECISION))) { std::swap(shapes, shapes2);
                                                    success = true;
                                            }
                                    } else {
                                            if (util::apply_layerset(shapes,
    layers, styles, shapes2, getValue(GV_PRECISION))) { std::swap(shapes,
    shapes2); success = true;
                                            }
                                    }
  ...
  if (settings_.get<ifcopenshell::geometry::settings::ForceSpaceTransparency>()
          .has() &&
      product->declaration().is("IfcSpace")) {
    for (auto &s : shapes) {
      if (s.hasStyle()) {
        // @todo the uglyness
        const_cast<taxonomy::style *>(&*s.StylePtr())->transparency =
            settings_
                .get<ifcopenshell::geometry::settings::ForceSpaceTransparency>()
                .get();
      }
    }
  }
  ...
  int parent_id = -1;
  try {
    IfcUtil::IfcBaseEntity *parent_object =
        mapping_->get_decomposing_entity(product);
    if (parent_object) {
  ...
  if (!settings_
           .get<ifcopenshell::geometry::settings::DisableOpeningSubtractions>()
           .get() &&
    ...
    try {
      ...
      std::transform(
          openings->begin(), openings->end(), std::back_inserter(opening_items),
          [this](IfcUtil::IfcBaseClass *opening) {
            auto prod_item = mapping()->map(opening);
            auto repr = mapping()->representation_of(
                opening->as<IfcUtil::IfcBaseEntity>());
            if (repr) {
              return std::make_pair(
                  mapping()->map(repr),
                  *taxonomy::cast<taxonomy::geom_item>(prod_item)->matrix);
            } else {
      ...
      opening_items.erase(
          std::remove_if(
              opening_items.begin(), opening_items.end(),
              [](const std::pair<taxonomy::ptr, taxonomy::matrix4> &p) {
                return !p.first;
              }),
          opening_items.end());
      ...
      if (opening_items.empty()) {
        ...
        kernel_->convert_openings(product, opening_items, shapes, *place,
                                  opened_shapes);
      }
    } catch (const std::exception &e) {
      Logger::Message(Logger::LOG_ERROR,
                      std::string("Error processing openings for: ") +
                          e.what() + ":",
                      product);
      caught_error = true;
    } catch (...) {
      Logger::Message(Logger::LOG_ERROR,
                      "Error processing openings for:", product);
    }

    if (!(caught_error && opened_shapes.size() < shapes.size())) {
      if (settings_.get<ifcopenshell::geometry::settings::UseWorldCoords>()
              .get()) {
        for (auto it = opened_shapes.begin(); it != opened_shapes.end(); ++it) {
          it->prepend(place);
        }
        place = ifcopenshell::geometry::taxonomy::make<
            ifcopenshell::geometry::taxonomy::matrix4>();
        representation_id_builder << "-world-coords";
      }
      shapes = opened_shapes;
    }
  } else if (settings_.get<ifcopenshell::geometry::settings::UseWorldCoords>()
                 .get()) {
    for (auto it = shapes.begin(); it != shapes.end(); ++it) {
      it->prepend(place);
    }
  ...
  shape = new IfcGeom::Representation::BRep(
      settings_, product_type, representation_id_builder.str(), shapes);

  std::string context_string = "";

  // IfcShapeRepresentation.
  const IfcUtil::IfcBaseEntity *representation =
      representation_node->instance->as<IfcUtil::IfcBaseEntity>();
  auto representation_identifier =
      representation->get("RepresentationIdentifier");
  if (!representation_identifier.isNull()) {
    context_string = (std::string)representation_identifier;
  } else {
    IfcUtil::IfcBaseClass *context =
        (IfcUtil::IfcBaseClass *)representation->get("ContextOfItems");
    auto context_type =
        context->as<IfcUtil::IfcBaseEntity>()->get("ContextType");
  ...
  auto elem = new IfcGeom::BRepElement(
      product->id(), parent_id, name, product_type, guid, context_string, place,
      boost::shared_ptr<IfcGeom::Representation::BRep>(shape), product);

  return elem;
}

IfcGeom::BRepElement *
ifcopenshell::geometry::Converter::create_brep_for_processed_representation(
    const IfcUtil::IfcBaseEntity *product, const taxonomy::matrix4::ptr &place,
  ...
  return new IfcGeom::BRepElement(product->id(), parent_id, name, product_type,
                                  guid, context_string, place,
                                  brep->geometry_pointer(), product);
}
...
ifcopenshell::geometry::Converter::create_brep_for_representation_and_product(
  ...
  return create_brep_for_representation_and_product(
      interpreted_representation, product,
      taxonomy::cast<taxonomy::geom_item>(mapping_->map(product))->matrix);
}

IfcGeom::ConversionResults
ifcopenshell::geometry::Converter::convert(IfcUtil::IfcBaseClass *item) {
