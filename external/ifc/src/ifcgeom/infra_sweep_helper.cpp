...
taxonomy::loft::ptr
ifcopenshell::geometry::make_loft(const Settings &settings_,
                                  const IfcUtil::IfcBaseClass *inst,
                                  const taxonomy::function_item::ptr &fn,
  ...
  // @todo currently only the case is handled where directrix returns a
  // function_item
  // @todo this "if" statement is not really required because the function
  // returns at the start if the Directrix is not a function_item function
  if (fn) {
    ...
    if (end - start < 1.e-9) {
      Logger::Warning(
          "Empty sweep domain with start at " +
              std::to_string(cross_sections.front().dist_along) + " end at " +
              std::to_string(cross_sections.back().dist_along) +
              " and curve domain length " + std::to_string(fn->length()),
          inst);
      return nullptr;
    }
    ...
    auto param_type =
        settings_.get<ifcopenshell::geometry::settings::FunctionStepType>()
            .get();
    auto param =
        settings_.get<ifcopenshell::geometry::settings::FunctionStepParam>()
            .get();
    size_t num_steps = 0;
    if (param_type ==
    ...
    for (size_t i = 0; i <= num_steps; ++i) {
      ...
      auto relative_dist_along = (dist_along - *profile_index) /
                                 (*(profile_index + 1) - *profile_index);
      const auto &profile_a =
          cross_sections[std::distance(longitudes.begin(), profile_index)]
              .section_geometry;
      const auto &offset_a =
          cross_sections[std::distance(longitudes.begin(), profile_index)]
              .offset;
      const auto &rotation_a =
          cross_sections[std::distance(longitudes.begin(), profile_index)]
              .rotation;
      ...
      //  - we're not exactly at the location of the current profile or whether
      //  there is an offset involved
      bool should_interpolate =
          (profile_index + 1 < longitudes.end()) &&
          (relative_dist_along >= 1.e-9 ||
           offset_a.cwiseAbs().maxCoeff() > 0. || rotation_a);
      ...
      if (should_interpolate) {
        ...
        // Only interpolate if the profiles are different or either of the
        // offsets is non-zero
        bool should_interpolate2 =
            (profile_a->instance != profile_b->instance) ||
            (offset_a.cwiseAbs().maxCoeff() > 0. ||
             offset_b.cwiseAbs().maxCoeff() > 0. || rotation_b);
        ...
        if (should_interpolate2) {
          ...
          if (profile_a->kind() == taxonomy::FACE) {
            ...
            auto profile_a_f =
                std::static_pointer_cast<taxonomy::face>(profile_a);
            auto profile_b_f =
                std::static_pointer_cast<taxonomy::face>(profile_b);

            if (profile_a_f->children.size() != profile_b_f->children.size()) {
              Logger::Warning("Mismatching number of face boundaries: " +
                                  std::to_string(profile_a_f->children.size()) +
                                  " vs " +
          ...
          // @todo should_interpolate should also be informed based by different
          // face matrices.
          if (profile_a->matrix || profile_b->matrix) {
            ...
            interpolated->matrix->components() =
                lerp(m4a, m4b, relative_dist_along);
          }

          auto interpolated_offset =
              lerp(offset_a, offset_b, relative_dist_along);
          if (rotation_a && rotation_b) {
            // @todo we don't support an overridden rotation on only one of the
            // placements in which case we would need to lerp with the rotation
            // component below in m4b.
            interpolated_rotation =
                lerp(*rotation_a, *rotation_b, relative_dist_along);
          } else {
            Logger::Error("Direction vectors on cross section placements only "
                          "supported when used consistently");
          }
          ...
          for (auto tmp_ : boost::combine(loops_a, loops_b)) {
            ...
            for (auto tmp__ : boost::combine(w1->children, w2->children)) {
              ...
              auto p3 = (lerp(p1->ccomponents(), p2->ccomponents(),
                              relative_dist_along) +
                         interpolated_offset)
                            .eval();
            ...
            if (interpolated->kind() == taxonomy::FACE) {
              std::static_pointer_cast<taxonomy::face>(interpolated)
                  ->children.push_back(interpolated_loop);
            } else {
              std::static_pointer_cast<taxonomy::loop>(interpolated)->children =
                  interpolated_loop->children;
            }
          }
        }
      }
      ...
      /* {
              std::wcout << "#" << pwf->instance->data().id() << " " <<
      dist_along << ": " << m4.col(3).row(2).value() << std::endl;
      }*/

      Eigen::Matrix4d m4b = Eigen::Matrix4d::Identity();
      if (interpolated_rotation) {
        // direction vectors on the linear placement overwrite the placement
        // otherwise inferred from the tangent
      ...
      if (interpolated) {
        ...
        if (profile_a->kind() == taxonomy::FACE) {
          loft->children.push_back(std::static_pointer_cast<taxonomy::face>(
              taxonomy::item::ptr(profile_a->clone_())));
        } else {
          loft->children.push_back(std::static_pointer_cast<taxonomy::loop>(
              taxonomy::item::ptr(profile_a->clone_())));
        }
        if (profile_a->matrix) {
          loft->children.back()->matrix =
              taxonomy::matrix4::ptr(profile_a->matrix->clone_());
        }
      }
      if (!loft->children.back()->matrix) {
        // @todo should this not be initialized by default? matrix4 already has
        // a 'lazy identity' mechanism.
