...
template <typename T> void istream_helper(std::istream &in, T &vs) {
  std::string tokens;
  in >> tokens;
...
std::istream &
ifcopenshell::geometry::settings::operator>>(std::istream &in,
                                             IteratorOutputOptions &ioo) {
  std::string token;
  in >> token;
