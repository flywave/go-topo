...
public:
  explicit Representation(const ifcopenshell::geometry::Settings &settings,
                          const std::string &entity, const std::string &id)
      : settings_(settings), entity_(entity), id_(id) {}
  const ifcopenshell::geometry::Settings &settings() const { return settings_; }
  const std::string &entity() const { return entity_; }
  // id starts with representation id and then it may have the following dash
  // separated elements:
...
public:
  BRep(const ifcopenshell::geometry::Settings &settings,
       const std::string &entity, const std::string &id,
       const IfcGeom::ConversionResults &shapes)
      : Representation(settings, entity, id), shapes_(shapes) {}
  virtual ~BRep() {}
  IfcGeom::ConversionResults::const_iterator begin() const {
    return shapes_.begin();
  }
  IfcGeom::ConversionResults::const_iterator end() const {
    return shapes_.end();
  }
  ...
  bool calculate_projected_surface_area(
      const ifcopenshell::geometry::taxonomy::matrix4 &ax, double &along_x,
...
public:
  ...
  const std::vector<int> &surface_style_ids() const {
    return surface_style_ids_;
  }
...
private:
  ...
  Triangulation(const ifcopenshell::geometry::Settings &settings,
                const std::string &entity, const std::string &id)
      : Representation(settings, entity, id), weld_offset_(0) {}

public:
  const std::vector<double> &verts() const { return verts_; }
  const std::vector<int> &faces() const { return faces_; }
  const std::vector<std::vector<int>> &polyhedral_faces_without_holes() const {
    return polyhedral_faces_without_holes_;
  }
  const std::vector<std::vector<std::vector<int>>> &
  polyhedral_faces_with_holes() const {
    return polyhedral_faces_with_holes_;
  }
  ...
  Triangulation(const ifcopenshell::geometry::Settings &settings,
                const std::string &entity, const std::string &id,
                const std::vector<double> &verts, const std::vector<int> &faces,
      ...
      : Representation(settings, entity, id), verts_(verts), faces_(faces),
        edges_(edges), normals_(normals), uvs_(uvs),
        material_ids_(material_ids), materials_(materials), item_ids_(item_ids),
  ...
  box_project_uvs(const std::vector<double> &vertices,
                  const std::vector<double> &normals);

  static Triangulation *
  empty(const ifcopenshell::geometry::Settings &settings) {
    return new Triangulation(settings, "", "");
  }

  /// Welds vertices that belong to different faces
  int addVertex(int item_index, int material_index, double X, double Y,
                double Z);
  ...
  void addFace(int item_id, int style,
  ...
  void registerEdgeCount(int n1, int n2,
