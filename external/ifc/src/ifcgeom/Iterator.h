...
struct geometry_conversion_result {
  ...
  std::vector<std::pair<const IfcUtil::IfcBaseEntity *,
...
private:
  ...
  typename std::list<IfcGeom::BRepElement *>::const_iterator
  ...
  // The object is fetched beforehand to be sure that get() returns a valid
  // element
  ...
  /// @todo public/private sections all over the place: move all public to the
  /// beginning of the class
public:
  void set_cache(GeometrySerializer *cache) { cache_ = cache; }

  const std::string &unit_name() const {
    return converter_->mapping()->get_length_unit_name();
  }
  double unit_magnitude() const {
    return converter_->mapping()->get_length_unit();
  }
  // Check if error occurred during iterator initialization or iteration over
  // elements.
  bool had_error_processing_elements() const {
    return had_error_processing_elements_;
  }

  boost::optional<bool> initialization_outcome_;

  /**
   * @return Returns true if the iterator is initialized with any elements,
   * false otherwise.
   *
   * @note
   * - A true return value does not guarantee successful initialization of all
   * elements. Some elements may have failed to initialize. Check
   * had_error_processing_elements() to see whether there were errors during the
   * initialization.
   *
   * - For non-concurrent iterators, a false return may occur if initialization
   * of the first element fails, even if subsequent elements could be
  ...
  bool initialize() {
    ...
    if (num_threads_ != 1) {
      // @todo this shouldn't be necessary with properly immutable taxonomy
      // items
    ...
    for (auto &task : reps) {
      ...
      if (!settings_.get<ifcopenshell::geometry::settings::NoParallelMapping>()
               .get()) {
        ...
        std::transform(
            task.products->begin(), task.products->end(),
            std::back_inserter(res.products),
            [this, &res](IfcUtil::IfcBaseClass *prod) {
              auto prod_item = converter_->mapping()->map(prod);
              return std::make_pair(
                  prod->as<IfcUtil::IfcBaseEntity>(),
                  ifcopenshell::geometry::taxonomy::cast<
                      ifcopenshell::geometry::taxonomy::geom_item>(prod_item)
                      ->matrix);
            });
      }
    ...
    for (auto &r : tasks_) {
      num_products +=
          !settings_.get<ifcopenshell::geometry::settings::NoParallelMapping>()
                  .get()
              ? r.products_2->size()
              : r.products.size();
    }
    ...
    // There needs to be two options, mapped item respecting (does that still
    work?), and optimized based on topology sorting.
    ...
    std::transform(products.begin(), products.end(), std::back_inserter(items),
    [this, &placements](IfcUtil::IfcBaseClass* p) { auto item =
    converter_->mapping()->map(p);
            // Product placements do not affect item reuse and should
    temporarily be swapped to identity if (item) { std::swap(placements[item],
    ((taxonomy::geom_ptr)item)->matrix);
            }
            return item;
    });
    ...
    while (it < items.end()) {
            ...
            r.item = *it;
            std::transform(it, jt, std::back_inserter(r.products), [&r,
    &placements](taxonomy::ptr product_node) { return
    std::make_pair((IfcUtil::IfcBaseEntity*) product_node->instance,
    placements[product_node]);
            });
            tasks_.push_back(r);
            it = jt;
    }
    */
    ...
    Logger::Notice(
        "Created " + boost::lexical_cast<std::string>(tasks_.size()) +
        " tasks for " + boost::lexical_cast<std::string>(num_products) +
        " products");
    ...
    if (tasks_.size() == 0) {
      ...
      if (num_threads_ != 1) {
        init_future_ = std::async(std::launch::async,
                                  [this]() { process_concurrently(); });

        // wait for the first element, because after init(), get() can be
        // called. so the element conversion must succeed
  ...
  void process_finished_rep(geometry_conversion_result *rep) {
    ...
    all_processed_elements_.insert(all_processed_elements_.end(),
                                   rep->elements.begin(), rep->elements.end());
    all_processed_native_elements_.insert(all_processed_native_elements_.end(),
                                          rep->breps.begin(), rep->breps.end());
  ...
  void process_concurrently() {
    ...
    for (unsigned i = 0; i < conc_threads; ++i) {
      kernel_pool.push_back(
          new ifcopenshell::geometry::Converter(ifc_file, settings_));
    }
    ...
    for (auto &rep : tasks_) {
      ...
      std::future<geometry_conversion_result *> fu = std::async(
          std::launch::async,
          [this](ifcopenshell::geometry::Converter *kernel,
            ...
            try {
              ...
              Logger::Error(
                  std::string("Exception '") + e.what() +
                      std::string(
                          "' occurred while iterator was creating a shape: "),
                  rep->item->instance);
              had_error_processing_elements_ = true;
            } catch (...) {
              Logger::Error("Unknown exception occurred while iteartor was "
                            "creating a shape: ",
                            rep->item->instance);
              had_error_processing_elements_ = true;
            }
    ...
    if (!terminating_) {
      Logger::Status(
          "\rDone creating geometry (" +
          boost::lexical_cast<std::string>(all_processed_elements_.size()) +
          " objects)							"
          "	");
    }
  }
  ...
  void compute_bounds(bool with_geometry) {
    ...
    if (with_geometry) {
      ...
      do {
        ...
        const IfcGeom::TriangulationElement *o =
        ...
        for (typename std::vector<double>::const_iterator it =
                 mesh.verts().begin();
             it != mesh.verts().end();) {
          const double &x = *(it++);
          const double &y = *(it++);
          ...
          for (int i = 0; i < 3; ++i) {
            bounds_min_.components()(i) =
                std::min(bounds_min_.components()(i), transformed(i));
            bounds_max_.components()(i) =
                std::max(bounds_max_.components()(i), transformed(i));
          }
        }
      ...
      for (auto &r : reps) {
        std::copy(r.products->begin(), r.products->end(),
                  std::back_inserter(products));
      }

      for (auto &product : products) {
        auto prod_item = converter_->mapping()->map(product);
        auto vec = ifcopenshell::geometry::taxonomy::cast<
                       ifcopenshell::geometry::taxonomy::geom_item>(prod_item)
                       ->matrix->translation_part();

        for (int i = 0; i < 3; ++i) {
          bounds_min_.components()(i) =
              std::min(bounds_min_.components()(i), vec(i));
          bounds_max_.components()(i) =
              std::max(bounds_max_.components()(i), vec(i));
        }
      }
    }
  }
  ...
  const ifcopenshell::geometry::taxonomy::point3 &bounds_min() const {
    return bounds_min_;
  }
  const ifcopenshell::geometry::taxonomy::point3 &bounds_max() const {
    return bounds_max_;
  }
...
private:
  ...
  Element *decorate_with_cache_(GeometrySerializer::read_type rt,
                                const std::string &product_guid,
                                const std::string &representation_id, Fn f) {
    ...
    if (cache_) {
      ...
      auto from_cache =
          cache_->read(*ifc_file, product_guid, representation_id, rt);
      if (from_cache) {
  ...
  void create_element_(ifcopenshell::geometry::Converter *kernel,
                       ifcopenshell::geometry::Settings settings,
                       geometry_conversion_result *rep) {
    if (!settings_.get<ifcopenshell::geometry::settings::NoParallelMapping>()
             .get()) {
      ...
      std::transform(
          rep->products_2->begin(), rep->products_2->end(),
          std::back_inserter(rep->products),
          [this, &rep, kernel](IfcUtil::IfcBaseClass *prod) {
    ...
    IfcGeom::BRepElement *brep =
        static_cast<IfcGeom::BRepElement *>(decorate_with_cache_(
            GeometrySerializer::READ_BREP,
            (std::string)product->get("GlobalId"),
            std::to_string(
                rep->item->instance->as<IfcUtil::IfcBaseEntity>()->id()),
            [kernel, settings, product, place, rep]() {
              return kernel->create_brep_for_representation_and_product(
                  rep->item, product, place);
            }));
      ...
      IfcGeom::BRepElement *brep2 =
          static_cast<IfcGeom::BRepElement *>(decorate_with_cache_(
              ...
              [kernel, settings, product2, place2, brep]() {
                return kernel->create_brep_for_processed_representation(
                    product2, place2, brep);
              }));
      if (brep2) {
        auto elem2 = process_based_on_settings(
            settings, brep2,
  ...
  IfcGeom::Element *
  process_based_on_settings(ifcopenshell::geometry::Settings settings,
                            IfcGeom::BRepElement *elem,
                            IfcGeom::TriangulationElement *previous = nullptr) {
    if (settings.get<ifcopenshell::geometry::settings::IteratorOutput>()
            .get() == ifcopenshell::geometry::settings::SERIALIZED) {
      try {
        return new IfcGeom::SerializedElement(*elem);
      } catch (...) {
        Logger::Message(Logger::LOG_ERROR,
                        "Getting a serialized element from model failed.");
        return nullptr;
      }
    } else if (settings.get<ifcopenshell::geometry::settings::IteratorOutput>()
                   .get() == ifcopenshell::geometry::settings::TRIANGULATED) {
      ...
      return decorate_with_cache_(
          GeometrySerializer::READ_TRIANGULATION, elem->guid(), gid2,
          [elem, previous]() {
            try {
              if (!previous) {
                ...
                return new TriangulationElement(*elem,
                                                previous->geometry_pointer());
              }
            } catch (...) {
              Logger::Message(
                  Logger::LOG_ERROR,
                  "Getting a triangulation element from model failed.");
            }
  ...
  void log_timepoints() const {
    ...
    std::array<std::string, 3> labels = {"Initializing mapping"s,
    ...
    for (auto it = time_points.begin() + 1; it != time_points.end(); ++it) {
      ...
      Logger::Notice(labels[std::distance(time_points.begin(), jt)] + " took " +
                     std::to_string(ms_double.count()) + "ms");
    }
  }
...
public:
  ...
  /// Moves to the next shape representation, create its geometry, and returns
  /// the associated product. Use get() to retrieve the created geometry.
  ...
  Element *get() {
    ...
    if (settings_.get<ifcopenshell::geometry::settings::UseElementHierarchy>()
            .get()) {
  ...
  /// Gets the native (Open Cascade or CGAL) representation of the current
  /// geometrical entity.
  BRepElement *get_native() { return *native_task_result_iterator_; }
  ...
  const Element *get_object(int id) {
    ...
    try {
      ...
      auto parent_object =
          converter_->mapping()->get_decomposing_entity(ifc_product);
      if (parent_object) {
      ...
      auto casted =
          mapped ? ifcopenshell::geometry::taxonomy::dcast<
                       ifcopenshell::geometry::taxonomy::geom_item>(mapped)
                 : nullptr;
    ...
    Element *ifc_object =
        new Element(settings_, id, parent_id, product_name, instance_type,
                    product_guid, "", m4, ifc_product);
    return ifc_object;
  }
  ...
  Iterator(const ifcopenshell::geometry::Settings &settings,
           IfcParse::IfcFile *file,
           const std::vector<IfcGeom::filter_t> &filters, int num_threads)
      : settings_(settings), ifc_file(file), filters_(filters),
        num_threads_(num_threads),
        converter_(new ifcopenshell::geometry::Converter(ifc_file, settings_)) {
  }

  Iterator(const ifcopenshell::geometry::Settings &settings,
           IfcParse::IfcFile *file)
      : settings_(settings), ifc_file(file), num_threads_(1),
        converter_(new ifcopenshell::geometry::Converter(ifc_file, settings_)) {
  }

  Iterator(const ifcopenshell::geometry::Settings &settings,
           IfcParse::IfcFile *file, int num_threads)
      : settings_(settings), ifc_file(file), num_threads_(num_threads),
        converter_(new ifcopenshell::geometry::Converter(ifc_file, settings_)) {
  }
