...
namespace IfcGeom {
	...
	public:
		...
		void process_concurrently() {
			...
			for (unsigned i = 0; i < conc_threads; ++i) {
				kernel_pool.push_back(new ifcopenshell::geometry::Converter(ifc_file, settings_));
			}
	...
	public:
		...
		Iterator( const ifcopenshell::geometry::Settings& settings, IfcParse::IfcFile* file, int num_threads)
			: settings_(settings)
			, ifc_file(file)
			, num_threads_(num_threads)
			, converter_(new ifcopenshell::geometry::Converter( ifc_file, settings_))
