...
// The following header entities are not normally encountered in IFC files and
// are not parsed. static const char * const FILE_POPULATION   =
// "FILE_POPULATION"; static const char * const SECTION_LANGUAGE  =
// "SECTION_LANGUAGE"; static const char * const SECTION_CONTEXT   =
// "SECTION_CONTEXT";
...
HeaderEntity::HeaderEntity(const char *const datatype, size_t size,
                           IfcFile *file)
    : datatype_(datatype), file_(file),
      data_(file ? read_from_file(file, size)
                 : IfcEntityInstanceData(storage_t(size))) {}
...
FileDescription::FileDescription(IfcFile *file)
    : HeaderEntity(FILE_DESCRIPTION, 2, file) {}
