...
#define IFCOPENSHELL_VERSION                                                   \
  STRINGIFY(IFCOPENSHELL_BRANCH) "-" STRINGIFY(IFCOPENSHELL_COMMIT)
#else
...
struct Token {
  ...
  Token(IfcSpfLexer *_lexer, unsigned _startPos, unsigned /*_endPos*/,
        TokenType _type)
...
public:
  ...
  /// Returns the token as a string in internal buffer (for optimization
  /// purposes)
  static const std::string &asStringRef(const Token &token);
  /// Returns the token as a string (without the dot or apostrophe)
  static boost::dynamic_bitset<> asBinary(const Token &token);
  /// Returns a string representation of the token (including the dot or
  /// apostrophe)
...
IFC_PARSE_API aggregate_of_instance::ptr
traverse(IfcUtil::IfcBaseClass *instance, int max_level = -1);

IFC_PARSE_API aggregate_of_instance::ptr
traverse_breadth_first(IfcUtil::IfcBaseClass *instance, int max_level = -1);
} // namespace IfcParse

IFC_PARSE_API std::ostream &operator<<(std::ostream &out,
