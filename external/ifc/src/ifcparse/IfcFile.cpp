...
void IfcParse::parse_context::push(Token t) { tokens_.push_back(t); }
...
namespace {
template <typename Variant, typename T> struct is_type_in_variant;
...
struct is_type_in_variant<boost::variant<First, Rest...>, T> {
  static constexpr bool value =
      ...
      is_type_in_variant<boost::variant<Rest...>, T>::value;
};
...
void dispatch_token(int instance_id, int attribute_id, IfcParse::Token t,
                    IfcParse::declaration *decl, Fn fn) {
  if (t.type == IfcParse::Token_BINARY) {
    ...
    if (decl && decl->as_enumeration_type()) {
      try {
        fn(EnumerationReference(
            decl->as_enumeration_type(),
            decl->as_enumeration_type()->lookup_enum_offset(s)));
      } catch (IfcParse::IfcException &e) {
        Logger::Error("An enumeration literal '" + s +
                      "' is not valid for type '" + decl->name() +
                      "' at offset " + std::to_string(t.startPos));
      }
    } else {
      Logger::Error("An enumeration literal '" + s +
                    "' is not expected at attribute index '" +
                    std::to_string(attribute_id) + "' at offset " +
                    std::to_string(t.startPos));
    }
  } else if (t.type == IfcParse::Token_FLOAT) {
    fn(IfcParse::TokenFunc::asFloat(t));
  } else if (t.type == IfcParse::Token_IDENTIFIER) {
    fn(IfcParse::reference_or_simple_type{IfcParse::InstanceReference{
        IfcParse::TokenFunc::asIdentifier(t), t.startPos}});
...
void construct_(int instance_id, int attribute_id, IfcParse::parse_context &p,
                const IfcParse::aggregation_type *aggr, Fn fn) {
  if (p.tokens_.empty()) {
    // @todo instead of ugly if-else we could also default initialize the
    // respective variant types below.
    if (aggr) {
      auto aggr_type = IfcUtil::make_aggregate(
          IfcUtil::from_parameter_type(aggr->type_of_element()));
      if (aggr_type == IfcUtil::Argument_AGGREGATE_OF_INT) {
        ...
        fn(aggregate_of_aggregate_of_instance::ptr(
  ...
  auto append_to_aggregate_storage = [&aggregate_storage](const auto &v) {
    if constexpr (is_type_in_variant_v<
                      possible_aggregation_types_t,
                      std::vector<std::decay_t<decltype(v)>>>) {
      if (aggregate_storage.which() == 0) {
        aggregate_storage = std::vector<std::decay_t<decltype(v)>>{v};
      } else {
        if (auto *vec_ptr = boost::get<std::vector<std::decay_t<decltype(v)>>>(
                &aggregate_storage)) {
          vec_ptr->push_back(v);
        } else {
          if constexpr (std::is_same_v<std::decay_t<decltype(v)>, int>) {
            auto *vec_ptr2 =
                boost::get<std::vector<double>>(&aggregate_storage);
            if (vec_ptr2) {
              // double[] + int
          ...
          if constexpr (std::is_same_v<std::decay_t<decltype(v)>,
                                       std::vector<int>>) {
            auto *vec_ptr2 = boost::get<std::vector<std::vector<double>>>(
                &aggregate_storage);
            if (vec_ptr2) {
              // double[][] + int[]
          ...
          if constexpr (std::is_same_v<std::decay_t<decltype(v)>,
                                       std::vector<double>>) {
            auto *vec_ptr2 =
                boost::get<std::vector<std::vector<int>>>(&aggregate_storage);
            if (vec_ptr2) {
              // int[][] -> double[][] + double[]
          ...
          auto current = boost::apply_visitor(
              [](auto v) {
                if constexpr (!std::is_same_v<decltype(v), Blank>) {
                  return std::string(
                      typeid(typename decltype(v)::value_type).name());
                } else {
          ...
          Logger::Error(
              "Inconsistent aggregate valuation while attempting to append " +
              std::string(typeid(decltype(v)).name()) + " to an aggregate of " +
              current);

          // @todo boolean -> logical upgrade
          // wait a second... there are no aggregate of bool / logical in the
          // schema..
          //
          // if constexpr (std::is_same_v<std::decay_t<decltype(v)>, bool>) {
          //     auto* vec_ptr =
          //     boost::get<std::vector<boost::tribool>(&aggregate_storage);
          //     vec_ptr->push_back(v);
          // }
          // if constexpr (std::is_same_v<std::decay_t<decltype(v)>,
          // boost::tribool>) {
          //     auto* vec_ptr =
          //     boost::get<std::vector<bool>(&aggregate_storage);
          //     std::vector<boost::tribool> ps(vec_ptr->begin(),
          //     vec_ptr->end()); ps.push_back(v); aggregate_storage = ps;
          // }
        }
      }
    } else {
      // @todo would be cool if we can trace this back to file offset
      Logger::Error(std::string("Aggregates of ") + typeid(decltype(v)).name() +
                    " are not supported in the IfcOpenShell parser");
    }
  };

  for (auto &t : p.tokens_) {
    boost::apply_visitor(
        [&aggregate_storage, &append_to_aggregate_storage, aggr, instance_id,
         attribute_id](const auto &v) {
          if constexpr (std::is_same_v<std::decay_t<decltype(v)>,
                                       IfcParse::Token>) {
            // @todo get aggregate of enumeration
            dispatch_token(
                instance_id, attribute_id, v,
                aggr && aggr->type_of_element()->as_named_type()
                    ? aggr->type_of_element()->as_named_type()->declared_type()
                    : nullptr,
                append_to_aggregate_storage);
          } else if constexpr (std::is_same_v<std::decay_t<decltype(v)>,
                                              IfcParse::parse_context *>) {
            // nested list
            if constexpr (Depth < 3) {
              construct_<Depth + 1>(instance_id, attribute_id, *v, nullptr,
                                    append_to_aggregate_storage);
            }
          } else {
...
IfcEntityInstanceData IfcParse::parse_context::construct(
    int name, unresolved_references &references_to_resolve,
    const IfcParse::declaration *decl, boost::optional<size_t> expected_size,
  ...
  if ((decl != nullptr) && (decl->as_type_declaration() != nullptr)) {
    ...
    transient_named_type.reset(
        new IfcParse::named_type(const_cast<IfcParse::declaration *>(decl)));
    ...
    std::transform(entity_attrs.begin(), entity_attrs.end(),
                   std::back_inserter(parameter_types),
                   [](auto *attr) { return attr->type_of_attribute(); });
  }
  ...
  if (((decl != nullptr) && (tokens_.size() != parameter_types.size())) ||
    ...
    Logger::Warning("Expected " + std::to_string(expected) +
                    " attribute values, found " +
                    std::to_string(tokens_.size()) + " for instance #" +
                    std::to_string(name > 0 ? name : 0));
  }
  ...
  for (; it != tokens_.end() &&
         ((decl == nullptr) || kt != parameter_types.end());
       ++it) {
    auto &token = *it;
    // @todo coerce to expected type, e.g empty -> std::vector<int>, bool ->
    // logical
    ...
    boost::apply_visitor(
        [this, &storage, name, &references_to_resolve, index, param_type,
         resolve_reference_index](const auto &v) {
          if constexpr (std::is_same_v<std::decay_t<decltype(v)>,
                                       IfcParse::Token>) {
            dispatch_token(
                name, index, v,
                param_type && param_type->as_named_type()
                    ? param_type->as_named_type()->declared_type()
                    : nullptr,
                [this, &storage, name, &references_to_resolve, index,
                 resolve_reference_index](auto v) {
                  if constexpr (std::is_same_v<
                                    std::decay_t<decltype(v)>,
                                    IfcParse::reference_or_simple_type>) {
                    if (name > 0) {
                      references_to_resolve.push_back(std::make_pair(
                          // @todo previously this was storage but apparently
                          // the pointer is not constant with the moving and
                          // temporary nature maybe it ought to be and in that
                          // case a pointer is more direct
                          MutableAttributeValue{
                              name, resolve_reference_index == -1
                                        ? index
                                        : (uint8_t)resolve_reference_index},
                          v));
                    }
                  } else {
                    storage.set(index, v);
            ...
            if (pt) {
              while (
                  pt->as_named_type() &&
                  pt->as_named_type()->declared_type()->as_type_declaration()) {
                pt = pt->as_named_type()
                         ->declared_type()
                         ->as_type_declaration()
                         ->declared_type();
            ...
            construct_<0>(
                name, index, *v, pt ? pt->as_aggregation_type() : nullptr,
                [this, &storage, name, &references_to_resolve, index,
                 resolve_reference_index](const auto &v) {
                  if constexpr (std::is_same_v<
                                    std::decay_t<decltype(v)>,
