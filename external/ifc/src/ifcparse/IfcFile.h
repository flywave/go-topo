...
private:
  typedef std::map<IfcUtil::IfcBaseClass *, IfcUtil::IfcBaseClass *>
      entity_entity_map_t;
...
public:
  ...
  template <class T> typename T::list::ptr instances_by_type_excl_subtypes() {
    IfcEntityList::ptr untyped_list =
  ...
  IfcEntityList::ptr traverse(IfcUtil::IfcBaseClass *instance,
                              int max_level = -1);

  IfcEntityList::ptr getInverse(int instance_id,
                                const IfcParse::declaration *type,
                                int attribute_index);

  /// Marks entity as modified so that potential cache for it is invalidated.
  /// @todo Currently the whole cache is invalidated. Implement more
  ...
  size_t load(unsigned entity_instance_name, Argument **&attributes,
...
namespace std {
template <>
struct iterator_traits<IFC_NAMESPACE::IfcParse::IfcFile::type_iterator> {
  typedef ptrdiff_t difference_type;
  typedef const IFC_NAMESPACE::IfcParse::declaration *value_type;
  typedef const IFC_NAMESPACE::IfcParse::declaration *&reference;
  typedef const IFC_NAMESPACE::IfcParse::declaration **pointer;
