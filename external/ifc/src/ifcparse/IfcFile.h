...
public:
  file_open_status(file_open_enum error) : error_(error) {}

  operator file_open_enum() const { return error_; }

  file_open_enum value() const { return error_; }

  operator bool() const { return error_ == SUCCESS; }
};
...
typedef boost::variant<InstanceReference, IfcUtil::IfcBaseClass *>
    reference_or_simple_type;
typedef std::list<std::pair<
    MutableAttributeValue,
    boost::variant<reference_or_simple_type,
                   std::vector<reference_or_simple_type>,
                   std::vector<std::vector<reference_or_simple_type>>>>>
    unresolved_references;

struct parse_context {
  std::list<boost::variant<IfcUtil::IfcBaseClass *, Token, parse_context *>>
      tokens_;
...
public:
  ...
  typedef std::map<const IfcParse::declaration *, aggregate_of_instance::ptr>
      entities_by_type_t;
  typedef boost::unordered_map<unsigned int, IfcUtil::IfcBaseClass *>
      entity_by_id_t;
  typedef boost::unordered_map<uint32_t, IfcUtil::IfcBaseClass *>
      entity_by_iden_t;
...
private:
  ...
  // internal_attribute_vector_simple_type_;
...
public:
  ...
  IfcFile(const IfcParse::schema_definition *schema =
              IfcParse::schema_by_name("IFC4"));

  /// Deleting the file will also delete all new instances that were added to
  /// the file (via memory allocation)
  virtual ~IfcFile();
  ...
  template <class T> typename T::list::ptr instances_by_type() {
  ...
  template <class T> typename T::list::ptr instances_by_type_excl_subtypes() {
    aggregate_of_instance::ptr untyped_list =
        instances_by_type_excl_subtypes(&T::Class());
    if (untyped_list) {
      return untyped_list->as<T>();
    }
    return typename T::list::ptr(new typename T::list);
  ...
  instances_by_type_excl_subtypes(const IfcParse::declaration *);
  ...
  /// Returns all entities in the file that match the positional argument.
  aggregate_of_instance::ptr
  instances_by_type_excl_subtypes(const std::string &type);
  ...
  static aggregate_of_instance::ptr traverse(IfcUtil::IfcBaseClass *instance,
                                             int max_level = -1);

  /// Same as traverse() but maintains topological order by using a
  /// breadth-first search
  static aggregate_of_instance::ptr
  traverse_breadth_first(IfcUtil::IfcBaseClass *instance, int max_level = -1);
  ...
  typename T::list::ptr getInverse(int instance_id, int attribute_index) {
    return getInverse(instance_id, &T::Class(), attribute_index)
        ->template as<T>();
  }

  aggregate_of_instance::ptr getInverse(int instance_id,
  ...
  void load(unsigned entity_instance_name, const IfcParse::entity *entity,
            parse_context &, int attribute_index = -1);
  void try_read_semicolon() const;

  void register_inverse(unsigned, const IfcParse::entity *from_entity, Token,
                        int attribute_index);
  void register_inverse(unsigned, const IfcParse::entity *from_entity,
                        IfcUtil::IfcBaseClass *, int attribute_index);
  void unregister_inverse(unsigned, const IfcParse::entity *from_entity,
                          IfcUtil::IfcBaseClass *, int attribute_index);
...
namespace std {
template <> struct iterator_traits<IfcParse::IfcFile::type_iterator> {
