...
#endif
    : stream(0), buffer(0), valid(false), eof(false) {
#ifdef _MSC_VER
...
IfcSpfStream::IfcSpfStream(std::istream &f, int l) : stream(0), buffer(0) {
  eof = false;
  size = l;
  char *buffer_rw = new char[size];
...
IfcSpfStream::IfcSpfStream(void *data, int l) : stream(0), buffer(0) {
  eof = false;
  size = l;
  buffer = (char *)data;
  valid = true;
...
//
char IfcSpfStream::Peek() { return buffer[ptr]; }

//
// Returns the character at specified offset
//
char IfcSpfStream::Read(unsigned int o) { return buffer[o]; }

//
// Returns the cursor position
//
unsigned int IfcSpfStream::Tell() { return ptr; }
...
IfcSpfLexer::~IfcSpfLexer() { delete decoder; }
...
char IfcSpfStream::peek_at(unsigned int local_ptr) { return buffer[local_ptr]; }
...
Token IfcParse::GeneralTokenPtr(IfcSpfLexer *lexer, unsigned start,
  ...
  if (first == '#') {
    ...
    if (ParseBool(tokenStr.c_str(),
                  token.value_bool)) // bool is also enumeration
...
bool TokenFunc::isOperator(const Token &t) { return t.type == Token_OPERATOR; }
...
bool TokenFunc::isString(const Token &t) { return t.type == Token_STRING; }

bool TokenFunc::isEnumeration(const Token &t) {
  return t.type == Token_ENUMERATION || t.type == Token_BOOL;
}

bool TokenFunc::isBinary(const Token &t) { return t.type == Token_BINARY; }

bool TokenFunc::isKeyword(const Token &t) { return t.type == Token_KEYWORD; }

bool TokenFunc::isInt(const Token &t) { return t.type == Token_INT; }

bool TokenFunc::isBool(const Token &t) { return t.type == Token_BOOL; }
...
TokenArgument::TokenArgument(const Token &t) { token = t; }
...
namespace {
template <typename T> class vector_or_array {
...
public:
  vector_or_array(std::vector<T> *vector)
      : vector_(vector), array_(0), size_(0), index_(0) {}

  vector_or_array(Argument **arr, size_t size)
      : vector_(0), array_(arr), size_(size), index_(0) {}
...
//
size_t IfcParse::IfcFile::load(unsigned entity_instance_name,
                               Argument **&attributes, size_t num_attributes) {
...
read_aggregate_of_aggregate_as_vector2(Argument **list, size_t size) {
...
TokenArgument::operator std::string() const {
  return TokenFunc::asString(token);
}
TokenArgument::operator boost::dynamic_bitset<>() const {
  return TokenFunc::asBinary(token);
}
TokenArgument::operator IfcUtil::IfcBaseClass *() const {
  return token.lexer->file->instance_by_id(TokenFunc::asIdentifier(token));
}
unsigned int TokenArgument::size() const { return 1; }
Argument *TokenArgument::operator[](unsigned int /*i*/) const {
  throw IfcException("Argument is not a list of attributes");
}
std::string TokenArgument::toString(bool upper) const {
  if (upper && TokenFunc::isString(token)) {
    return IFC_NAMESPACE::IfcWrite::IfcCharacterEncoder(
        TokenFunc::asString(token));
  } else {
...
IfcEntityInstanceData *IfcParse::read(unsigned int i, IfcFile *f,
  ...
  IfcEntityInstanceData *e =
      new IfcEntityInstanceData(ty, f, i, offset.get_value_or(0));
  return e;
}
...
void IfcParse::IfcFile::unregister_inverse(unsigned id_from,
  ...
  std::vector<unsigned int>::iterator it =
      std::find(ids.begin(), ids.end(), id_from);
  if (it == ids.end()) {
...
//
// Returns the entities of Entity type that have this entity in their
// ArgumentList
//
IfcEntityList::ptr
IfcEntityInstanceData::getInverse(const IfcParse::declaration *type,
...
void IfcEntityInstanceData::load() const {
  ...
  if (n != getArgumentCount()) {
    Logger::Error("Wrong number of attributes on instance with id #" +
...
public:
  add_to_instance_list_visitor(IfcEntityList::ptr &list) : list_(list) {}
...
private:
  ...
  template <typename T> void apply_attribute_(T &t, Argument *attr) const {
    if (!attr) {
      return;
    }
    ...
    if (attr->type() == IfcUtil::Argument_ENTITY_INSTANCE) {
      ...
      for (IfcEntityList::it it = entity_list_attribute->begin();
      ...
      for (IfcEntityListList::outer_it it = entity_list_attribute->begin();
           it != entity_list_attribute->end(); ++it) {
        for (IfcEntityListList::inner_it jt = it->begin(); jt != it->end();
             ++jt) {
          t(*jt);
        }
      }
...
void IfcEntityInstanceData::setArgument(unsigned int i, Argument *a,
                                        IfcUtil::ArgumentType attr_type) {
  if (attributes_ == 0) {
    load();
  }
  ...
  IFC_NAMESPACE::IfcWrite::IfcWriteArgument *copy =
      new IFC_NAMESPACE::IfcWrite::IfcWriteArgument();
  ...
  case IfcUtil::Argument_DERIVED:
    copy->set(IFC_NAMESPACE::IfcWrite::IfcWriteArgument::Derived());
    break;
  ...
  case IfcUtil::Argument_ENUMERATION: {
    ...
    const IfcParse::enumeration_type *enum_type = type()
                                                      ->as_entity()
    ...
    std::vector<std::string>::const_iterator it =
        std::find(enum_type->enumeration_items().begin(),
                  enum_type->enumeration_items().end(), enum_literal);

    if (it == enum_type->enumeration_items().end()) {
      throw IfcParse::IfcException(enum_literal +
                                   " does not name a valid item for " +
                                   enum_type->name());
    }

    copy->set(IFC_NAMESPACE::IfcWrite::IfcWriteArgument::EnumerationReference(
        it - enum_type->enumeration_items().begin(), it->c_str()));
    break;
  }
  ...
  case IfcUtil::Argument_AGGREGATE_OF_ENTITY_INSTANCE: {
    ...
    for (IfcEntityList::it it = instances->begin(); it != instances->end();
         ++it) {
  ...
  case IfcUtil::Argument_AGGREGATE_OF_AGGREGATE_OF_ENTITY_INSTANCE: {
    ...
    for (IfcEntityListList::outer_it it = instances->begin();
         it != instances->end(); ++it) {
  ...
  case IfcUtil::Argument_AGGREGATE_OF_EMPTY_AGGREGATE: {
    IfcUtil::ArgumentType t2 = IfcUtil::from_parameter_type(
  ...
  case IfcUtil::Argument_UNKNOWN:
    throw IfcParse::IfcException(
        std::string("Unknown attribute encountered: '") + a->toString() +
        "' at index '" + boost::lexical_cast<std::string>(i) + "'");
    break;
  }
...
void IfcFile::initialize_(IfcParse::IfcSpfStream *s) {
  ...
  while (!stream->eof) {
    if (token_stream[0].type == IfcParse::Token_IDENTIFIER &&
      ...
      data = new IfcEntityInstanceData(entity_type, this, current_id,
                                       token_stream[2].startPos);
...
public:
  traversal_visitor(std::set<IfcUtil::IfcBaseClass *> &visited,
                    IfcEntityList::ptr &list, int level, int max_level)
...
IfcEntityList::ptr IfcParse::traverse(IfcUtil::IfcBaseClass *instance,
                                      int max_level) {
  std::set<IfcUtil::IfcBaseClass *> visited;
...
IfcEntityList::ptr IfcFile::traverse(IfcUtil::IfcBaseClass *instance,
                                     int max_level) {
  return IfcParse::traverse(instance, max_level);
}

void IfcFile::mark_entity_as_modified(int /*id*/) { by_ref_cached_.clear(); }
...
IfcUtil::IfcBaseClass *IfcFile::addEntity(IfcUtil::IfcBaseClass *entity) {
  ...
  if (parsing_complete_) {
    try {
      ...
      for (IfcEntityList::it it = entity_attributes->begin();
           it != entity_attributes->end(); ++it) {
        if (*it != entity) {
          entity_entity_map_t::iterator mit2 = entity_file_map.find(*it);
          if (mit2 == entity_file_map.end()) {
            entity_file_map.insert(
                entity_entity_map_t::value_type(*it, addEntity(*it)));
          }
        }
      }
    } catch (...) {
      Logger::Message(Logger::LOG_ERROR,
                      "Failed to visit forward references of", entity);
    }
  }
    ...
    for (unsigned i = 0; i < we->getArgumentCount(); ++i) {
      ...
      if (attr_type == IfcUtil::Argument_ENTITY_INSTANCE) {
        ...
        if (boost::math::isnan(conversion_factor)) {
          const std::pair<IfcUtil::IfcBaseClass *, double> this_file_unit =
              getUnit("LENGTHUNIT");
          const std::pair<IfcUtil::IfcBaseClass *, double> other_file_unit =
              other_file->getUnit("LENGTHUNIT");
...
void IfcFile::removeEntity(IfcUtil::IfcBaseClass *entity) {
  ...
  // the loop: boost::shared_ptr<IfcEntityList> entities = ...; for (auto it =
  // entities->end() - 1; it >= entities->begin(); --it) {
  ...
  // weak. std::set<IFC_NAMESPACE::IfcSchema::Type::Enum> weak_roots;
  ...
  if (references) {
    for (IfcEntityList::it iit = references->begin(); iit != references->end();
         ++iit) {
      IfcUtil::IfcBaseEntity *related_instance = (IfcUtil::IfcBaseEntity *)*iit;

      for (unsigned i = 0; i < related_instance->data().getArgumentCount();
           ++i) {
        ...
        case IfcUtil::Argument_AGGREGATE_OF_AGGREGATE_OF_ENTITY_INSTANCE: {
          ...
          if (instance_list_list->contains(entity)) {
            ...
            for (IfcEntityListList::outer_it it = instance_list_list->begin();
  ...
  {
    IfcEntityList::ptr instances_of_same_type =
        instances_by_type_excl_subtypes(ty);
  ...
  if (it != byref.end()) {
    ...
    if (cached_it != by_ref_cached_.end()) {
      ...
      if (it->second.size()) {
        ...
        for (std::vector<unsigned>::const_iterator jt = ids.begin();
             jt != ids.end(); ++jt) {
...
IfcFile::type_iterator IfcFile::types_end() const { return bytype_excl.end(); }
...
IfcEntityList::ptr IfcFile::getInverse(int instance_id,
    ...
    for (IfcEntityList::it it = units->begin(); it != units->end(); ++it) {
      ...
      if (unit->declaration().is("IfcNamedUnit")) {
        ...
        if (unit->declaration().is("IfcConversionBasedUnit")) {
          ...
          return_value.second *=
              static_cast<double>(*vlc->data().getArgument(0));
