...
// A static locale for the real number parser. strtod() is locale-dependent,
// causing issues in locales that have ',' as a decimal separator. Therefore the
// non standard _strtod_l() / strtod_l() is used and a reference to the "C"
// locale is obtained here. The alternative is to use
// std::istringstream::imbue(std::locale::classic()), but there are subtleties
// in parsing in MSVC2010 and it appears to be much slower.
#if defined(_MSC_VER)
...
IfcSpfStream::IfcSpfStream(const std::string &path)
#endif
    : stream_(0), buffer_(0), valid(false), eof(false) {
#ifdef _MSC_VER
  ...
  if (mmap) {
    mfs =
        boost::iostreams::mapped_file_source(boost::filesystem::wpath(fn_wide));
  } else {
#endif
...
IfcSpfStream::IfcSpfStream(void *data, int length) : stream_(0), buffer_(0) {
  eof = false;
  size = length;
  buffer_ = (char *)data;
  valid = true;
...
IfcSpfStream::~IfcSpfStream() { Close(); }
...
//
char IfcSpfStream::Peek() { return buffer_[ptr_]; }

//
// Returns the character at specified offset
//
char IfcSpfStream::Read(unsigned int offset) { return buffer_[offset]; }

//
// Returns the cursor position
//
unsigned int IfcSpfStream::Tell() const { return ptr_; }
...
IfcSpfLexer::~IfcSpfLexer() { delete decoder_; }
...
Token IfcSpfLexer::Next() {
  ...
  // std::wcout << "token: " << pos << " " << TokenFunc::asStringRef(t).c_str()
  // << std::endl;
  return t;
}
...
inline void RemoveTokenSeparators(IfcSpfStream *stream, unsigned start,
                                  unsigned end, std::string &oDestination) {
  oDestination.clear();
...
Token IfcParse::OperatorTokenPtr(IfcSpfLexer *lexer, unsigned start,
                                 unsigned end) {
...
Token IfcParse::GeneralTokenPtr(IfcSpfLexer *lexer, unsigned start,
                                unsigned end) {
  ...
  if (first == '#') {
    ...
    if (!ParseInt(tokenStr.c_str() + 1, token.value_int)) {
      Logger::Message(Logger::LOG_ERROR, "Token '" + tokenStr + "' at offset " +
                                             std::to_string(token.startPos) +
                                             " is not valid");
    ...
    if (ParseBool(tokenStr.c_str(),
                  token.value_int)) { // bool is also enumeration
...
bool TokenFunc::isInt(const Token &token) { return token.type == Token_INT; }

bool TokenFunc::isBool(const Token &token) {
  // Bool and logical share the same storage type, just logical unknown is
  // stored as 2.
...
int TokenFunc::asIdentifier(const Token &token) {
  if (token.type != Token_IDENTIFIER) {
    throw IfcInvalidTokenException(token.startPos, toString(token),
                                   "instance name");
  }
  return token.value_int;
}
...
const std::string &TokenFunc::asStringRef(const Token &token) {
  if (token.type == Token_NONE) {
    throw IfcParse::IfcException(
        "Null token encountered, premature end of file?");
  }
...
//
void IfcParse::IfcFile::load(unsigned entity_instance_name,
                             const IfcParse::entity *entity,
                             parse_context &context, int attribute_index) {
  ...
  */
  ...
  while ((next.startPos != 0U) || (next.lexer != nullptr)) {
    if (TokenFunc::isOperator(next, ',')) {
      ...
      return_value++;
      load(entity_instance_name, entity, context.push(),
           attribute_index == -1 ? (int)attribute_index_within_data
                                 : attribute_index);
    } else {
      return_value++;
      if (TokenFunc::isIdentifier(next) && entity) {
        register_inverse(entity_instance_name, entity, next,
                         attribute_index == -1 ? attribute_index_within_data
                                               : attribute_index);
      }
      ...
      if (TokenFunc::isKeyword(next)) {
        try {
          const auto *decl =
              schema_->declaration_by_name(TokenFunc::asStringRef(next));
          parse_context ps;
          tokens->Next();
          ...
          // the 2nd pass.
          load(entity_instance_name, entity, ps,
               attribute_index == -1 ? (int)attribute_index_within_data
                                     : attribute_index);
          auto *simple_type_instance = schema_->instantiate(
              decl, ps.construct(entity_instance_name, references_to_resolve,
                                 decl, boost::none,
                                 attribute_index == -1
                                     ? (int)attribute_index_within_data
                                     : attribute_index));
          ...
          Logger::Message(Logger::LOG_ERROR, std::string(e.what()) +
                                                 " at offset " +
                                                 std::to_string(next.startPos));
          // #4070 We didn't actually capture an aggregate entry, undo length
          // increment.
          return_value--;
        }
      } else {
...
IfcEntityInstanceData IfcParse::read(unsigned int i, IfcFile *f) {
  ...
  const IfcParse::declaration *ty =
      f->schema()->declaration_by_name(TokenFunc::asStringRef(datatype));
  parse_context pc;
  f->tokens->Next();
  f->load(i, ty->as_entity(), pc, -1);
  return IfcEntityInstanceData(
      pc.construct(i, f->references_to_resolve, ty, boost::none));
}
...
void IfcParse::IfcFile::register_inverse(unsigned id_from,
                                         const IfcParse::entity *from_entity,
                                         Token t, int attribute_index) {
  // Assume a check on token type has already been performed
  const auto *e = from_entity;
  byref_excl_[{t.value_int, e->index_in_schema(), attribute_index}].push_back(
      id_from);
}
...
void IfcParse::IfcFile::register_inverse(unsigned id_from,
                                         ...
                                         int attribute_index) {
  const auto *e = from_entity;
  byref_excl_[{inst->id(), e->index_in_schema(), attribute_index}].push_back(
      id_from);
}

void IfcParse::IfcFile::unregister_inverse(unsigned id_from,
                                           const IfcParse::entity *from_entity,
                                           IfcUtil::IfcBaseClass *inst,
                                           int attribute_index) {
  std::vector<int> &ids = byref_excl_[{
      inst->id(), from_entity->index_in_schema(), attribute_index}];
  std::vector<int>::iterator iter = std::find(ids.begin(), ids.end(), id_from);
  if (iter == ids.end()) {
    // @todo inverses also need to be populated when multiple instances are
    // added to a new file. throw IfcParse::IfcException("Instance not found
    // among inverses");
  } else {
    ids.erase(iter);
  }
...
private:
  ...
  template <typename T> void serialize(const std::vector<T> &i) {
    data_ << "(";
    for (typename std::vector<T>::const_iterator it = i.begin(); it != i.end();
         ++it) {
      if (it != i.begin()) {
        data_ << ",";
      }
  ...
  // The REAL token definition from the IFC SPF standard does not necessarily
  // match the output of the C++ ostream formatting operation. REAL = [ SIGN ]
...
public:
  ...
  void operator()(const boost::logic::tribool &i) {
    data_ << (i ? ".T." : (boost::logic::indeterminate(i) ? ".U." : ".F."));
  }
  void operator()(const double &i) { data_ << format_double(i); }
  void operator()(const boost::dynamic_bitset<> &i) {
    data_ << format_binary(i);
  }
  ...
  void operator()(const aggregate_of_aggregate_of_instance::ptr &i) {
    data_ << "(";
    for (aggregate_of_aggregate_of_instance::outer_it outer_it = i->begin();
         outer_it != i->end(); ++outer_it) {
      if (outer_it != i->begin()) {
        data_ << ",";
      }
      data_ << "(";
      for (aggregate_of_aggregate_of_instance::inner_it inner_it =
               outer_it->begin();
  ...
  void operator()(const empty_aggregate_of_aggregate_t & /*unused*/) const {
    data_ << "()";
  }
};
...
void StringBuilderVisitor::serialize(const std::vector<std::string> &i) {
  data_ << "(";
  for (std::vector<std::string>::const_iterator it = i.begin(); it != i.end();
       ++it) {
    if (it != i.begin()) {
      data_ << ",";
    }
...
template <> void StringBuilderVisitor::serialize(const std::vector<double> &i) {
  data_ << "(";
  for (std::vector<double>::const_iterator it = i.begin(); it != i.end();
       ++it) {
    if (it != i.begin()) {
      data_ << ",";
    }
...
template <>
void StringBuilderVisitor::serialize(
    const std::vector<boost::dynamic_bitset<>> &i) {
  data_ << "(";
  for (std::vector<boost::dynamic_bitset<>>::const_iterator it = i.begin();
       it != i.end(); ++it) {
    if (it != i.begin()) {
      data_ << ",";
    }
...
void StringBuilderVisitor::operator()(const std::vector<int> &i) {
  serialize(i);
}
void StringBuilderVisitor::operator()(const std::vector<double> &i) {
  serialize(i);
}
void StringBuilderVisitor::operator()(const std::vector<std::string> &i) {
  serialize(i);
}
void StringBuilderVisitor::operator()(
    const std::vector<boost::dynamic_bitset<>> &i) {
  serialize(i);
}
void StringBuilderVisitor::operator()(const std::vector<std::vector<int>> &i) {
  data_ << "(";
  for (std::vector<std::vector<int>>::const_iterator it = i.begin();
       it != i.end(); ++it) {
    if (it != i.begin()) {
      data_ << ",";
    }
...
void StringBuilderVisitor::operator()(
  ...
  data_ << "(";
  for (std::vector<std::vector<double>>::const_iterator it = i.begin();
       it != i.end(); ++it) {
    if (it != i.begin()) {
      data_ << ",";
    }
...
//
void IfcEntityInstanceData::toString(std::ostream &ss, bool upper,
                                     const entity *decl) const {
  ss.imbue(std::locale::classic());
...
namespace {
// @todo remove redundancy with python wrapper code (which is not identical due
// to different handling of enumerations)
IfcUtil::ArgumentType get_argument_type(const IfcParse::declaration *decl,
                                        size_t i) {
...
public:
  ...
  void operator()(IfcUtil::IfcBaseClass *inst, int index) {
    file_.unregister_inverse(data_->id(), data_->declaration().as_entity(),
                             inst, index);
  }
};
...
public:
  ...
  void operator()(IfcUtil::IfcBaseClass *inst, int index) {
    file_.register_inverse(data_->id(), data_->declaration().as_entity(), inst,
                           index);
  }
};
...
public:
  ...
  void operator()(IfcUtil::IfcBaseClass *inst) { list_->push(inst); }
};
...
private:
  ...
  void apply_attribute_(T &t, const AttributeValue &attr, int index) const {
    if (attr.type() == IfcUtil::Argument_ENTITY_INSTANCE) {
      ...
      for (aggregate_of_instance::it it = entity_list_attribute->begin();
           it != entity_list_attribute->end(); ++it) {
        t(*it, index);
      }
    } else if (attr.type() ==
      ...
      for (aggregate_of_aggregate_of_instance::outer_it it =
               entity_list_attribute->begin();
           it != entity_list_attribute->end(); ++it) {
        for (aggregate_of_aggregate_of_instance::inner_it jt = it->begin();
             jt != it->end(); ++jt) {
          t(*jt, index);
        }
      }
...
public:
  apply_individual_instance_visitor(const AttributeValue &attribute, int idx)
      : attribute_(attribute), attribute_index_(idx) {}
...
void IfcUtil::IfcBaseClass::set_attribute_value(size_t i, const T &t) {
  ...
  if (file_ != nullptr) {
    ...
    if (i == 0 && (file_->ifcroot_type() != nullptr) &&
        this->declaration().is(*file_->ifcroot_type())) {
      try {
...
template <typename T>
void IfcUtil::IfcBaseClass::set_attribute_value(const std::string &s,
                                                const T &t) {
...
IfcFile::IfcFile(const IfcParse::schema_definition *schema)
    : schema_(schema), ifcroot_type_(schema_->declaration_by_name("IfcRoot")),
      MaxId(0), tokens(0), stream(0) {
...
void IfcFile::initialize_(IfcParse::IfcSpfStream *s) {
  ...
  while (!stream->eof) {
    if (token_stream[0].type == IfcParse::Token_IDENTIFIER &&
      ...
      try {
        entity_type = schema_->declaration_by_name(
            TokenFunc::asStringRef(token_stream[2]));
      } catch (const IfcException &ex) {
        Logger::Message(Logger::LOG_ERROR,
                        std::string(ex.what()) + " at offset " +
                            std::to_string(token_stream[2].startPos));
        goto advance;
      }

      if (entity_type->as_entity() == nullptr) {
        Logger::Message(Logger::LOG_ERROR,
                        "Non entity type " + entity_type->name() +
                            " at offset " +
                            std::to_string(token_stream[2].startPos));
        goto advance;
      }
      ...
      instance = schema_->instantiate(
          entity_type, ps.construct(current_id, references_to_resolve,
                                    entity_type, boost::none));
      instance->file_ = this;
      instance->id_ = current_id;

      /// @todo Printing to stdout in a library class feels weird. Maybe move
      /// the progress prints to the client code?
    ...
    } else if (token_stream[0].type == IfcParse::Token_IDENTIFIER &&
               (instance != nullptr)) {
      register_inverse(current_id, instance->declaration().as_entity(),
                       token_stream[0], attribute_index);
    } else if (token_stream[0].type == IfcParse::Token_OPERATOR &&
               token_stream[0].value_char == '(') {
      paren_stack_depth++;
    } else if (token_stream[0].type == IfcParse::Token_OPERATOR &&
               token_stream[0].value_char == ')') {
      paren_stack_depth--;
    ...
    } else if (paren_stack_depth == 1 &&
               token_stream[0].type == IfcParse::Token_OPERATOR &&
               token_stream[0].value_char == ',') {
      attribute_index++;
    }
  ...
  advance:
    ...
    try {
      ...
      Logger::Message(Logger::LOG_ERROR,
                      std::string(e.what()) + ". Parsing terminated");
    } catch (...) {
  ...
  for (const auto &p : references_to_resolve) {
    ...
    if (auto *v = boost::get<reference_or_simple_type>(&p.second)) {
      if (auto *name = boost::get<InstanceReference>(v)) {
        ...
        if (it == byid_.end()) {
          Logger::Error("Instance reference #" + std::to_string(*name) +
                        " used by instance #" + std::to_string(ref) +
                        " at attribute index " + std::to_string(refattr) +
                        " not found at offset " +
                        std::to_string(name->file_offset));
        } else {
    ...
    } else if (auto *v = boost::get<std::vector<reference_or_simple_type>>(
                   &p.second)) {
    ...
    } else if (auto *v = boost::get<
                   std::vector<std::vector<reference_or_simple_type>>>(
                   &p.second)) {
      aggregate_of_aggregate_of_instance::ptr instances(
...
public:
  traversal_visitor(std::set<IfcUtil::IfcBaseClass *> &visited,
                    traversal_recorder &list, int level, int max_level)
      : visited_(visited), list_(list), level_(level), max_level_(max_level) {}

  void operator()(IfcUtil::IfcBaseClass *inst, int index);
};

void traverse_(IfcUtil::IfcBaseClass *instance,
...
aggregate_of_instance::ptr IfcParse::traverse(IfcUtil::IfcBaseClass *instance,
                                              int max_level) {
  std::set<IfcUtil::IfcBaseClass *> visited;
...
// keeping track of their rank and return a list ordered by rank. Is this
// equivalent?
aggregate_of_instance::ptr
IfcParse::traverse_breadth_first(IfcUtil::IfcBaseClass *instance,
                                 int max_level) {
  std::set<IfcUtil::IfcBaseClass *> visited;
...
aggregate_of_instance::ptr IfcFile::traverse(IfcUtil::IfcBaseClass *instance,
                                             int max_level) {
...
/// @note: for backwards compatibility
aggregate_of_instance::ptr
IfcFile::traverse_breadth_first(IfcUtil::IfcBaseClass *instance,
                                int max_level) {
  return IfcParse::traverse_breadth_first(instance, max_level);
}

void IfcFile::addEntities(aggregate_of_instance::ptr entities) {
  for (aggregate_of_instance::it i = entities->begin(); i != entities->end();
       ++i) {
    addEntity(*i);
  }
}
...
IfcUtil::IfcBaseClass *IfcFile::addEntity(IfcUtil::IfcBaseClass *entity,
  ...
  if (id != -1 && byid_.find((unsigned)id) != byid_.end()) {
    throw IfcParse::IfcException("An instance with id " +
                                 boost::lexical_cast<std::string>(id) +
                                 " is already part of this file");
  }

  if (entity->declaration().schema() != schema()) {
    throw IfcParse::IfcException("Unabled to add instance from " +
                                 entity->declaration().schema()->name() +
                                 " schema to file with " + schema()->name() +
                                 " schema");
  }
  ...
  try {
    ...
    for (aggregate_of_instance::it it = entity_attributes->begin();
         it != entity_attributes->end(); ++it) {
      if (*it != entity) {
        entity_entity_map_t::iterator mit2 =
            entity_file_map_.find((*it)->identity());
        if (mit2 == entity_file_map_.end()) {
          entity_file_map_.insert(entity_entity_map_t::value_type(
              (*it)->identity(), addEntity(*it)));
        }
      }
    }
  } catch (...) {
    Logger::Message(Logger::LOG_ERROR, "Failed to visit forward references of",
                    entity);
  }
  ...
  if (entity->file_ != nullptr) {
    if (entity->file_ == this) {
      if (entity->declaration().as_entity() == nullptr) {
        // While not a mapping that can be queried, we do need to free the
        // instance later on
    ...
    for (size_t i = 0; i < new_entity->data().size(); ++i) {
      ...
      if (entity->declaration().as_entity() != nullptr) {
        decl = 0;
        const parameter_type *pt = entity->declaration()
                                       .as_entity()
      ...
      if (attr_type == IfcUtil::Argument_ENTITY_INSTANCE) {
        entity_entity_map_t::const_iterator eit = entity_file_map_.find(
            ((IfcUtil::IfcBaseClass *)(attr))->identity());
        if (eit == entity_file_map_.end()) {
          throw IfcParse::IfcException("Unable to map instance to file");
        }
        // We directly use storage set not to trigger inverse recalculation
        ...
        for (aggregate_of_instance::it it = instances->begin();
             it != instances->end(); ++it) {
          entity_entity_map_t::const_iterator eit =
              entity_file_map_.find((*it)->identity());
          if (eit == entity_file_map_.end()) {
            throw IfcParse::IfcException("Unable to map instance to file");
          }
        ...
        aggregate_of_aggregate_of_instance::ptr new_instances(
            new aggregate_of_aggregate_of_instance);
        for (aggregate_of_aggregate_of_instance::outer_it it =
                 instances->begin();
             it != instances->end(); ++it) {
          ...
          for (aggregate_of_aggregate_of_instance::inner_it jt = it->begin();
               jt != it->end(); ++jt) {
            entity_entity_map_t::const_iterator eit =
                entity_file_map_.find((*jt)->identity());
            if (eit == entity_file_map_.end()) {
              throw IfcParse::IfcException("Unable to map instance to file");
            }
        ...
        new_entity->data().storage_.set(i, new_instances);
                 ...
                 decl->is(*schema()->declaration_by_name("IfcLengthMeasure"))) {
        if (boost::math::isnan(conversion_factor)) {
          std::pair<IfcUtil::IfcBaseClass *, double> this_file_unit = {nullptr,
                                                                       1.0};
          std::pair<IfcUtil::IfcBaseClass *, double> other_file_unit = {nullptr,
                                                                        1.0};
          try {
          ...
          if ((this_file_unit.first != nullptr) &&
              (other_file_unit.first != nullptr)) {
        ...
        if (attr_type == IfcUtil::Argument_DOUBLE) {
          ...
          for (std::vector<double>::iterator it = v.begin(); it != v.end();
               ++it) {
          ...
          for (std::vector<std::vector<double>>::iterator it = v.begin();
               it != v.end(); ++it) {
            std::vector<double> &v2 = (*it);
            for (std::vector<double>::iterator jt = v2.begin(); jt != v2.end();
                 ++jt) {
    ...
    entity_file_map_.insert(
        entity_entity_map_t::value_type(entity->identity(), new_entity));
  }
...
void IfcFile::removeEntity(IfcUtil::IfcBaseClass *entity) {
  ...
  // Attention when running removeEntity inside a loop over a list of entities
  // to be removed. This invalidates the iterator. A workaround is to reverse
  // the loop: boost::shared_ptr<aggregate_of_instance> entities = ...; for
  // (auto it = entities->end() - 1; it >= entities->begin(); --it) {
  //    IfcUtil::IfcBaseClass *const inst = *it;
  //    model->removeEntity(inst);
  // }

  // TODO: Create a set of weak relations. Inverse relations that do not dictate
  // an instance to be retained. For example: when deleting an
  // IfcRepresentation, the individual IfcRepresentationItems can not be deleted
  // if an IfcStyledItem is related. Hence, the
  // IfcRepresentationItem::StyledByItem relation could be characterized as
  // weak. std::set<IfcSchema::Type::Enum> weak_roots;
...
void IfcFile::process_deletion_() {
  ...
  for (const auto &id : batch_deletion_ids_.get<0>()) {
    ...
    if (references) {
      for (aggregate_of_instance::it iit = references->begin();
           iit != references->end(); ++iit) {
        IfcUtil::IfcBaseEntity *related_instance =
            (IfcUtil::IfcBaseEntity *)*iit;

        if (std::find(batch_deletion_ids_.begin(), batch_deletion_ids_.end(),
                      related_instance->id()) != batch_deletion_ids_.end()) {
          continue;
        }
        ...
        for (size_t i = 0; i < related_instance->data().size(); ++i) {
          ...
          case IfcUtil::Argument_AGGREGATE_OF_ENTITY_INSTANCE: {
            ...
            if (instance_list->contains(entity)) {
              ...
              if ((instance_list->size() == 0U) &&
                  related_instance->declaration()
                      .as_entity()
                      ->attribute_by_index(i)
                      ->optional()) {
                // @todo we can also check the lower bound of the attribute type
          ...
          case IfcUtil::Argument_AGGREGATE_OF_AGGREGATE_OF_ENTITY_INSTANCE: {
            ...
            if (instance_list_list->contains(entity)) {
              aggregate_of_aggregate_of_instance::ptr new_list(
                  new aggregate_of_aggregate_of_instance);
              for (aggregate_of_aggregate_of_instance::outer_it it =
                       instance_list_list->begin();
                   it != instance_list_list->end(); ++it) {
                std::vector<IfcUtil::IfcBaseClass *> instances = *it;
                std::vector<IfcUtil::IfcBaseClass *>::iterator jt;
                while ((jt = std::find(instances.begin(), instances.end(),
    ...
    if (!batch_mode_) {
      byref_excl_.erase(
          ...
          byref_excl_.upper_bound({id, std::numeric_limits<short>::max(),
                                   std::numeric_limits<short>::max()}));

      // byref_excl_.erase(id);

      // This is based on traversal which needs instances to still be contained
      // in the map. another option would be to keep byid intact for the
      ...
      for (aggregate_of_instance::it it = entity_attributes->begin();
        ...
        // Do not update inverses for simple types (which have id()==0 in
        // IfcOpenShell).
        if (name != 0) {
          {
            auto lower = byref_excl_.lower_bound({name, -1, -1});
            auto upper = byref_excl_.upper_bound(
                {name, std::numeric_limits<short>::max(),
                 std::numeric_limits<short>::max()});
    ...
    if (entity->declaration().is(*ifcroot_type_) &&
        !entity->data().get_attribute_value(0).isNull()) {
    ...
    // entity_file_map is in place to prevent duplicate definitions with usage
    // of add(). Upon deletion the pairs need to be erased.
  ...
  if (batch_mode_) {
    for (auto it = byref_excl_.begin(); it != byref_excl_.end();) {
      bool do_delete =
          batch_deletion_ids_.get<1>().find(std::get<INSTANCE_ID>(it->first)) !=
          batch_deletion_ids_.get<1>().end();
      if (!do_delete) {
        it->second.erase(
            std::remove_if(it->second.begin(), it->second.end(),
                           [this](int x) {
                             return batch_deletion_ids_.get<1>().find(x) !=
                                    batch_deletion_ids_.get<1>().end();
                           }),
            it->second.end());
...
namespace {
template <typename Fn> void visit_subtypes(const IfcParse::entity *ent, Fn fn) {
  fn(ent);
...
aggregate_of_instance::ptr
IfcFile::instances_by_type(const IfcParse::declaration *t) {
...
aggregate_of_instance::ptr
IfcFile::instances_by_type_excl_subtypes(const IfcParse::declaration *t) {
  entities_by_type_t::const_iterator it = bytype_excl_.find(t);
  return (it == bytype_excl_.end())
             ? aggregate_of_instance::ptr(new aggregate_of_instance)
             : it->second;
}
...
aggregate_of_instance::ptr
IfcFile::instances_by_type_excl_subtypes(const std::string &t) {
...
aggregate_of_instance::ptr IfcFile::instances_by_reference(int t) {
  ...
  auto upper = byref_excl_.upper_bound({t, std::numeric_limits<short>::max(),
                                        std::numeric_limits<short>::max()});
...
IfcUtil::IfcBaseClass *IfcFile::instance_by_id(int id) {
  ...
  if (it == byid_.end()) {
    throw IfcException("Instance #" + boost::lexical_cast<std::string>(id) +
                       " not found");
  }
  return it->second;
}
...
IfcFile::type_iterator IfcFile::types_end() const { return bytype_excl_.end(); }

namespace {
struct id_instance_pair_sorter {
  bool
  operator()(const IfcParse::IfcFile::entity_by_id_t::value_type &a,
             const IfcParse::IfcFile::entity_by_id_t::value_type &b) const {
...
std::ostream &operator<<(std::ostream &out, const IfcParse::IfcFile &file) {
  ...
  typedef std::vector<std::pair<unsigned int, IfcUtil::IfcBaseClass *>>
      vector_t;
...
std::vector<int> IfcFile::get_inverse_indices(int instance_id) {
  ...
  auto upper =
      byref_excl_.upper_bound({instance_id, std::numeric_limits<short>::max(),
                               std::numeric_limits<short>::max()});
  ...
  for (auto it = lower; it != upper; ++it) {
    for (auto &i : it->second) {
      ...
      // to count double. Because byref contains mappings for every supertype of
      // id=i.
      if (instance_by_id(i)->declaration().index_in_schema() ==
          std::get<1>(it->first)) {
...
aggregate_of_instance::ptr
IfcFile::getInverse(int instance_id, const IfcParse::declaration *type,
                    int attribute_index) {
  ...
  visit_subtypes(type->as_entity(), [this, attribute_index, instance_id,
                                     &return_value](
                                        const IfcParse::declaration *ent) {
    if (attribute_index == -1) {
      auto lower =
          byref_excl_.lower_bound({instance_id, ent->index_in_schema(), -1});
      auto upper = byref_excl_.upper_bound({instance_id, ent->index_in_schema(),
                                            std::numeric_limits<short>::max()});
      ...
      auto it = byref_excl_.find(
          {instance_id, ent->index_in_schema(), attribute_index});
...
IfcFile::getUnit(const std::string &unit_type) {
  std::pair<IfcUtil::IfcBaseClass *, double> return_value(0, 1.);

  aggregate_of_instance::ptr projects =
      instances_by_type(schema()->declaration_by_name("IfcProject"));
  ...
  if (projects && projects->size() == 1) {
    ...
    IfcUtil::IfcBaseClass *unit_assignment =
        project->data().get_attribute_value(
            project->declaration().as_entity()->attribute_index(
                "UnitsInContext"));

    aggregate_of_instance::ptr units =
        unit_assignment->data().get_attribute_value(
            unit_assignment->declaration().as_entity()->attribute_index(
                "Units"));

    for (aggregate_of_instance::it it = units->begin(); it != units->end();
         ++it) {
      ...
      if (unit->declaration().is("IfcNamedUnit")) {
        ...
        if (unit->declaration().is("IfcConversionBasedUnit")) {
          IfcUtil::IfcBaseClass *mu = unit->data().get_attribute_value(
              unit->declaration().as_entity()->attribute_index(
                  "ConversionFactor"));
          ...
          return_value.second *=
              static_cast<double>(vlc->data().get_attribute_value(0));
...
void IfcParse::IfcFile::build_inverses_(IfcUtil::IfcBaseClass *inst) {
  std::function<void(IfcUtil::IfcBaseClass *, int)> fn =
      [this, inst](IfcUtil::IfcBaseClass *attr, int idx) {
        if (attr->declaration().as_entity() != nullptr) {
          unsigned entity_attribute_id = attr->id();
          const auto *decl = inst->declaration().as_entity();
          byref_excl_[{entity_attribute_id, decl->index_in_schema(), idx}]
              .push_back(inst->id());
        }
      };
    ...
    data.storage_.apply_visitor(
        [this, i](const auto &v) {
          ...
          if constexpr (std::is_same_v<U, aggregate_of_instance::ptr>) {
            ...
            aggregate_of_aggregate_of_instance::ptr v2(
                new aggregate_of_aggregate_of_instance);
            if (v) {
              for (auto &i : *v) {
                v2->push(i);
...
template void IFC_PARSE_API IfcUtil::IfcBaseClass::set_attribute_value<Blank>(
    size_t index, const Blank &value);
template void IFC_PARSE_API
IfcUtil::IfcBaseClass::set_attribute_value<int>(size_t index, const int &value);
template void IFC_PARSE_API IfcUtil::IfcBaseClass::set_attribute_value<bool>(
    size_t index, const bool &value);
template void IFC_PARSE_API
IfcUtil::IfcBaseClass::set_attribute_value<boost::logic::tribool>(
    size_t index, const boost::logic::tribool &value);
template void IFC_PARSE_API IfcUtil::IfcBaseClass::set_attribute_value<double>(
    size_t index, const double &value);
template void IFC_PARSE_API
IfcUtil::IfcBaseClass::set_attribute_value<std::string>(
    size_t index, const std::string &value);
template void IFC_PARSE_API
IfcUtil::IfcBaseClass::set_attribute_value<boost::dynamic_bitset<>>(
    size_t index, const boost::dynamic_bitset<> &value);
template void IFC_PARSE_API
IfcUtil::IfcBaseClass::set_attribute_value<EnumerationReference>(
    size_t index, const EnumerationReference &value);
template void IFC_PARSE_API
IfcUtil::IfcBaseClass::set_attribute_value<IfcUtil::IfcBaseClass *>(
    size_t index, IfcUtil::IfcBaseClass *const &value);
template void IFC_PARSE_API
IfcUtil::IfcBaseClass::set_attribute_value<std::vector<int>>(
    size_t index, const std::vector<int> &value);
template void IFC_PARSE_API
IfcUtil::IfcBaseClass::set_attribute_value<std::vector<double>>(
    size_t index, const std::vector<double> &value);
template void IFC_PARSE_API
IfcUtil::IfcBaseClass::set_attribute_value<std::vector<std::string>>(
    size_t index, const std::vector<std::string> &value);
template void IFC_PARSE_API IfcUtil::IfcBaseClass::set_attribute_value<
    std::vector<boost::dynamic_bitset<>>>(
    size_t index, const std::vector<boost::dynamic_bitset<>> &value);
template void IFC_PARSE_API
IfcUtil::IfcBaseClass::set_attribute_value<aggregate_of_instance::ptr>(
    size_t index, const aggregate_of_instance::ptr &value);
template void IFC_PARSE_API
IfcUtil::IfcBaseClass::set_attribute_value<std::vector<std::vector<int>>>(
    size_t index, const std::vector<std::vector<int>> &value);
template void IFC_PARSE_API
IfcUtil::IfcBaseClass::set_attribute_value<std::vector<std::vector<double>>>(
    size_t index, const std::vector<std::vector<double>> &value);
template void IFC_PARSE_API IfcUtil::IfcBaseClass::set_attribute_value<
    aggregate_of_aggregate_of_instance::ptr>(
    size_t index, const aggregate_of_aggregate_of_instance::ptr &value);

template void IFC_PARSE_API IfcUtil::IfcBaseClass::set_attribute_value<Blank>(
    const std::string &name, const Blank &value);
template void IFC_PARSE_API IfcUtil::IfcBaseClass::set_attribute_value<int>(
    const std::string &name, const int &value);
template void IFC_PARSE_API IfcUtil::IfcBaseClass::set_attribute_value<bool>(
    const std::string &name, const bool &value);
template void IFC_PARSE_API
IfcUtil::IfcBaseClass::set_attribute_value<boost::logic::tribool>(
    const std::string &name, const boost::logic::tribool &value);
template void IFC_PARSE_API IfcUtil::IfcBaseClass::set_attribute_value<double>(
...
template void IFC_PARSE_API
IfcUtil::IfcBaseClass::set_attribute_value<boost::dynamic_bitset<>>(
    const std::string &name, const boost::dynamic_bitset<> &value);
template void IFC_PARSE_API
IfcUtil::IfcBaseClass::set_attribute_value<EnumerationReference>(
    const std::string &name, const EnumerationReference &value);
template void IFC_PARSE_API
IfcUtil::IfcBaseClass::set_attribute_value<IfcUtil::IfcBaseClass *>(
    const std::string &name, IfcUtil::IfcBaseClass *const &value);
template void IFC_PARSE_API
IfcUtil::IfcBaseClass::set_attribute_value<std::vector<int>>(
...
template void IFC_PARSE_API IfcUtil::IfcBaseClass::set_attribute_value<
    std::vector<boost::dynamic_bitset<>>>(
    const std::string &name, const std::vector<boost::dynamic_bitset<>> &value);
template void IFC_PARSE_API
IfcUtil::IfcBaseClass::set_attribute_value<aggregate_of_instance::ptr>(
    const std::string &name, const aggregate_of_instance::ptr &value);
template void IFC_PARSE_API
IfcUtil::IfcBaseClass::set_attribute_value<std::vector<std::vector<int>>>(
    const std::string &name, const std::vector<std::vector<int>> &value);
template void IFC_PARSE_API
IfcUtil::IfcBaseClass::set_attribute_value<std::vector<std::vector<double>>>(
    const std::string &name, const std::vector<std::vector<double>> &value);
template void IFC_PARSE_API IfcUtil::IfcBaseClass::set_attribute_value<
    aggregate_of_aggregate_of_instance::ptr>(
    const std::string &name,
    const aggregate_of_aggregate_of_instance::ptr &value);
