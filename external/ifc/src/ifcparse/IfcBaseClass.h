...
protected:
  ...
  template <typename T>
  std::enable_if_t<!std::is_same<IfcBaseClass, T>::value &&
                   !std::is_same<IfcBaseEntity, T>::value &&
                   !std::is_same<IfcBaseType, T>::value>
  raise_error_on_concrete_class() const {
    throw IfcParse::IfcException("Instance of type " +
                                 this->declaration().name() +
                                 " cannot be cast to " + T::Class().name());
  }
...
public:
  ...
  template <class T> T *as(bool do_throw = false) {
  ...
  template <class T> const T *as(bool do_throw = false) const {
    if (is_null(this)) {
...
public:
  IfcBaseClass(IfcEntityInstanceData &&data)
      : identity_(counter_++), id_(0), file_(nullptr), data_(std::move(data)) {}
  ...
  template <typename T> void set_attribute_value(size_t i, const T &t);
...
public:
  ...
  IfcBaseEntity(size_t n) : IfcBaseClass(IfcEntityInstanceData(storage_t(n))) {}

  virtual const IfcParse::declaration &declaration() const = 0;

  AttributeValue get(const std::string &name) const;

  template <typename T> T get_value(const std::string &name) const;

  template <typename T>
  T get_value(const std::string &name, const T &default_value) const;

  boost::shared_ptr<aggregate_of_instance>
  get_inverse(const std::string &name) const;
...
public:
  IfcLateBoundEntity(const IfcParse::declaration *decl,
                     IfcEntityInstanceData &&data)
      : IfcBaseEntity(std::move(data)), decl_(decl) {}

  virtual const IfcParse::declaration &declaration() const { return *decl_; }
};

// TODO: Investigate whether these should be template classes instead
class IFC_PARSE_API IfcBaseType : public IfcBaseClass {
public:
  IfcBaseType(IfcEntityInstanceData &&data) : IfcBaseClass(std::move(data)) {}

  IfcBaseType() : IfcBaseClass(IfcEntityInstanceData(storage_t(1))) {}
...
template <typename T>
T IfcBaseEntity::get_value(const std::string &name,
...
template <class U> typename U::list::ptr aggregate_of_instance::as() {
...
template <class U>
typename aggregate_of_aggregate_of<U>::ptr
aggregate_of_aggregate_of_instance::as() {
  typename aggregate_of_aggregate_of<U>::ptr result(
      new aggregate_of_aggregate_of<U>);
