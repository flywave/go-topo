...
public:
  EnumerationReference(const IfcParse::enumeration_type *enumeration,
                       size_t index)
      : enumeration_(enumeration), index_(index) {}

  const char *value() const { return enumeration_->lookup_enum_value(index_); }

  size_t index() const { return index_; }

  const IfcParse::enumeration_type *enumeration() const { return enumeration_; }
};
class Blank {};
class Derived {};
...
typedef VariantArray<
    ...
    Blank,
    // @todo Derived is not really necessary anymore, just serialize correctly
    // based on schema A derived argument, it will always serialize to *
    Derived,
...
struct AttributeValue {
  ...
  AttributeValue() : array_(nullptr), index_(0) {}
...
public:
  ...
  IfcEntityInstanceData(storage_t &&storage) : storage_(std::move(storage)) {}
  ...
  void set_attribute_value(size_t index, AttributeValue&, IfcUtil::ArgumentType
  attr_type = IfcUtil::Argument_UNKNOWN);
  */

  size_t size() const { return storage_.size(); }

  void toString(std::ostream &, bool upper = false,
                const IfcParse::entity *ent = nullptr) const;
};
