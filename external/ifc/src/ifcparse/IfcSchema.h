...
public:
  ...
  virtual const named_type *as_named_type() const {
    return static_cast<named_type *>(0);
  }
  virtual const simple_type *as_simple_type() const {
    return static_cast<simple_type *>(0);
  }
  virtual const aggregation_type *as_aggregation_type() const {
    return static_cast<aggregation_type *>(0);
  }
...
public:
  named_type(declaration *declared_type) : declared_type_(declared_type) {}
...
public:
  simple_type(data_type declared_type) : declared_type_(declared_type) {}
...
public:
  aggregation_type(aggregate_type type_of_aggregation, int bound1, int bound2,
                   parameter_type *type_of_element)
      : type_of_aggregation_(type_of_aggregation), bound1_(bound1),
        bound2_(bound2), type_of_element_(type_of_element) {}
...
public:
  declaration(const std::string &name, int index_in_schema)
      : name_(name), name_upper_(boost::to_upper_copy(name)),
        index_in_schema_(index_in_schema), schema_(0) {}
  ...
  virtual const type_declaration *as_type_declaration() const {
    return static_cast<type_declaration *>(0);
  }
  virtual const select_type *as_select_type() const {
    return static_cast<select_type *>(0);
  }
  virtual const enumeration_type *as_enumeration_type() const {
    return static_cast<enumeration_type *>(0);
  }
...
public:
  type_declaration(const std::string &name, int index_in_schema,
                   const parameter_type *declared_type)
      : declaration(name, index_in_schema), declared_type_(declared_type) {}
...
public:
  select_type(const std::string &name, int index_in_schema,
              const std::vector<const declaration *> &select_list)
      : declaration(name, index_in_schema), select_list_(select_list) {}
...
public:
  ...
  size_t lookup_enum_offset(const std::string &string) const {
    size_t index = 0;
    for (auto it = enumeration_items_.begin(); it != enumeration_items_.end();
         ++it, ++index) {
      if (string == *it) {
        return index;
      }
...
public:
  attribute(const std::string &name, parameter_type *type_of_attribute,
            bool optional)
      : name_(name), type_of_attribute_(type_of_attribute),
        optional_(optional) {}
...
public:
  inverse_attribute(const std::string &name, aggregate_type type_of_aggregation,
                    int bound1, int bound2, const entity *entity_reference,
                    const attribute *attribute_reference)
      : name_(name), type_of_aggregation_(type_of_aggregation), bound1_(bound1),
        bound2_(bound2), entity_reference_(entity_reference),
...
protected:
  bool is_abstract_;
  const entity
      *supertype_; /* NB: IFC explicitly allows only single inheritance */
  ...
  public:
    attribute_by_name_cmp(const std::string name) : name_(name) {}
    bool operator()(const attribute *attr) { return attr->name() == name_; }
  };
...
public:
  entity(const std::string &name, bool is_abstract, int index_in_schema,
         entity *supertype)
      : declaration(name, index_in_schema), is_abstract_(is_abstract),
  ...
  void set_attributes(const std::vector<const attribute *> &attributes,
                      const std::vector<bool> &derived) {
    attributes_ = attributes;
    derived_ = derived;
  }

  void set_inverse_attributes(
      const std::vector<const inverse_attribute *> &inverse_attributes) {
  ...
  const std::vector<const attribute *> all_attributes() const {
    ...
    if (supertype_ != nullptr) {
      const std::vector<const attribute *> supertype_attrs =
          supertype_->all_attributes();
      std::copy(supertype_attrs.begin(), supertype_attrs.end(),
  ...
  const std::vector<const inverse_attribute *> all_inverse_attributes() const {
    ...
    if (supertype_ != nullptr) {
      const std::vector<const inverse_attribute *> supertype_inv_attrs =
          supertype_->all_inverse_attributes();
      std::copy(supertype_inv_attrs.begin(), supertype_inv_attrs.end(),
                std::back_inserter(attrs));
    }
    std::copy(inverse_attributes_.begin(), inverse_attributes_.end(),
              std::back_inserter(attrs));
    return attrs;
  }
  ...
  ptrdiff_t attribute_index(const attribute *attr) const {
    ...
    do {
      if (index > -1) {
        ...
        iter = std::find(current->attributes().begin(),
                         current->attributes().end(), attr);
  ...
  ptrdiff_t attribute_index(const std::string &attr_name) const {
    ...
    do {
      if (index > -1) {
        ...
        std::vector<const attribute *>::const_iterator iter;
        iter = std::find_if(current->attributes().begin(),
                            current->attributes().end(), cmp);
        if (iter != current->attributes().end()) {
          index = std::distance(current->attributes().begin(), iter);
        }
      }
...
public:
  ...
  operator()(const IfcParse::declaration *decl,
             IfcEntityInstanceData &&data) const = 0;
};
...
public:
  schema_definition(const std::string &name,
  ...
  const declaration *declaration_by_name(const std::string &name) const {
    ...
    if (std::any_of(name.begin(), name.end(),
                    [](char character) { return std::islower(character); })) {
      temp_string_() = name;
      boost::to_upper(temp_string_());
      name_ptr = &temp_string_();
    }
    std::vector<const declaration *>::const_iterator iter =
        std::lower_bound(declarations_.begin(), declarations_.end(), *name_ptr,
                         declaration_by_name_cmp());
    if (iter == declarations_.end() || (**iter).name_uc() != *name_ptr) {
      throw IfcParse::IfcException("Entity with name '" + name +
                                   "' not found in schema '" + name_ + "'");
    }
    return *iter;
  }
