...
template <typename Fn> void visit_any_impl(Fn &fn, const boost::any &a) {}
...
template <typename Fn, typename U> void visit_any(Fn fn, const boost::any &a);
...
protected:
  ...
  stack_node()
      : type_(stack_empty), inst_(nullptr), idx_(-1), inv_(nullptr),
        aggregate_elem_type_(nullptr) {}

public:
  static stack_node instance(const std::string &id,
  ...
  static stack_node
  aggregate_element(const IfcParse::parameter_type *aggregate_elem_type,
                    int idx) {
    stack_node node;
  ...
  static stack_node inverse(IfcUtil::IfcBaseClass *inst,
                            const IfcParse::inverse_attribute *inv) {
    stack_node node;
  ...
  const IfcParse::parameter_type *aggregate_elem_type() const {
    return aggregate_elem_type_;
  }

  std::string repr() const {
    std::stringstream stream;
    static const char *const node_type_names[] = {"empty", "inst",   "attr",
                                                  "aggr",  "agelem", "inv",
                                                  "sel",   "head",   "hdentry"};
...
struct ifcxml_parse_state {
  ...
  std::vector<std::tuple<IfcUtil::IfcBaseEntity *, size_t, std::string>>
      forward_references;
  ifcxml_dialect dialect;
};

// ifc4 allows for aggregates to be concatenated using whitespace.
template <typename T> std::vector<T> split(const std::string &value) {
  std::vector<std::string> strs;
  boost::split(
      strs, value, [](char character) { return character == ' '; },
...
boost::any parse_attribute_value(const IfcParse::parameter_type *ty,
  ...
  if (cpp_type == IfcUtil::Argument_STRING) {
    ...
    const auto *enum_type =
        ty->as_named_type()->declared_type()->as_enumeration_type();

    std::vector<std::string>::const_iterator iter = std::find(
        enum_type->enumeration_items().begin(),
        enum_type->enumeration_items().end(), boost::to_upper_copy(value));

    any = EnumerationReference(
        enum_type, std::distance(enum_type->enumeration_items().begin(), iter));
...
static void end_element(void *user, const xmlChar *tag) {
  ...
  if (!state->stack.empty() &&
      state->stack.back().ntype() == stack_node::node_aggregate) {
  ...
  if (state->dialect == ifcxml_dialect_ifc2x3 &&
      state->stack.back().ntype() == stack_node::node_instance) {
    if (state->stack.back().inst() != nullptr) {
      state->idmap[state->stack.back().id()] =
          state->file->addEntity(state->stack.back().inst())->id();
    }
  }
  ...
  if (tagname != "uos" && tagname != "ex:iso_10303_28" &&
      tagname != "ifc:ifcXML" && tagname != "ifcXML") {
    if (state->stack.empty()) {
...
static void process_characters(void *user, const xmlChar *character, int len) {
  ...
  if (!state->stack.empty() && state->stack.back().inst() != nullptr &&
      (state->stack.back().inst()->declaration().as_type_declaration() !=
       nullptr)) {
    const auto *pt = state->stack.back()
                         .inst()
                         ->declaration()
                         .as_type_declaration()
                         ->declared_type();
    ...
    const std::string tagname =
        boost::replace_all_copy(state->stack.back().tagname(), "ex:", "");
    ...
    const auto *pt = state->stack.back()
                         ...
                         ->declaration()
                         .as_entity()
                         ->attribute_by_index(state->stack.back().idx())
                         ->type_of_attribute();
    ...
    if (cpp_type != IfcUtil::Argument_ENTITY_INSTANCE) {
      ...
      if (!val.empty()) {
        visit_any(
            [&state](auto &v) {
              state->stack.back().inst()->set_attribute_value(
                  state->stack.back().idx(), v);
            },
            val);
      }
    }
...
static void start_element(void *user, const xmlChar *tag,
                          const xmlChar **attrs) {
  ifcxml_parse_state *state = (ifcxml_parse_state *)user;
    ...
    while (attrs[i] != NULL) {
      if ((i % 2) != 0) {
        ...
        if ((tagname == "ifc:ifcXML" || tagname == "ifcXML") &&
            ...
            (boost::starts_with(
                 value, "http://www.buildingsmart-tech.org/ifcXML/IFC4") ||
             boost::starts_with(
                 value, "http://www.buildingsmart-tech.org/ifc/IFC4"))) {
          // We're expecting a schemaLocation like
          // "http://www.buildingsmart-tech.org/ifcXML/IFC4/Add2
          // IFC4_ADD2_TC1.xsd" With token compression this is split into: [0]
          // http: [1] www.buildingsmart-tech.org [2] ifcXML [3] IFC4 [4] Add2
          // [5] IFC4_ADD2_TC1.xsd
          // The hosstname will likely change though.
          auto it = boost::algorithm::make_split_iterator(
              value, boost::algorithm::token_finder(
          ...
          if (it != end) {
            ...
            state->file =
                new IfcParse::IfcFile(IfcParse::schema_by_name(schema_name));
            state->dialect = ifcxml_dialect_ifc4;
          }
          goto end;
        } else if (tagname == "ex:iso_10303_28" &&
                   attrname == "xsi:schemaLocation" &&
                   boost::starts_with(
                       value, "http://www.iai-tech.org/ifcXML/IFC2x3")) {
          state->file =
              new IfcParse::IfcFile(IfcParse::schema_by_name("IFC2X3"));
  ...
  {
    // ifcXML id attributes are commonly numeric identifiers prefixed with 'i'
    // (as XML identifiers need to start with a alphabetic character). This
    // convention is not always followed, so a mapping is kept from XML string
    // attribute to numeric index into the IfcParse::IfcFile.
    std::string id;

    // Create an attribute value from an instance. Potentially NULL in case it
    // is a forward reference to an instance not yet encountered.
    auto instance_to_attribute =
        [&state](const boost::variant<std::string, IfcUtil::IfcBaseClass *>
                     &inst_or_ref,
                 size_t attribute_index, IfcUtil::IfcBaseClass *&inst) {
          if (inst_or_ref.which() == 0) {
            inst = nullptr;
            // This attribute is NULL initially and after parsing the complete
            // file populated in a subsequent step.
            state->forward_references.push_back(std::make_tuple(
                inst->as<IfcUtil::IfcBaseEntity>(), attribute_index,
                boost::get<std::string>(inst_or_ref)));
          } else {
    ...
    // Create or reference an instance from the file and set attributes based on
    // XML attributes.
    auto create_instance = [&state,
                            &attributes](const IfcParse::declaration *decl) {
      ...
      for (auto &pair : attributes) {
        if (pair.first == "id" || pair.first == "href" || pair.first == "ref") {
          ...
          decl = state->file->schema()
                     ->declaration_by_name(pair.second)
                     ->as_entity();
        }
      }

      auto untyped = IfcEntityInstanceData(storage_t(
          decl->as_entity() != nullptr ? decl->as_entity()->attribute_count()
                                       : 1));

      const IfcParse::entity *entity = decl->as_entity();
      if (entity != nullptr) {
        for (auto &pair : attributes) {
          if (pair.first == "id" || pair.first == "xsi:type" ||
              pair.first == "pos") {
            continue;
          }
          ...
          if (idx != -1) {
            ...
            auto val =
                parse_attribute_value(attr->type_of_attribute(), pair.second);
            if (!val.empty()) {
              visit_any(
                  [&untyped, idx](auto &v) { untyped.storage_.set(idx, v); },
                  val);
            }
          } else {
            Logger::Error("Unknown attribute '" + pair.first + "' on entity '" +
                          entity->name() + "' with value '" + pair.second +
                          "'");
          }
        }
      }
      ...
      IfcUtil::IfcBaseClass *newinst =
          state->file->schema()->instantiate(decl, std::move(untyped));
    ...
    if (state_type == stack_node::node_select) {
      ...
      const IfcParse::parameter_type *element_type =
          attribute_type->as_aggregation_type()->type_of_element();
      ...
      if (element_type->as_simple_type() != nullptr) {
        state->stack.push_back(
            stack_node::aggregate_element(element_type, aggrpos));
      } else {
      ...
      const IfcParse::entity *current =
          state->stack.back().inst()->declaration().as_entity();
      if (current == nullptr) {
        Logger::Error("'" + state->stack.back().inst()->declaration().name() +
                      "' is not an entity, unable to set attribute '" +
                      tagname + "'");
        // We need to push something on the stack. Likely there has been some
        // extra indirection that is not understood.
        ...
        if (idx == -1) {
          ...
          auto found =
              std::find_if(inverses.begin(), inverses.end(),
                           [&tagname](const IfcParse::inverse_attribute *attr) {
          ...
          if (found == inverses.end()) {
            ...
            if ((*found)->bound1() == 0 && (*found)->bound2() == 1) {
              ...
              state->stack.push_back(
                  stack_node::inverse(state->stack.back().inst(), *found));
            }
          }
        } else {
          const IfcParse::parameter_type *attribute_type =
              current->attribute_by_index(idx)->type_of_attribute();
          if (state->dialect == ifcxml_dialect_ifc2x3) {
            follow_named(attribute_type);
            if (attribute_type->as_aggregation_type() != nullptr) {
              state->stack.push_back(
                  stack_node::aggregate(state->stack.back().inst(), idx));
            } else {
              state->stack.push_back(stack_node::instance_attribute(
                  state->stack.back().inst(), idx));
            }
          } else {
            if (IfcUtil::from_parameter_type(attribute_type) ==
                IfcUtil::Argument_ENTITY_INSTANCE) {
              if (const auto *entity = attribute_type->as_named_type()
                                           ->declared_type()
                                           ->as_entity()) {
                ...
                state->stack.push_back(stack_node::instance(
                    id,
                    boost::get<IfcUtil::IfcBaseClass *>(inst_or_reference)));
              } else if (attribute_type->as_named_type()
                             ->declared_type()
                             ->as_select_type() != nullptr) {
                // Select types cause an additional indirection, so the current
                // stack node is simply repeated
                state->stack.push_back(
                    stack_node::select(state->stack.back().inst(), idx));
              }
      ...
      if (state_type == stack_node::node_header) {
        ...
        const IfcParse::entity *entity = decl->as_entity();
        if ((entity == nullptr) &&
            state_type != stack_node::node_instance_attribute) {
        ...
        if (state_type == stack_node::node_inverse) {
          ...
          state->stack.back().inst()->data().storage_.set(
              state->stack.back().idx(), inst);
        }
...
#ifdef WITH_IFCXML
IFC_PARSE_API IfcParse::IfcFile *
IfcParse::parse_ifcxml(const std::string &filename) {
