...
protected:
  ...
  HeaderEntity(const char *const datatype, size_t size,
...
public:
  virtual size_t getArgumentCount() const { return data_.size(); }
...
public:
  ...
  std::vector<std::string> description() const {
    return data_.get_attribute_value(0);
  }
  std::string implementation_level() const {
    return data_.get_attribute_value(1);
  }

  void description(const std::vector<std::string> &value) {
    setValue(0, value);
  }
...
public:
  ...
  std::vector<std::string> author() const {
    return data_.get_attribute_value(2);
  }
  std::vector<std::string> organization() const {
    return data_.get_attribute_value(3);
  }
  std::string preprocessor_version() const {
    return data_.get_attribute_value(4);
  }
  std::string originating_system() const {
    return data_.get_attribute_value(5);
  }
  ...
  void organization(const std::vector<std::string> &value) {
    setValue(3, value);
  }
...
public:
  ...
  std::vector<std::string> schema_identifiers() const {
    return data_.get_attribute_value(0);
  }

  void schema_identifiers(const std::vector<std::string> &value) {
    setValue(0, value);
  }
};
...
private:
  ...
  void readParen();
  void readSemicolon();
  enum Trail { TRAILING_SEMICOLON, TRAILING_PAREN, NONE };
  void readTerminal(const std::string &term, Trail trail);

public:
  explicit IfcSpfHeader(IfcParse::IfcFile *file = nullptr)
      : file_(file), file_description_(0), file_name_(0), file_schema_(0) {
    if (file == nullptr) {
