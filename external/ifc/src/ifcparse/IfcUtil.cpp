...
unsigned int aggregate_of_instance::size() const {
  return (unsigned int)list_.size();
}
void aggregate_of_instance::reserve(unsigned capacity) {
  list_.reserve((size_t)capacity);
}
aggregate_of_instance::it aggregate_of_instance::begin() {
  return list_.begin();
}
...
void aggregate_of_instance::remove(IfcUtil::IfcBaseClass *instance) {
  ...
  while ((iter = std::find(list_.begin(), list_.end(), instance)) !=
         list_.end()) {
    list_.erase(iter);
  }
}

aggregate_of_instance::ptr aggregate_of_instance::filtered(
  ...
  for (it it = begin(); it != end(); ++it) {
    ...
    for (std::set<const IfcParse::declaration *>::const_iterator jt =
             entities.begin();
         jt != entities.end(); ++jt) {
...
Argument::operator int() const { throw IfcParse::IfcException("Argument is not
an integer"); } Argument::operator bool() const { throw
IfcParse::IfcException("Argument is not a boolean"); } Argument::operator
boost::logic::tribool() const { throw IfcParse::IfcException("Argument is not a
logical"); } Argument::operator double() const { throw
IfcParse::IfcException("Argument is not a number"); } Argument::operator
std::string() const { throw IfcParse::IfcException("Argument is not a string");
} Argument::operator boost::dynamic_bitset<>() const { throw
IfcParse::IfcException("Argument is not a binary"); } Argument::operator
IfcUtil::IfcBaseClass*() const { throw IfcParse::IfcException("Argument is not
an entity instance"); } Argument::operator std::vector<double>() const { throw
IfcParse::IfcException("Argument is not a list of floats"); } Argument::operator
std::vector<int>() const { throw IfcParse::IfcException("Argument is not a list
of ints"); } Argument::operator std::vector<std::string>() const { throw
IfcParse::IfcException("Argument is not a list of strings"); }
Argument::operator std::vector<boost::dynamic_bitset<>>() const { throw
IfcParse::IfcException("Argument is not a list of binaries"); }
Argument::operator aggregate_of_instance::ptr() const { throw
IfcParse::IfcException("Argument is not a list of entity instances"); }
Argument::operator std::vector<std::vector<int>>() const { throw
IfcParse::IfcException("Argument is not a list of list of ints"); }
Argument::operator std::vector<std::vector<double>>() const { throw
IfcParse::IfcException("Argument is not a list of list of floats"); }
Argument::operator aggregate_of_aggregate_of_instance::ptr() const { throw
IfcParse::IfcException("Argument is not a list of list of entity instances"); }
*/
...
void IfcUtil::sanitate_material_name(std::string &str) {
  // Spaces in material names have been observed to cause problems with obj and
  // dae importers. Handle other potential problematic characters here too if
  // observing problems.
...
void IfcUtil::IfcBaseEntity::populate_derived() {
  for (auto it = declaration().as_entity()->derived().begin();
       it != declaration().as_entity()->derived().end(); ++it) {
    if (*it) {
...
IfcUtil::IfcBaseEntity::get_inverse(const std::string &name) const {
  if (file_ == nullptr) {
    throw IfcParse::IfcException("Instance not added to file");
  }
  const std::vector<const IfcParse::inverse_attribute *> attrs =
      declaration().as_entity()->all_inverse_attributes();
  std::vector<const IfcParse::inverse_attribute *>::const_iterator iter =
      attrs.begin();
  for (; iter != attrs.end(); ++iter) {
    if ((*iter)->name() == name) {
      return file_->getInverse(
          id_, (*iter)->entity_reference(),
          (int)(*iter)->entity_reference()->attribute_index(
...
IfcUtil::from_parameter_type(const IfcParse::parameter_type *pt) {
  // TODO: How to detect derived types here without a reference to the refering
  // entity?
  ...
  if (nt != nullptr) {
    ...
    if (nt->declared_type()->as_type_declaration() != nullptr) {
      return from_parameter_type(
          nt->declared_type()->as_type_declaration()->declared_type());
    }
  } else if (st != nullptr) {
...
std::string IfcUtil::path::to_utf8(const std::wstring &str) {
  int buffer_size =
      WideCharToMultiByte(CP_UTF8, 0, str.c_str(), -1, 0, 0, 0, 0);
...
IFC_PARSE_API bool IfcUtil::path::rename_file(const std::string &old_filename,
                                              const std::string &new_filename) {
  std::wstring old_filename_w = from_utf8(old_filename);
  std::wstring new_filename_w = from_utf8(new_filename);
  delete_file(new_filename);
  const bool success =
      !!MoveFileW(old_filename_w.c_str(), new_filename_w.c_str());
  return success;
}
...
IFC_PARSE_API bool IfcUtil::path::rename_file(const std::string &old_filename,
                                              const std::string &new_filename) {
  // Whether or not rename() replaces an existing file is
  // implementation-specific, so remove() possible existing file always.
  delete_file(new_filename);
