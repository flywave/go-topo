...
template <class T> class aggregate_of;
...
public:
  ...
  template <class U> typename U::list::ptr as();

  void remove(IfcUtil::IfcBaseClass *);
  aggregate_of_instance::ptr
  filtered(const std::set<const IfcParse::declaration *> &entities);
...
public:
  ...
  void push(ptr instance) {
    if (instance) {
      for (typename T::list::it it = instance->begin(); it != instance->end();
           ++it) {
        push(*it);
      }
    }
  ...
  bool contains(T *type) const {
    return std::find(list_.begin(), list_.end(), type) != list_.end();
  }
  template <class U> typename U::list::ptr as() {
  ...
  void remove(T *type) {
    ...
    while ((iter = std::find(list_.begin(), list_.end(), type)) !=
           list_.end()) {
      list_.erase(iter);
    }
  }
...
template <class T> class aggregate_of_aggregate_of;
...
public:
  ...
  void push(const aggregate_of_instance::ptr &instance) {
    if (instance) {
      ...
      for (std::vector<IfcUtil::IfcBaseClass *>::const_iterator iter =
               instance->begin();
  ...
  template <class U> typename aggregate_of_aggregate_of<U>::ptr as();
};
...
public:
  ...
  aggregate_of_aggregate_of_instance::ptr generalize() {
    aggregate_of_aggregate_of_instance::ptr result(
