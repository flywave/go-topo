...
bool IfcParse::declaration::is(const std::string &name) const {
  ...
  if ((this->as_entity() != nullptr) &&
      (this->as_entity()->supertype() != nullptr)) {
    return this->as_entity()->supertype()->is(name);
  }
  if (this->as_type_declaration() != nullptr) {
    const IfcParse::named_type *named_type =
        this->as_type_declaration()->declared_type()->as_named_type();
...
IfcParse::schema_definition::schema_definition(
  ...
  std::sort(declarations_.begin(), declarations_.end(),
            declaration_by_index_sort());
  for (std::vector<const declaration *>::iterator it = declarations_.begin();
...
IfcParse::schema_definition::~schema_definition() {
  for (std::vector<const declaration *>::const_iterator it =
           declarations_.begin();
       it != declarations_.end(); ++it) {
    delete *it;
  }
...
IfcParse::schema_definition::instantiate(const IfcParse::declaration *decl,
...
IfcParse::schema_by_name(const std::string &name) {
  ...
  std::map<std::string, const IfcParse::schema_definition *>::const_iterator
      iter = schemas.find(boost::to_upper_copy(name));
  if (iter == schemas.end()) {
...
void IfcParse::clear_schemas() {
  ...
  // we pop schemas until map is empty, because map iteration is invalidated
  // after each erasure
  while (!schemas.empty()) {
