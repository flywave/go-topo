...
public:
  ...
  int operator()(const empty_aggregate_of_aggregate_t & /*unused*/) const {
    return 0;
  }
  ...
  int operator()(const std::vector<std::vector<int>> &i) const {
    return (int)i.size();
  }
  int operator()(const std::vector<std::vector<double>> &i) const {
    return (int)i.size();
  }
  int operator()(const std::vector<std::string> &i) const {
    return (int)i.size();
  }
  int operator()(const std::vector<boost::dynamic_bitset<>> &i) const {
    return (int)i.size();
  }
  int operator()(const EnumerationReference & /*i*/) const { return -1; }
  int operator()(const IfcUtil::IfcBaseClass *const & /*i*/) const {
    return -1;
  }
  int operator()(const aggregate_of_instance::ptr &i) const {
    return i->size();
  }
  int operator()(const aggregate_of_aggregate_of_instance::ptr &i) const {
    return i->size();
  }
};

AttributeValue::operator int() const { return array_->get<int>(index_); }

AttributeValue::operator bool() const { return array_->get<bool>(index_); }

AttributeValue::operator double() const { return array_->get<double>(index_); }
...
AttributeValue::operator std::string() const {
  if (array_->has<EnumerationReference>(index_)) {
    ...
    // @todo also we don't really need to store a reference to the enumeration
    // type, when this same type is already stored on the definition of the
    // entity and no other value can be provided.
...
AttributeValue::operator boost::shared_ptr<aggregate_of_aggregate_of_instance>()
    const {
  return array_->get<boost::shared_ptr<aggregate_of_aggregate_of_instance>>(
      index_);
}

bool AttributeValue::isNull() const { return array_->has<Blank>(index_); }
